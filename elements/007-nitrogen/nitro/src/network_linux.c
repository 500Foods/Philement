...
static int nitro_count_interfaces() __attribute__((unused));
static int nitro_count_interfaces() {
    struct ifaddrs *ifaddr;
    if (getifaddrs(&ifaddr) == -1) {
        nitro_log("ERROR", "getifaddrs failed: %s", strerror(errno));
        return 0;
    }
...
nitro_network_info_t *nitro_get_network_info() {
    nitro_network_info_t *info = malloc(sizeof(nitro_network_info_t));
    if (!info) {
        perror("malloc");
        return NULL;
    }
    ...
    for (struct ifaddrs *ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        if (ifa->ifa_addr && (ifa->ifa_addr->sa_family == AF_INET || ifa->ifa_addr->sa_family == AF_INET6)) {
            ...
            if (new_interface && info->count < NITRO_MAX_INTERFACES) {
            ...
            if (interface_index != -1 && info->interfaces[interface_index].ip_count < NITRO_MAX_IPS) {
