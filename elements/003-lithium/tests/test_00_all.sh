...
# CHANGELOG
# 9.0.0 - 2025-12-31 - Updatted for Lithium project
...
# Test configuration
TEST_NAME="Lithium Test Suite Orchestration"
TEST_ABBR="ORC"
TEST_NUMBER="00"
TEST_COUNTER=0
TEST_VERSION="8.0.0"
...
source "${LITHIUM_ROOT}/tests/lib/framework.sh"
...
# Cache directory
CACHE_CMD_DIR="${HOME}/.cache/lithium/commands"
mkdir -p "${CACHE_CMD_DIR}"
export CACHE_CMD_DIR

# SVG locations
# shellcheck disable=SC2154 # PROJECT_DIR defined externally in framework.sh
results_svg_path="${PROJECT_DIR}/tests/images//COMPLETE.svg"
coverage_svg_path="${PROJECT_DIR}/tests/images/COVERAGE.svg"
...
layout_json_content='{
    "title": "{BOLD}{WHITE}Lithium Test Suite Results for Build '"${BUILD_NUMBER}"'%{RESET}",
...
metrics_dir="${LITHIUM_DOCS_ROOT}/metrics/${current_month}"
...
cloc_json_main=""
cloc_json_stats=""
if cloc_output=$(env -i LANG="${LANG}" LITHIUM_ROOT="${LITHIUM_ROOT}" LITHIUM_DOCS_ROOT="${LITHIUM_DOCS_ROOT}" bash -c "${SCRIPT_DIR}/lib/cloc_tables.sh"); then
