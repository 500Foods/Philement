...
# CHANGELOG                          
# 5.0.0 - 2025-12-31 - Updated for Lithium project
...
process_message() {
    ...
    # Replace LITHIUM_ROOT with relative path (anywhere in the string)
    if [[ -n "${LITHIUM_ROOT:-}" ]]; then
        processed_message="${processed_message//${LITHIUM_ROOT}/}"
    fi

    # Replace LITHIUM_DOCS_ROOT with relative path (anywhere in the string)
    if [[ -n "${LITHIUM_DOCS_ROOT:-}" ]]; then
        processed_message="${processed_message//${LITHIUM_DOCS_ROOT}/}"
    fi
