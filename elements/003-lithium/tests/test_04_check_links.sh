...
print_command "${TEST_NUMBER}" "${TEST_COUNTER}" ".${SITEMAP} ${TARGET_README} --webroot '${PHILEMENT_ROOT}' --include '${LITHIUM_ROOT}' --include '${LITHIUM_DOCS_ROOT}' --noreport --quiet"

# Run github-sitemap.sh with webroot and include options
SITEMAP_EXIT_CODE=0
if ! "${SITEMAP}" "${TARGET_README}" --webroot "${PHILEMENT_ROOT}" --include "${LITHIUM_ROOT}" --include "${LITHIUM_DOCS_ROOT}" --noreport --quiet > "${MARKDOWN_CHECK}" 2>&1; then
    SITEMAP_EXIT_CODE=$?
else
    SITEMAP_EXIT_CODE=0
fi
...
# Find all .md files under LITHIUM_ROOT and LITHIUM_DOCS_ROOT
RELATIVE_LINK_FILES=()
while IFS= read -r -d '' file; do
    RELATIVE_LINK_FILES+=("${file}")
done < <(find "${LITHIUM_ROOT}" "${LITHIUM_DOCS_ROOT}" -name "*.md" -type f -print0 2>/dev/null || true)
