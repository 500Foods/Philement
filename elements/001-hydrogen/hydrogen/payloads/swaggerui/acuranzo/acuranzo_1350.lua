-- Migration: acuranzo_1350.lua
-- PRIORITIZE 2.35: Stripe.Checkout fail-closed on retired
--
-- Do not restamp 1317.
--
...
--
-- CHANGELOG
-- 1.0.0 - 2026-08-21 - Checkout not_offered when retired (PRIORITIZE 2.35)
...
table.insert(queries,{sql=[====[
    ...
    SELECT
        ...
        [=[
            ...
            SET code = [==[
-- Stripe.Checkout (STRIPE_PLAN Phase 5; 1317 bind fix; 1350 retired)
...
local function load_course(course_id)
    local _cr, err = H.query_sync([[
        SELECT course_id, pricing_type, canvas_course_id, stripe_product_id, retired
        FROM ${SCHEMA}courses
...
return 0
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Checkout fail-closed on retired' AS name,
        [=[
            # Forward Migration ${MIGRATION}: Checkout fail-closed on retired=1 (do not restamp 1317)
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
...
return 0
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_APPLIED_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Restore Stripe.Checkout without retired check' AS name,
        [=[
            # Reverse Migration ${MIGRATION}: Restore prior Stripe.Checkout
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
