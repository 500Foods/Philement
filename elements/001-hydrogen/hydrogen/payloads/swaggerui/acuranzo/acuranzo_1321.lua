-- Migration: acuranzo_1321.lua
-- PRIORITIZE 2.8: Enroll.FreeCourse writes user_enrollments
--
-- Do not restamp 1300. Canvas enroll is unchanged. Helium row is
-- source='free', renew_policy='free_renew', expires_at=now()+90d.
-- Insert only when no row exists for (account_id, course_id).
--
-- luacheck: no max line length
-- luacheck: no unused args
--
-- CHANGELOG
-- 1.0.0 - 2026-08-20 - Replace Enroll.FreeCourse body (Helium entitlement write)
...
table.insert(queries,{sql=[====[
    ...
    SELECT
        ...
        [=[
            ...
            SET code = [==[
-- Enroll.FreeCourse (Phase 49; 1321 user_enrollments writer)
...
return succeed(eerr == "already", canvas_user_id, canvas_course_id)
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'FreeCourse writes user_enrollments'                                                        AS name,
        [=[
            # Forward Migration ${MIGRATION}: Enroll.FreeCourse Helium entitlement write

            After a successful (or already-enrolled) Canvas seat,
            insert a `user_enrollments` row when none exists for
            this account+course. Archived / superseded rows are
            not resurrected. No diagram (script restamp).
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
...
return succeed(eerr == "already", canvas_user_id, canvas_course_id)
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_APPLIED_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Restore Enroll.FreeCourse 1300 body'                                                        AS name,
        [=[
            # Reverse Migration ${MIGRATION}: Restore Enroll.FreeCourse 1300 body
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
