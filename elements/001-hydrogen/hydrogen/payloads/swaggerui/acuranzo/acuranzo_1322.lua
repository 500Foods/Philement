-- Migration: acuranzo_1322.lua
-- PRIORITIZE 2.8: Provision.EnsureCanvasUser writes intro user_enrollments
--
-- Do not restamp 1290. Intro Canvas enroll is unchanged. Helium row
-- is source='intro', renew_policy='free_renew', expires_at NULL.
...
--
-- CHANGELOG
-- 1.0.0 - 2026-08-20 - Replace EnsureCanvasUser body (intro Helium entitlement write)
...
H.set_current_state("done")
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'EnsureCanvasUser writes intro user_enrollments'                                                        AS name,
        [=[
            # Forward Migration ${MIGRATION}: EnsureCanvasUser intro Helium entitlement write

            After the Phase 31 intro Canvas enroll, insert a
            `user_enrollments` row when none exists. Catalog lookup
            is by `courses.canvas_course_id`. No diagram.
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
