-- Migration: acuranzo_1338.lua
-- PRIORITIZE 2.29: QueryRefs 147/148 SELECT tags
--
-- Do not restamp 1313 (already APPLIED). Adds c.tags to the catalog
-- list/detail public queries. Typed columns stay.
--
-- luacheck: no max line length
-- luacheck: no unused args
--
-- CHANGELOG
-- 1.0.0 - 2026-08-20 - Add tags to catalog QueryRefs 147/148
        ...
        [=[
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Add tags to QueryRefs 147/148'                                     AS name,
        [=[
            # Forward Migration ${MIGRATION}: Catalog tags JSON

            PRIORITIZE 2.29. Replaces the SELECT body of public
            QueryRefs **147** (list) and **148** (by slug) so they
            return `tags` (JSON object) alongside the typed columns.
            Reception `normalizeCourse` overlays known keys. No new
            C route. #147 still `published=1` only (2.27 is #148
            enrolled bypass, not this item).
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
...
-- ----------------------------------------------------------------------------
-- Reverse: restore 1313 SELECT lists (stripe_price_id, no tags)
-- ----------------------------------------------------------------------------
table.insert(queries,{sql=[====[
        ...
        [=[
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_APPLIED_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Restore QueryRefs 147/148 without tags'                            AS name,
        [=[
            # Reverse Migration ${MIGRATION}: Restore 1313 catalog SELECTs
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
