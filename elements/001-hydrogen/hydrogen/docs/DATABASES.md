...
- **BROTLI.** Unfortunately, it turns out that DB2 has a hard limit on how big a string can be when the DB2 parser first analyzes an SQL statement - 32 KB. There are no such restrictions when passing parameters to DB2. So not the biggest problem. But it turns out that the migrations contain themes - large CSS blocks (ironically around 30 KB) and larger still when encoded as Base64. To get around this limit, and to be a tiny bit more efficient in terms of storage, the Lua multi-line strings that are greater than 1,000 characters are now first compressed with `brotli --best` and then encoded as Base64. This necessitated changes all the way downstream from there, specifically including the test scripts ([Test 31: Migrations](../tests/docs/test_31_migrations.md) and [Test 39: Database Diagrams](../tests/docs/test_39_database_diagrams.md) in particular), as well as in each database engine where these compressed+encoded strings needed to be decoded+decompressed before they could be inserted into the database. As a result we now have a matching set of brotli-decompress UDFs for our four database engines. Handy.
...
[Test 31: Migrations](../tests/docs/test_31_migrations.md) runs sqruff validation on migration scripts. The individual engine migration tests ([Test 32: PostgreSQL Migrations](../tests/docs/test_32_postgres_migrations.md), [Test 33: MySQL Migrations](../tests/docs/test_33_mysql_migrations.md), [Test 34: SQLite Migrations](../tests/docs/test_34_sqlite_migrations.md), [Test 35: DB2 Migrations](../tests/docs/test_35_db2_migrations.md)) perform the full LOAD/APPLY/REVERSE cycle to validate that schema changes execute successfully and can be rolled back.

For more details on Lua scripting, sqruff usage, and the migration system, see [CURIOSITIES.md](../CURIOSITIES.md).
...
- **BROTLI.** Custom [BROTLI UDF](../extras/brotli_udf_postgresql/README.md) for decoding brotli data.
...
- **BROTLI.** Custom [BROTLI UDF](../extras/brotli_udf_mysql/README.md) for decoding brotli data.
...
- **BROTLI.** Custom [BROTLI UDF](../extras/brotli_udf_sqlite/README.md) for decoding brotli data.
...
- **BROTLI.** Custom [BROTLI UDF](../extras/brotli_udf_db2/README.md) for decoding brotli data.
