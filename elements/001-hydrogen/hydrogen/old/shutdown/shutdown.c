...
void graceful_shutdown(void) {
    ...
    // Format current time with ISO 8601
    ...
    time_t start_sec = tv.tv_sec - (time_t)shutdown_time;
    ...
    update_subsystem_on_shutdown("Registry");
    ...
    // Handle restart or shutdown
    if (restart_requested) {
        // Reset shutdown flags for landing system
        __sync_bool_compare_and_swap(&server_stopping, 1, 0);
        __sync_bool_compare_and_swap(&shutdown_in_progress, 1, 0);
        __sync_synchronize();
        
        // Landing system will handle the restart
        return;
    }
