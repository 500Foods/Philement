...
// ISO 8601 unchanged. For DB_ENGINE_MYSQL and DB_ENGINE_SQLITE the helper translates the value to
// 'YYYY-MM-DD HH:MM:SS' (truncating fractional seconds, dropping trailing 'Z') before storing it
// under the STRING bucket, because both engines store DATETIME as text and string comparisons in
// claim/recover queries fail when ISO 8601 'T'/'Z' format is used. For every other engine (PostgreSQL,
// DB2, CockroachDB, YugabyteDB) the value passes through unchanged so they keep their existing shape.
// `database_name` resolves to a DatabaseConnection whose `type` selects the engine. Pass NULL or
// an unrecognised name to fall through unchanged (safe).
bool repo_add_datetime(json_t* root, const char* name, const char* iso8601,
                       const char* database_name);

// PERSIST_PLAN Phase 2c: low-level ISO 8601 -> text DATETIME translator for engines that store
// timestamps as TEXT (MySQL DATETIME, SQLite text). Exposed (non-static) so Unity tests can
// exercise the transform directly without a full app_config / DatabaseConnection fixture.
// The function is pure: returns a heap-allocated string the caller must free, or NULL on
// allocation failure. NULL/empty inputs and values without a 'T' at position 10 are passed
// through unchanged (so the helper is idempotent for already-formatted values and never destroys
