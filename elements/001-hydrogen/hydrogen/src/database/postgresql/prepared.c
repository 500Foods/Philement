...
bool postgresql_prepare_statement(DatabaseHandle* connection, const char* name, const char* sql, PreparedStatement** stmt, bool add_to_cache) {
    ...
    if (!connection->config || connection->config->prepared_statement_cache_size <= 0) {
        cache_size = 1000; // Default cache size when config is NULL, 0, or negative
    } else {
...
bool postgresql_unprepare_statement(DatabaseHandle* connection, PreparedStatement* stmt) {
    ...
    // NOTE: We do NOT issue DEALLOCATE here because:
    // 1. PostgreSQL automatically deallocates prepared statements when connection closes
    // 2. Issuing DEALLOCATE during LRU eviction inside a transaction can cause transaction abort
    //    if the statement doesn't exist (which happens frequently during cache churn)
    // 3. Transaction abort invalidates ALL statements in that transaction, including newly prepared ones
    //
    // This is different from DB2/MySQL/SQLite which require explicit cleanup of handles
    ...
    log_this(connection->designator ? connection->designator : SR_DATABASE,
             "PostgreSQL prepared statement removed from cache", LOG_LEVEL_TRACE, 0);
    return true;
}