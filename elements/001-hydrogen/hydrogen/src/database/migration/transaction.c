...
/*
 * Firebird metadata (CREATE/ALTER/DROP/RECREATE) is only visible to later
 * DSQL after commit. Detect leading DDL keywords (skip whitespace/comments).
 */
bool firebird_migration_sql_is_ddl(const char* sql) {
    ...
    static const char* const kws[] = { "CREATE", "ALTER", "DROP", "RECREATE", NULL };
    for (size_t i = 0; kws[i]; i++) {
        size_t n = strlen(kws[i]);
        if (strncasecmp(p, kws[i], n) == 0) {
...
bool execute_firebird_migration(DatabaseHandle* connection, char** statements, size_t statement_count,
                              const char* migration_file, const char* dqm_label) {
    /*
     * Firebird does not expose uncommitted DDL to later DSQL in the same
     * transaction (metadata cache). Commit after each successful DDL so
     * following DML can see new objects; keep consecutive DML in one
     * transaction so batches stay atomic. Prior DDL commits stay applied
     * if a later statement fails (Firebird migration reality).
     */
    log_this(dqm_label, "Starting Firebird migration %s (%zu statements, commit-after-DDL)", LOG_LEVEL_TRACE, 2, migration_file, statement_count);

    Transaction* fb_transaction = NULL;
    if (!database_engine_begin_transaction(connection, DB_ISOLATION_READ_COMMITTED, &fb_transaction)) {
        log_this(dqm_label, "Failed to begin Firebird transaction for migration %s", LOG_LEVEL_ERROR, 1, migration_file);
        return false;
    }
    ...
    for (size_t j = 0; j < statement_count; j++) {
        ...
        if (!stmt_request) {
            log_this(dqm_label, "Failed to allocate statement request", LOG_LEVEL_ERROR, 0);
            (void)database_engine_rollback_transaction(connection, fb_transaction);
            database_engine_cleanup_transaction(fb_transaction);
            return false;
        }
        ...
        bool ok = stmt_success && stmt_result && stmt_result->success;
        if (ok) {
            log_this(dqm_label, "Statement %zu executed successfully (hash: %s): affected %lld rows", LOG_LEVEL_TRACE, 3, j + 1, stmt_hash, (long long)stmt_result->affected_rows);
        } else {
            log_this(dqm_label, "Statement %zu failed (hash: %s)", LOG_LEVEL_ERROR, 2, j + 1, stmt_hash);
        }
        ...
        if (!ok) {
            if (!database_engine_rollback_transaction(connection, fb_transaction)) {
                log_this(dqm_label, "Failed to rollback migration %s after statement %zu", LOG_LEVEL_ERROR, 2, migration_file, j + 1);
            } else {
                log_this(dqm_label, "Migration %s rolled back open transaction after statement %zu (earlier DDL already committed)", LOG_LEVEL_TRACE, 2, migration_file, j + 1);
            }
            database_engine_cleanup_transaction(fb_transaction);
            return false;
        }

        if (firebird_migration_sql_is_ddl(statements[j])) {
            if (!database_engine_commit_transaction(connection, fb_transaction)) {
                log_this(dqm_label, "Failed to commit migration %s after DDL statement %zu", LOG_LEVEL_ERROR, 2, migration_file, j + 1);
                database_engine_cleanup_transaction(fb_transaction);
                return false;
            }
            database_engine_cleanup_transaction(fb_transaction);
            ...
            if (j + 1 < statement_count) {
                if (!database_engine_begin_transaction(connection, DB_ISOLATION_READ_COMMITTED, &fb_transaction)) {
                    log_this(dqm_label, "Failed to begin Firebird transaction after DDL in migration %s (next statement %zu)", LOG_LEVEL_ERROR, 2, migration_file, j + 2);
                    return false;
                }
            }
        }
    ...
    if (fb_transaction) {
        if (!database_engine_commit_transaction(connection, fb_transaction)) {
            log_this(dqm_label, "Failed to commit migration %s final DML batch", LOG_LEVEL_ERROR, 1, migration_file);
            database_engine_cleanup_transaction(fb_transaction);
            return false;
        }
        database_engine_cleanup_transaction(fb_transaction);
