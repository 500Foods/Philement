...
void free_payload_resources(void) {
    ...
    log_this(SR_LANDING, "LANDING: PAYLOAD", LOG_LEVEL_STATE);
    
    // Free any resources allocated during payload launch
    log_this(SR_LANDING, "  Step 1: Freeing payload resources", LOG_LEVEL_STATE);
    
    // Call the payload cleanup function for OpenSSL
    cleanup_openssl();
    log_this(SR_LANDING, "  Step 2: OpenSSL resources cleaned up", LOG_LEVEL_STATE);
    
    // Update the registry that payload has been shut down
    update_subsystem_after_shutdown("Payload");
    log_this(SR_LANDING, "  Step 3: Payload subsystem marked as inactive", LOG_LEVEL_STATE);
    
    log_this(SR_LANDING, "LANDING: PAYLOAD cleanup complete", LOG_LEVEL_STATE);
}
...
int land_payload_subsystem(void) {
    ...
    if (subsys_id < 0 || !is_subsystem_running(subsys_id)) {
        log_this(SR_LANDING, "Payload not running, skipping shutdown", LOG_LEVEL_STATE);
        return 1;  // Success - nothing to do
    }
    
    // Step 1: Mark as stopping
    update_subsystem_state(subsys_id, SUBSYSTEM_STOPPING);
    log_this(SR_LANDING, "LANDING: PAYLOAD - Beginning shutdown sequence", LOG_LEVEL_STATE);
    ...
    if (final_state == SUBSYSTEM_INACTIVE) {
        log_this(SR_LANDING, "LANDING: PAYLOAD - Successfully landed and ready for future restart", LOG_LEVEL_STATE);
    } else {
        log_this(SR_LANDING, "LANDING: PAYLOAD - Warning: Unexpected final state: %s", LOG_LEVEL_ALERT,
