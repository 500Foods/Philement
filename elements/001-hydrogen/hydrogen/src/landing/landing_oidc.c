...
static void free_oidc_resources(void) {
    ...
    log_this(SR_LANDING, "LANDING: OIDC", LOG_LEVEL_STATE);
    
    // Check if OIDC is enabled before attempting cleanup
    if (app_config && !app_config->oidc.enabled) {
        log_this(SR_LANDING, "  Step 1: OIDC disabled, skipping cleanup", LOG_LEVEL_STATE);
        update_subsystem_after_shutdown("OIDC");
        log_this(SR_LANDING, "  Step 2: OIDC subsystem marked as inactive", LOG_LEVEL_STATE);
        log_this(SR_LANDING, "LANDING: OIDC cleanup complete", LOG_LEVEL_STATE);
        return;
    }

    // Clean up OIDC service resources
    log_this(SR_LANDING, "  Step 1: Stopping OIDC authentication service", LOG_LEVEL_STATE);

    // Clean up any active OIDC sessions or tokens
    log_this(SR_LANDING, "  Step 2: Clearing active OIDC sessions", LOG_LEVEL_STATE);
    // Any cached authentication sessions would be cleared here

    log_this(SR_LANDING, "  Step 3: Revoking cached tokens", LOG_LEVEL_STATE);
    // Any cached access/refresh tokens would be revoked here

    log_this(SR_LANDING, "  Step 4: Closing OIDC network connections", LOG_LEVEL_STATE);
    // Any HTTP connections to OIDC provider would be closed here

    log_this(SR_LANDING, "  Step 5: Clearing OIDC key cache", LOG_LEVEL_STATE);
    // Any cached encryption keys would be cleared here

    log_this(SR_LANDING, "  Step 6: Freeing OIDC configuration", LOG_LEVEL_STATE);
    // OIDC-specific configuration resources would be freed here

    // Update the registry that OIDC has been shut down
    update_subsystem_after_shutdown("OIDC");
    log_this(SR_LANDING, "  Step 7: OIDC subsystem marked as inactive", LOG_LEVEL_STATE);
...
int land_oidc_subsystem(void) {
    ...
    if (subsys_id < 0 || !is_subsystem_running(subsys_id)) {
        log_this(SR_LANDING, "OIDC not running, skipping shutdown", LOG_LEVEL_STATE);
        return 1;  // Success - nothing to do
    }

    // Step 1: Mark as stopping
    update_subsystem_state(subsys_id, SUBSYSTEM_STOPPING);
    log_this(SR_LANDING, "LANDING: OIDC - Beginning shutdown sequence", LOG_LEVEL_STATE);
    ...
    if (final_state == SUBSYSTEM_INACTIVE) {
        log_this(SR_LANDING, "LANDING: OIDC - Successfully landed and ready for future restart", LOG_LEVEL_STATE);
    } else {
        log_this(SR_LANDING, "LANDING: OIDC - Warning: Unexpected final state: %s", LOG_LEVEL_ALERT,
