...
static void free_notify_resources(void) {
    ...
    if (app_config && !app_config->notify.enabled) {
        log_this("Landing", "  Step 1: Notify disabled, skipping cleanup", LOG_LEVEL_STATE);
        update_subsystem_after_shutdown("Notify");
        log_this("Landing", "  Step 2: Notify subsystem marked as inactive", LOG_LEVEL_STATE);
        log_this("Landing", "LANDING: NOTIFY cleanup complete", LOG_LEVEL_STATE);
        return;
    }

    // Clean up notification service resources
    log_this("Landing", "  Step 1: Stopping notification service", LOG_LEVEL_STATE);
    ...
    log_this("Landing", "  Step 2: Clearing notification templates", LOG_LEVEL_STATE);
    // Any cached notification templates would be freed here

    log_this("Landing", "  Step 3: Closing notification connections", LOG_LEVEL_STATE);
    // Any persistent connections (SMTP, etc.) would be closed here

    // Update the registry that notify has been shut down
    update_subsystem_after_shutdown("Notify");
    log_this("Landing", "  Step 4: Notify subsystem marked as inactive", LOG_LEVEL_STATE);
...
int land_notify_subsystem(void) {
    ...
    if (subsys_id < 0 || !is_subsystem_running(subsys_id)) {
        log_this("Landing", "Notify not running, skipping shutdown", LOG_LEVEL_STATE);
        return 1;  // Success - nothing to do
    }

    // Step 1: Mark as stopping
    update_subsystem_state(subsys_id, SUBSYSTEM_STOPPING);
    log_this("Landing", "LANDING: NOTIFY - Beginning shutdown sequence", LOG_LEVEL_STATE);
    ...
    if (final_state == SUBSYSTEM_INACTIVE) {
        log_this("Landing", "LANDING: NOTIFY - Successfully landed and ready for future restart", LOG_LEVEL_STATE);
    } else {
        log_this("Landing", "LANDING: NOTIFY - Warning: Unexpected final state: %s", LOG_LEVEL_ALERT,
