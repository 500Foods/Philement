...
static void free_resources_resources(void) {
    ...
    log_this(SR_LANDING, "  Step 1: Stopping resource monitoring threads", LOG_LEVEL_STATE, 0);
    ...
    log_this(SR_LANDING, "  Step 2: Freeing resource configuration", LOG_LEVEL_STATE, 0);
    // Resource configuration is part of app_config and will be cleaned up
    // when the main config is freed during final shutdown

    log_this(SR_LANDING, "  Step 3: Clearing resource limits", LOG_LEVEL_STATE, 0);
    ...
    log_this(SR_LANDING, "  Step 4: Resources subsystem marked as inactive", LOG_LEVEL_STATE, 0);

    log_this(SR_LANDING, "LANDING: RESOURCES cleanup complete", LOG_LEVEL_STATE, 0);
}
...
int land_resources_subsystem(void) {
    ...
    if (subsys_id < 0 || !is_subsystem_running(subsys_id)) {
        log_this(SR_LANDING, "Resources not running, skipping shutdown", LOG_LEVEL_STATE, 0);
        return 1;  // Success - nothing to do
    }

    // Step 1: Mark as stopping
    update_subsystem_state(subsys_id, SUBSYSTEM_STOPPING);
    log_this(SR_LANDING, "LANDING: RESOURCES - Beginning shutdown sequence", LOG_LEVEL_STATE, 0);
    ...
    if (final_state == SUBSYSTEM_INACTIVE) {
        log_this(SR_LANDING, "LANDING: RESOURCES - Successfully landed and ready for future restart", LOG_LEVEL_STATE, 0);
    } else {
        log_this(SR_LANDING, "LANDING: RESOURCES - Warning: Unexpected final state: %s", LOG_LEVEL_ALERT, 1, subsystem_state_to_string(final_state));
    }
