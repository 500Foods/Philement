...
int land_api_subsystem(void) {
    ...
    log_this(SR_API, "LANDING: API", LOG_LEVEL_STATE, 0);

    bool success = true;

    // Step 1: Verify state
    log_this(SR_API, "  Step 1: Verifying state", LOG_LEVEL_STATE, 0);
    if (!is_subsystem_running_by_name(SR_API)) {
        log_this(SR_API, "API already shut down", LOG_LEVEL_STATE, 0);
        log_this(SR_API, "LANDING: API - Already landed", LOG_LEVEL_STATE, 0);
        return 1;
    }
    log_this(SR_API, "    State verified", LOG_LEVEL_STATE, 0);

    // Step 2: Clean up API resources
    log_this(SR_API, "  Step 2: Cleaning up API resources", LOG_LEVEL_STATE, 0);

    // Clean up API endpoints
    cleanup_api_endpoints();
    log_this(SR_API, "    API endpoints cleaned up", LOG_LEVEL_STATE, 0);

    // Step 3: Update registry state
    log_this(SR_API, "  Step 3: Updating registry state", LOG_LEVEL_STATE, 0);
    update_subsystem_on_shutdown(SR_API);

    log_this(SR_API, "LANDING: API - Successfully landed", LOG_LEVEL_STATE, 0);
