...
static SystemMetrics* collect_all_metrics(const WebSocketMetrics *ws_metrics) {
    ...
    if (!metrics) {
        log_this(SR_STATUS, "Failed to allocate metrics structure", LOG_LEVEL_ERROR);
        return NULL;
    }

    // Collect system metrics
    if (!collect_system_info(metrics)) {
        log_this(SR_STATUS, "Failed to collect system info", LOG_LEVEL_ERROR);
        goto error;
    }

    if (!collect_cpu_metrics(&metrics->cpu)) {
        log_this(SR_STATUS, "Failed to collect CPU metrics", LOG_LEVEL_ERROR);
        goto error;
    }

    if (!collect_memory_metrics(&metrics->memory)) {
        log_this(SR_STATUS, "Failed to collect memory metrics", LOG_LEVEL_ERROR);
        goto error;
    }

    if (!collect_network_metrics(&metrics->network)) {
        log_this(SR_STATUS, "Failed to collect network metrics", LOG_LEVEL_ERROR);
        goto error;
    }

    if (!collect_filesystem_metrics(&metrics->filesystems, &metrics->filesystem_count)) {
        log_this(SR_STATUS, "Failed to collect filesystem metrics", LOG_LEVEL_ERROR);
        goto error;
    }

    // Collect process metrics
    if (!collect_file_descriptors(&metrics->file_descriptors, &metrics->fd_count)) {
        log_this(SR_STATUS, "Failed to collect file descriptors", LOG_LEVEL_ERROR);
        goto error;
    }

    size_t vmsize, vmrss, vmswap;
    if (!get_process_memory(&vmsize, &vmrss, &vmswap)) {
        log_this(SR_STATUS, "Failed to collect process memory metrics", LOG_LEVEL_ERROR);
        goto error;
    }
    ...
    if (!collect_service_metrics(metrics, ws_metrics)) {
        log_this(SR_STATUS, "Failed to collect service metrics", LOG_LEVEL_ERROR);
        goto error;
    }
