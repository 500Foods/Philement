...
void get_process_memory(size_t *vmsize, size_t *vmrss, size_t *vmswap) {
    ...
    *vmswap = 0;
    ...
    if (!status) {
        log_this("MemoryMetrics", "Failed to open /proc/self/status", LOG_LEVEL_DEBUG);
        return;
    }
...
json_t* get_file_descriptors_json(void) {
    ...
    if (!dir) {
        log_this("Utils", "Failed to open /proc/self/fd", LOG_LEVEL_DEBUG);
        return fd_array;
    }
...
json_t* get_system_status_json(const WebSocketMetrics *ws_metrics) {
    ...
    json_object_set_new(logging, "log_file", json_string(app_config->server.log_file));
