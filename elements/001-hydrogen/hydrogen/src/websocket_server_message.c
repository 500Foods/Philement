...
int ws_handle_receive(struct lws *wsi, WebSocketSessionData *session, void *in, size_t len)
{
    ...
    log_this("WebSocket", "Processing complete message", 0, true, true, true);
    ...
    if (json_is_string(type_json)) {
        ...
        log_this("WebSocket", "Processing message type: %s", 0, true, true, true, type);
...
static int handle_message_type(struct lws *wsi, const char *type)
{
    ...
    log_this("WebSocket", "Unknown message type: %s", 1, true, true, true, type);
    return -1;
}
...
int ws_write_json_response(struct lws *wsi, json_t *json)
{
    ...
    if (!response_str) {
        log_this("WebSocket", "Failed to serialize JSON response", 3, true, true, true);
        return -1;
    }
    ...
    if (buf) {
        ...
        log_this("WebSocket", "Failed to allocate response buffer", 3, true, true, true);
    }
