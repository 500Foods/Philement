...
{
    if (!ws_context) {
        log_this(SR_WEBSOCKET, "No server context available", LOG_LEVEL_ERROR);
        return;
    }

    log_this(SR_WEBSOCKET, "Preparing status response", LOG_LEVEL_STATE);
    ...
    if (pretty_str) {
        log_this(SR_WEBSOCKET, "Status response (pretty printed):", LOG_LEVEL_STATE);
        // Split into lines for better readability
        char *line_start = pretty_str;
        char *line_end;
        while ((line_end = strchr(line_start, '\n')) != NULL) {
            *line_end = '\0';
            log_this(SR_WEBSOCKET, "  %s", LOG_LEVEL_STATE, line_start);
    ...
    if (buf) {
        ...
        log_this(SR_WEBSOCKET, "Wrote %d bytes to WebSocket", LOG_LEVEL_STATE, written);
        free(buf);
        ...
        log_this(SR_WEBSOCKET, "Failed to allocate buffer for response", LOG_LEVEL_ERROR);
    }
