/*
 * Mail Relay Subsystem Launch Readiness Check
 * 
 * This module verifies that all prerequisites for the Mail relay subsystem
...
static const char* mail_relay_messages[15]; // Up to 15 messages plus NULL terminator
...
static void add_message(const char* format, ...) {
    ...
    if (message) {
        mail_relay_messages[message_count++] = message;
        mail_relay_messages[message_count] = NULL; // Ensure NULL termination
    }
}
...
LaunchReadiness check_mail_relay_launch_readiness(void) {
    ...
    for (int i = 0; i < 15; i++) {
        mail_relay_messages[i] = NULL;
    }
    
    // Add subsystem name as first message
    add_message("MailRelay");
    ...
    // Check 3: Mail Server
    add_message("  No-Go:   Mail Server (check connection)");
    ...
    add_message("  Decide:  No-Go For Launch of Mail Relay Subsystem");
    
    // Build the readiness structure
    LaunchReadiness readiness = {
        .subsystem = "MailRelay",
        .ready = overall_readiness,
        .messages = mail_relay_messages
    };
