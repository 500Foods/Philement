...
bool load_swagger_config(json_t* root, AppConfig* config) {
    ...
    if (!config->swagger) {
        ...
        if (!config->swagger) {
            log_this("Config", "Failed to allocate Swagger configuration", LOG_LEVEL_ERROR);
            return false;
        }
    ...
    if (success) {
        ...
        if (json_is_object(metadata)) {
            log_config_item("Metadata", "Configured", false);
            ...
            if (json_is_object(contact)) {
                log_config_item("Contact", "Configured", false);
            ...
            if (json_is_object(license)) {
                log_config_item("License", "Configured", false);
        ...
        if (json_is_object(ui_options)) {
            log_config_item("UIOptions", "Configured", false);
...
int config_swagger_validate(const SwaggerConfig* config) {
    if (!config) {
        log_this("Config", "Swagger config pointer is NULL", LOG_LEVEL_ERROR);
        return -1;
    }
    ...
    if (!config->prefix || strlen(config->prefix) < MIN_PREFIX_LENGTH ||
        ...
        log_this("Config", "Invalid Swagger prefix", LOG_LEVEL_ERROR);
        return -1;
    }
    ...
    if (!config->metadata.title || strlen(config->metadata.title) < MIN_TITLE_LENGTH ||
        ...
        log_this("Config", "Invalid Swagger title", LOG_LEVEL_ERROR);
        return -1;
    }
    ...
    if (!config->metadata.version || strlen(config->metadata.version) < MIN_VERSION_LENGTH ||
        ...
        log_this("Config", "Invalid Swagger version", LOG_LEVEL_ERROR);
        return -1;
    }

    if (config->metadata.description &&
        strlen(config->metadata.description) > MAX_DESCRIPTION_LENGTH) {
        log_this("Config", "Swagger description too long", LOG_LEVEL_ERROR);
        return -1;
    }
