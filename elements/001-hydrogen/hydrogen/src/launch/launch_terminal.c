...
int launch_terminal_subsystem(void) {
    ...
    log_this(SR_TERMINAL, "LAUNCH: " SR_TERMINAL, LOG_LEVEL_STATE, 0);

    // Step 1: Register with registry and add dependencies
    log_this(SR_TERMINAL, "  Step 1: Registering with registry", LOG_LEVEL_STATE, 0);
    ...
    if (terminal_subsystem_id < 0) {
        ...
        if (terminal_subsystem_id < 0) {
            log_this(SR_TERMINAL, "Failed to register Terminal subsystem", LOG_LEVEL_ERROR, 0);
            log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: Registration failed", LOG_LEVEL_STATE, 0);
            return 0;
        }
    }
    ...
    log_this(SR_TERMINAL, "    Registration complete", LOG_LEVEL_STATE, 0);

    // Step 2: Verify system state
    log_this(SR_TERMINAL, "  Step 2: Verifying system state", LOG_LEVEL_STATE, 0);

    if (server_stopping) {
        log_this(SR_TERMINAL, "    Cannot initialize Terminal during shutdown", LOG_LEVEL_STATE, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: System in shutdown", LOG_LEVEL_STATE, 0);
        return 0;
    }

    if (!server_starting) {
        log_this(SR_TERMINAL, "    Cannot initialize Terminal outside startup phase", LOG_LEVEL_STATE, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: Not in startup phase", LOG_LEVEL_STATE, 0);
        return 0;
    }

    if (!app_config) {
        log_this(SR_TERMINAL, "    Terminal configuration not loaded", LOG_LEVEL_STATE, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: No configuration", LOG_LEVEL_STATE, 0);
        return 0;
    }

    if (!app_config->terminal.enabled) {
        log_this(SR_TERMINAL, "    Terminal disabled in configuration", LOG_LEVEL_STATE, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Disabled by configuration", LOG_LEVEL_STATE, 0);
        return 1; // Not an error if disabled
    }
    log_this(SR_TERMINAL, "    System state verified", LOG_LEVEL_STATE, 0);

    // Step 3: Verify dependencies
    log_this(SR_TERMINAL, "  Step 3: Verifying dependencies", LOG_LEVEL_STATE, 0);

    // Check Registry first
    if (!is_subsystem_running_by_name("Registry")) {
        log_this(SR_TERMINAL, "    Registry not running", LOG_LEVEL_ERROR, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: Registry dependency not met", LOG_LEVEL_STATE, 0);
        return 0;
    }
    log_this(SR_TERMINAL, "    Registry dependency verified", LOG_LEVEL_STATE, 0);

    // Check WebServer subsystem
    if (!is_subsystem_running_by_name(SR_WEBSERVER)) {
        log_this(SR_TERMINAL, "    " SR_WEBSERVER " subsystem not running", LOG_LEVEL_ERROR, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: " SR_WEBSERVER " dependency not met", LOG_LEVEL_STATE, 0);
        return 0;
    }
    log_this(SR_TERMINAL, "    " SR_WEBSERVER " subsystem verified", LOG_LEVEL_STATE, 0);

    // Check WebSocket subsystem
    if (!is_subsystem_running_by_name(SR_WEBSOCKET)) {
        log_this(SR_TERMINAL, "    " SR_WEBSOCKET " subsystem not running", LOG_LEVEL_ERROR, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: " SR_WEBSOCKET " dependency not met", LOG_LEVEL_STATE, 0);
        return 0;
    }
    log_this(SR_TERMINAL, "    " SR_WEBSOCKET " subsystem verified", LOG_LEVEL_STATE, 0);
    ...
    if (payload_id < 0 || get_subsystem_state(payload_id) != SUBSYSTEM_RUNNING) {
        log_this(SR_TERMINAL, "    " SR_PAYLOAD " subsystem not running", LOG_LEVEL_ERROR, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: " SR_PAYLOAD " subsystem dependency not met", LOG_LEVEL_STATE, 0);
        return 0;
    }

    // Check for terminal files in payload cache
    if (!is_payload_cache_available()) {
        log_this(SR_TERMINAL, "    Payload cache not available", LOG_LEVEL_STATE, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: Payload cache not available", LOG_LEVEL_STATE, 0);
        return 0;
    }
    ...
    if (!files_retrieved || !terminal_files || num_terminal_files == 0) {
        log_this(SR_TERMINAL, "    No terminal files found in payload cache", LOG_LEVEL_STATE, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: Missing Terminal UI files", LOG_LEVEL_STATE, 0);
        return 0;
    }

    log_this(SR_TERMINAL, "    Terminal files verified (%zu files in cache):", LOG_LEVEL_STATE, 1, num_terminal_files);
    for (size_t i = 0; i < num_terminal_files; i++) {
        log_this(SR_TERMINAL, "      -> %s (%s)", LOG_LEVEL_STATE, 2,
    ...
    if (!init_terminal_support(mutable_terminal_config)) {
        log_this(SR_TERMINAL, "    Failed to load terminal files into memory", LOG_LEVEL_ERROR, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: File loading failed", LOG_LEVEL_STATE, 0);
        free(terminal_files);
        return 0;
    }
    ...
    free(terminal_files);
    log_this(SR_TERMINAL, "    All dependencies verified", LOG_LEVEL_STATE, 0);
    log_this(SR_TERMINAL, "    " SR_TERMINAL " subsystem initialized", LOG_LEVEL_STATE, 0);
    ...
    if (!register_web_endpoint(&terminal_endpoint)) {
        log_this(SR_TERMINAL, "    Failed to register endpoint", LOG_LEVEL_ERROR, 0);
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL - Failed: Endpoint registration failed", LOG_LEVEL_STATE, 0);
        return 0;
    }

    // Log configuration
    log_this(SR_TERMINAL, "    Configuration:", LOG_LEVEL_STATE, 0);
    log_this(SR_TERMINAL, "      -> Enabled: yes", LOG_LEVEL_STATE, 0);
    log_this(SR_TERMINAL, "      -> Web Path: %s", LOG_LEVEL_STATE, 1, app_config->terminal.web_path);
    log_this(SR_TERMINAL, "      -> WebRoot: %s", LOG_LEVEL_STATE, 1, app_config->terminal.webroot);
    log_this(SR_TERMINAL, "      -> Shell: %s", LOG_LEVEL_STATE, 1, app_config->terminal.shell_command);
    log_this(SR_TERMINAL, "      -> Max Sessions: %d", LOG_LEVEL_STATE, 1, app_config->terminal.max_sessions);
    log_this(SR_TERMINAL, "      -> Payload: available", LOG_LEVEL_STATE, 0);
    log_this(SR_TERMINAL, "    " SR_TERMINAL " subsystem initialized", LOG_LEVEL_STATE, 0);

    // Step 5: Update registry and verify state
    log_this(SR_TERMINAL, "  Updating " SR_REGISTRY, LOG_LEVEL_STATE, 0);
    ...
    if (final_state == SUBSYSTEM_RUNNING) {
        log_this(SR_TERMINAL, "LAUNCH: TERMINAL Success: Launched and running", LOG_LEVEL_STATE, 0);
        return 1;
    } else {
