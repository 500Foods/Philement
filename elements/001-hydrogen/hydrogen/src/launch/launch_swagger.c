...
int launch_swagger_subsystem(void) {
    ...
    log_this(SR_SWAGGER, "    Registration complete", LOG_LEVEL_STATE);

    // Step 2: Verify system state
    log_this(SR_SWAGGER, "  Step 2: Verifying system state", LOG_LEVEL_STATE);
    
    if (server_stopping) {
        log_this(SR_SWAGGER, "    Cannot initialize Swagger during shutdown", LOG_LEVEL_STATE);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: System in shutdown", LOG_LEVEL_STATE);
        return 0;
    }

    if (!server_starting) {
        log_this(SR_SWAGGER, "    Cannot initialize Swagger outside startup phase", LOG_LEVEL_STATE);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: Not in startup phase", LOG_LEVEL_STATE);
        return 0;
    }

    if (!app_config) {
        log_this(SR_SWAGGER, "    Swagger configuration not loaded", LOG_LEVEL_STATE);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: No configuration", LOG_LEVEL_STATE);
        return 0;
    }

    if (!app_config->swagger.enabled) {
        log_this(SR_SWAGGER, "    Swagger disabled in configuration", LOG_LEVEL_STATE);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Disabled by configuration", LOG_LEVEL_STATE);
        return 1; // Not an error if disabled
    }
    log_this(SR_SWAGGER, "    System state verified", LOG_LEVEL_STATE);

    // Step 3: Verify dependencies
    log_this(SR_SWAGGER, "  Step 3: Verifying dependencies", LOG_LEVEL_STATE);

    // Check Registry first
    if (!is_subsystem_running_by_name("Registry")) {
        log_this(SR_SWAGGER, "    Registry not running", LOG_LEVEL_ERROR);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: Registry dependency not met", LOG_LEVEL_STATE);
        return 0;
    }
    ...
    log_this(SR_SWAGGER, "      -> Enabled: yes", LOG_LEVEL_STATE);
    log_this(SR_SWAGGER, "      -> Prefix: %s", LOG_LEVEL_STATE, app_config->swagger.prefix);
