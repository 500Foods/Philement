...
int launch_swagger_subsystem(void) {
    ...
    log_this(SR_SWAGGER, "LAUNCH: " SR_SWAGGER, LOG_LEVEL_STATE, 0);

    // Step 1: Register with registry and add dependencies
    log_this(SR_SWAGGER, "  Step 1: Registering with registry", LOG_LEVEL_STATE, 0);
    
    if (swagger_subsystem_id < 0) {
        log_this(SR_SWAGGER, "    Failed to register Swagger subsystem", LOG_LEVEL_ERROR, 0);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: Registration failed", LOG_LEVEL_STATE, 0);
        return 0;
    }
    ...
    log_this(SR_SWAGGER, "    Registration complete", LOG_LEVEL_STATE, 0);

    // Step 2: Verify system state
    log_this(SR_SWAGGER, "  Step 2: Verifying system state", LOG_LEVEL_STATE, 0);
    
    if (server_stopping) {
        log_this(SR_SWAGGER, "    Cannot initialize Swagger during shutdown", LOG_LEVEL_STATE, 0);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: System in shutdown", LOG_LEVEL_STATE, 0);
        return 0;
    }

    if (!server_starting) {
        log_this(SR_SWAGGER, "    Cannot initialize Swagger outside startup phase", LOG_LEVEL_STATE, 0);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: Not in startup phase", LOG_LEVEL_STATE, 0);
        return 0;
    }

    if (!app_config) {
        log_this(SR_SWAGGER, "    Swagger configuration not loaded", LOG_LEVEL_STATE, 0);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: No configuration", LOG_LEVEL_STATE, 0);
        return 0;
    }

    if (!app_config->swagger.enabled) {
        log_this(SR_SWAGGER, "    Swagger disabled in configuration", LOG_LEVEL_STATE, 0);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Disabled by configuration", LOG_LEVEL_STATE, 0);
        return 1; // Not an error if disabled
    }
    log_this(SR_SWAGGER, "    System state verified", LOG_LEVEL_STATE, 0);

    // Step 3: Verify dependencies
    log_this(SR_SWAGGER, "  Step 3: Verifying dependencies", LOG_LEVEL_STATE, 0);

    // Check Registry first
    if (!is_subsystem_running_by_name("Registry")) {
        log_this(SR_SWAGGER, "    Registry not running", LOG_LEVEL_ERROR, 0);
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: Registry dependency not met", LOG_LEVEL_STATE, 0);
        return 0;
    }
    log_this(SR_SWAGGER, "    Registry dependency verified", LOG_LEVEL_STATE, 0);
    ...
    if (!is_subsystem_running_by_name(SR_API)) {
        ...
        log_this(SR_SWAGGER, "LAUNCH: SWAGGER - Failed: " SR_API " subsystem dependency not met", LOG_LEVEL_STATE, 0);
        return 0;
    }
    log_this(SR_SWAGGER, "    " SR_API " subsystem dependency verified", LOG_LEVEL_STATE, 0);

    // Check webserver
    if (!is_subsystem_running_by_name(SR_WEBSERVER)) {
        log_this(SR_SWAGGER, "    " SR_WEBSERVER " subsystem not running", LOG_LEVEL_ERROR, 0);
        log_this(SR_SWAGGER, "LAUNCH: " SR_SWAGGER" Failed: " SR_WEBSERVER " subsystem dependency not met", LOG_LEVEL_STATE, 0);
        return 0;
    }
    log_this(SR_SWAGGER, "    " SR_WEBSERVER " subsystem verified", LOG_LEVEL_STATE, 0);
    ...
    if (payload_id < 0 || get_subsystem_state(payload_id) != SUBSYSTEM_RUNNING) {
        ...
        log_this(SR_SWAGGER, "LAUNCH: " SR_PAYLOAD " Failed: " SR_PAYLOAD "subsystem dependency not met", LOG_LEVEL_STATE, 0);
        return 0;
    }
    log_this(SR_SWAGGER, "    " SR_PAYLOAD " subsystem verified", LOG_LEVEL_STATE, 0);

    // Check for swagger files in payload cache
    if (!is_payload_cache_available()) {
        log_this(SR_SWAGGER, "    Payload cache not available", LOG_LEVEL_STATE, 0);
        log_this(SR_SWAGGER, "LAUNCH: " SR_SWAGGER " Failed: Payload cache not available", LOG_LEVEL_STATE, 0);
        return 0;
    }
    ...
    if (!register_web_endpoint(&swagger_endpoint)) {
        ...
        log_this(SR_SWAGGER, "LAUNCH: " SR_SWAGGER " Failed: Endpoint registration failed", LOG_LEVEL_STATE, 0);
        return 0;
    }
    
    // Log configuration
    log_this(SR_SWAGGER, "    Configuration:", LOG_LEVEL_STATE, 0);
    log_this(SR_SWAGGER, "      -> Enabled: yes", LOG_LEVEL_STATE, 0);
    log_this(SR_SWAGGER, "      -> Prefix: %s", LOG_LEVEL_STATE, 1, app_config->swagger.prefix);
    log_this(SR_SWAGGER, "      -> Title: %s", LOG_LEVEL_STATE, 1, app_config->swagger.metadata.title);
    log_this(SR_SWAGGER, "      -> Version: %s", LOG_LEVEL_STATE, 1, app_config->swagger.metadata.version);
    log_this(SR_SWAGGER, "      -> Payload: available", LOG_LEVEL_STATE, 0);
    log_this(SR_SWAGGER, "    " SR_SWAGGER " subsystem initialized", LOG_LEVEL_STATE, 0);

    // Step 4: Update registry and verify state
    log_this(SR_SWAGGER, "  Updating " SR_REGISTRY, LOG_LEVEL_STATE, 0);
    ...
    if (final_state == SUBSYSTEM_RUNNING) {
        log_this(SR_SWAGGER, "LAUNCH: " SR_SWAGGER " Success: Launched and running", LOG_LEVEL_STATE, 0);
    } else {
        log_this(SR_SWAGGER, "LAUNCH: " SR_SWAGGER " Warning: Unexpected final state: %s", LOG_LEVEL_ALERT, 1, subsystem_state_to_string(final_state));
        return 0;
    }
