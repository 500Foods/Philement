...
static void _mdns_server_build_interface_announcement(uint8_t *packet, size_t *packet_len, const char *hostname, 
    ...
    for (size_t i = 0; i < mdns_server_instance->num_services; i++) {
        ...
        const char *local_suffix = strstr(mdns_server_instance->services[i].type, ".local");
        size_t type_len;
        if (local_suffix) {
            ptrdiff_t diff = local_suffix - mdns_server_instance->services[i].type;
            if (diff > 0) {
            ...
            type_len = strlen(mdns_server_instance->services[i].type);
        }
        strncpy(service_type, mdns_server_instance->services[i].type, type_len);
        ...
        if (total_len >= sizeof(full_service_name)) {
            log_this("mDNSServer", "Service name too long: %s.%s.local truncated", LOG_LEVEL_ALERT,
                     mdns_server_instance->services[i].name, service_type);
        ...
        snprintf(full_service_name, sizeof(full_service_name), "%.*s.%.*s.local", 
                 (int)name_len, mdns_server_instance->services[i].name, (int)stype_len, service_type);

        ptr = write_dns_ptr_record(ptr, mdns_server_instance->services[i].type, full_service_name, ttl);
        header->ancount = htons(ntohs(header->ancount) + 1);
        ptr = write_dns_srv_record(ptr, full_service_name, 0, 0, (uint16_t)mdns_server_instance->services[i].port, hostname, ttl);
        header->ancount = htons(ntohs(header->ancount) + 1);
        ptr = write_dns_txt_record(ptr, full_service_name, mdns_server_instance->services[i].txt_records, mdns_server_instance->services[i].num_txt_records, ttl);
        header->ancount = htons(ntohs(header->ancount) + 1);
    }
...
static void close_mdns_server_interfaces(mdns_server_t *mdns_server_instance) {
...
void mdns_server_shutdown(mdns_server_t *mdns_server_instance) {
    ...
    if (net_info_instance && net_info_instance->primary_index != -1) {
        ...
        for (size_t iface_idx = 0; iface_idx < mdns_server_instance->num_interfaces; iface_idx++) {
            ...
            mdns_server_build_announcement(packet, &packet_len, mdns_server_instance->hostname, mdns_server_instance, 0, iface_net_info);
            free_single_interface_net_info(iface_net_info);
