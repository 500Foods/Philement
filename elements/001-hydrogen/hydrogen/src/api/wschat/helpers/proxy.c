...
// Global multi-stream manager instance (singleton)
static MultiStreamManager g_multi_manager = {0};

// Streaming thread context - passed to worker thread
typedef struct {
...
typedef struct {
    ...
    size_t bytes_received;       // Track total bytes received
    size_t chunks_processed;     // Track chunks processed
    bool first_data_logged;      // Log first data arrival only once
} StreamContext;
...
static size_t stream_write_callback(const void* contents, size_t size, size_t nmemb, void* userp) {
    ...
    ctx->bytes_received += realsize;
    
    // Log first data arrival (TTFB diagnostic)
    if (!ctx->first_data_logged && realsize > 0) {
        ctx->first_data_logged = true;
        log_this(SR_CHAT, "First response data received: %zu bytes (TTFB)", LOG_LEVEL_DEBUG, 1, realsize);
    }
...
static int stream_debug_callback(CURL* handle, curl_infotype type, char* data, size_t size, void* userptr) {
    ...
    // Only log connection and header info, not data
    if (type == CURLINFO_TEXT) {
        // Connection phase messages - log at debug level
        if (size > 0) {
            ...
            // Only log interesting connection events
            if (strstr(debug_buf, "Connected to") || 
                strstr(debug_buf, "Trying ") ||
                strstr(debug_buf, "TLS") ||
                strstr(debug_buf, "SSL") ||
                strstr(debug_buf, "HTTP")) {
                log_this(SR_CHAT, "CURL: %s", LOG_LEVEL_DEBUG, 1, debug_buf);
            }
        }
    } else if (type == CURLINFO_HEADER_OUT) {
        // Outgoing headers - log at startup
        log_this(SR_CHAT, "CURL sending request headers", LOG_LEVEL_DEBUG, 0);
    }
    // Don't log data transfers - too verbose
...
static void* stream_worker_thread(void* arg) {
    ...
    StreamContext ctx = {0};
    ctx.chunk_callback = thread_ctx->chunk_callback;
    ctx.user_data = thread_ctx->user_data;
    ctx.line_buffer_capacity = 4096;
    ctx.bytes_received = 0;
    ctx.chunks_processed = 0;
    ctx.first_data_logged = false;
    ctx.line_buffer = malloc(ctx.line_buffer_capacity);
    if (!ctx.line_buffer) {
        thread_ctx->success = false;
        thread_ctx->completed = true;
        return NULL;
    }
    ctx.line_buffer[0] = '\0';
    ...
    const ChatEngineConfig* engine = thread_ctx->engine;
    const ChatProxyConfig* config = &thread_ctx->config;
    ...
    // Flush any remaining data in line buffer (in case API didn't send trailing newline)
    if (ctx.line_buffer_len > 0 && curl_result == CURLE_OK) {
        ChatStreamChunk* chunk = chat_stream_chunk_parse(ctx.line_buffer);
        if (chunk) {
            ctx.chunk_callback(chunk, ctx.user_data);
            chat_stream_chunk_destroy(chunk);
    ...
    if (curl_result == CURLE_OK) {
        log_this(SR_CHAT, "Stream completed: %zu bytes, %zu chunks", LOG_LEVEL_DEBUG, 2, 
                 ctx.bytes_received, ctx.chunks_processed);
    } else {
        log_this(SR_CHAT, "Streaming CURL error: %s (%zu bytes received)", LOG_LEVEL_ERROR, 2, 
                 curl_easy_strerror(curl_result), ctx.bytes_received);
    }
    ...
    thread_ctx->success = (curl_result == CURLE_OK);
    thread_ctx->completed = true;
 ...
 * 
 * We use DETACHED worker threads for streaming to avoid blocking LWS.
 * The thread writes chunks directly via callback, which is technically
 * not thread-safe with LWS (lws_write should be called from service thread).
 * 
 * This works in practice for moderate loads. For high concurrency or 
 * guaranteed thread-safety, migrate to libcurl multi interface integrated
 * with LWS event loop (see libcurl multi-uv.c example).
 * 
 * Current: Detached thread per stream request
 * Future: curl_multi_socket_action() + LWS poll integration
 */

// Stream request tracking for async completion
typedef struct StreamRequest {
    pthread_t worker_tid;
    StreamThreadContext thread_ctx;
    volatile bool request_active;
    struct StreamRequest* next;
} StreamRequest;

// Active stream requests list (for cleanup)
static StreamRequest* active_streams = NULL;
...
static void cleanup_completed_streams(void) {
    ...
    while (*pp) {
        StreamRequest* req = *pp;
        if (req->thread_ctx.completed) {
            pthread_join(req->worker_tid, NULL);
            free(req->thread_ctx.request_json);
            *pp = req->next;
            free(req);
        } else {
            pp = &(*pp)->next;
    ...
    pthread_mutex_unlock(&active_streams_mutex);
}
...
bool chat_proxy_send_stream(const ChatEngineConfig* engine,
    ...
    req->thread_ctx.engine = engine;
    req->thread_ctx.request_json = strdup(request_json);
    req->thread_ctx.config = *config;
    req->thread_ctx.chunk_callback = chunk_callback;
    req->thread_ctx.user_data = user_data;
    req->thread_ctx.completed = false;
    req->thread_ctx.success = false;
    req->request_active = true;
    
    if (!req->thread_ctx.request_json) {
        free(req);
        return false;
    }
    
    // Spawn worker thread as DETACHED - does NOT block caller
    ...
    if (create_result != 0) {
        log_this(SR_CHAT, "Failed to create streaming worker thread: %d", LOG_LEVEL_ERROR, 1, create_result);
        free(req->thread_ctx.request_json);
        free(req);
        return false;
    }
    
    // Add to active streams list for tracking
    pthread_mutex_lock(&active_streams_mutex);
    req->next = active_streams;
    active_streams = req;
    pthread_mutex_unlock(&active_streams_mutex);
    
    // Return immediately - streaming is now async
    return true;
}

// ============================================================================
// Multi-Stream Manager Access
// ============================================================================
