...
//@ swagger:description Executes a pre-defined query from the Query Table Cache using a query reference ID with JWT authentication. The database name is extracted from the JWT token claims, ensuring secure routing. Supports typed parameters (INTEGER, STRING, BOOLEAN, FLOAT). Returns query results in JSON format with execution metadata. Accepts both GET with query parameters and POST with JSON body.
