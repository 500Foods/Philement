 ...
 *
 * This module provides authenticated database queries execution in parallel.
...
// Include queries.h for DeduplicationResult enum
 ...
 * JWT token in Authorization header. The database name is extracted from the 
 * JWT token claims (not from request parameters), ensuring authenticated database routing.
 *
 ...
 *
 * The database field is extracted from the authenticated JWT token via the 
 * Authorization header, ensuring the user can only access databases they're
 * authorized for.
 *
...
//@ swagger:response 400 application/json {"type":"object","properties":{"success":{"type":"boolean","example":false},"error":{"type":"string","description":"Error message","example":"Missing required parameter: queries"}}}
