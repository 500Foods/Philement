...
static json_t* execute_single_query(const char *database, json_t *query_obj)
{
    if (!database || !query_obj) {
        log_this(SR_AUTH, "execute_single_query: NULL parameters", LOG_LEVEL_ERROR, 0);
    ...
    if (!query_ref_json || !json_is_integer(query_ref_json)) {
        log_this(SR_AUTH, "execute_single_query: Missing or invalid query_ref", LOG_LEVEL_ERROR, 0);
        json_t *error_result = json_object();
        json_object_set_new(error_result, "success", json_false());
    ...
    log_this(SR_AUTH, "execute_single_query: Query completed, query_ref=%d", 
    ...
    void **con_cls)
{
    ...
    log_this(SR_AUTH, "handle_conduit_auth_queries_request: Executing %zu queries", LOG_LEVEL_DEBUG, 1, query_count);
