/**
 * @file alt_chat.c
 * @brief Alternative Authenticated Chat API Endpoint Implementation
 *
 * Handles single-model chat requests with JWT authentication and database override.
 * Validates JWT from Authorization header but extracts database from request body,
 * enabling cross-database chat operations for management scenarios.
 *
...
/**
 * @brief Handle POST /api/conduit/alt_chat request
 *
 * Alternative authenticated chat endpoint that allows specifying a different
 * database than the one in the JWT claims. Useful for management database
 * patterns where authentication is centralized but chat operations target
 * client databases.
 */
enum MHD_Result handle_alt_chat_request(struct MHD_Connection *connection,
    ...
    if (strcmp(method, "POST") != 0) {
        ...
        json_t *error = alt_chat_build_error_response("Method not allowed - use POST");
    ...
    if (!extract_and_validate_jwt(auth_header, &jwt_result)) {
        ...
        json_t *error = alt_chat_build_error_response(error_msg);
    ...
    if (!request_json) {
        free_jwt_validation_result(&jwt_result);
        json_t *error_response = alt_chat_build_error_response("Invalid JSON in request body");
        enum MHD_Result ret = api_send_json_response(connection, error_response, MHD_HTTP_BAD_REQUEST);
        json_decref(error_response);
        return ret;
    }

    // Parse request parameters including database from request body
    char *database = NULL;
    ...
    bool parse_ok = alt_chat_parse_request(request_json, &database, &engine_name,
    ...
    if (!parse_ok) {
        ...
        json_t *error_response = alt_chat_build_error_response(error_message ? error_message : "Invalid request");
        free(error_message);
        enum MHD_Result ret = api_send_json_response(connection, error_response, MHD_HTTP_BAD_REQUEST);
        json_decref(error_response);
        return ret;
    ...
    if (stream) {
        ...
        free_jwt_validation_result(&jwt_result);
        json_decref(request_json);
        json_t *error_response = alt_chat_build_error_response("Streaming not yet implemented");
    ...
    if (!db_queue) {
        ...
        free_jwt_validation_result(&jwt_result);
        json_decref(request_json);
        json_t *error_response = alt_chat_build_error_response("Database not found");
        enum MHD_Result ret = api_send_json_response(connection, error_response, MHD_HTTP_BAD_REQUEST);
        json_decref(error_response);
        return ret;
    }
    ...
    if (!cec) {
        ...
        free_jwt_validation_result(&jwt_result);
        json_decref(request_json);
        json_t *error_response = alt_chat_build_error_response("Chat not enabled for this database");
    ...
    if (!engine) {
        ...
        free_jwt_validation_result(&jwt_result);
        json_decref(request_json);
        json_t *error_response = alt_chat_build_error_response(engine_name ? "Engine not found" : "No default engine configured");
        enum MHD_Result ret = api_send_json_response(connection, error_response, MHD_HTTP_BAD_REQUEST);
        json_decref(error_response);
        return ret;
    }
    ...
    if (!engine->is_healthy) {
        ...
        free_jwt_validation_result(&jwt_result);
        json_decref(request_json);
        json_t *error_response = alt_chat_build_error_response("Engine is currently unavailable");
        enum MHD_Result ret = api_send_json_response(connection, error_response, MHD_HTTP_SERVICE_UNAVAILABLE);
        json_decref(error_response);
        return ret;
    }
    ...
    if (!request_body) {
        ...
        free_jwt_validation_result(&jwt_result);
        json_decref(request_json);
        json_t *error_response = alt_chat_build_error_response("Failed to build request");
    ...
    if (!request_success) {
        ...
        json_t *error_response = alt_chat_build_error_response(error_msg);
    ...
    json_t *response = alt_chat_build_response(
        database,
        engine->name,
...
/**
 * @brief Parse and validate alt_chat request JSON
 *
 * Similar to auth_chat_parse_request but also extracts database from request body.
 *
 * @param request_json The parsed JSON request body
 * @param database Output parameter for database name (required, allocated)
 * @param engine Output parameter for engine name (optional, allocated if present)
 * @param messages Output parameter for messages array (not owned, don't free)
 * @param temperature Output parameter for temperature
 * @param max_tokens Output parameter for max_tokens
 * @param stream Output parameter for stream flag
 * @param error_message Output parameter for error message (allocated on failure)
 * @return true if valid, false if error (error_message will be set)
 */
bool alt_chat_parse_request(json_t *request_json,
    ...
    *error_message = NULL;
    ...
    // Extract database (required for alt_chat)
    ...
    if (engine_obj && json_is_string(engine_obj)) {
        *engine = strdup(json_string_value(engine_obj));
        if (!*engine) {
            free(*database);
            *database = NULL;
            *error_message = strdup("Failed to allocate engine string");
            return false;
        }
    }
...
/**
 * @brief Build alt_chat success response
 */
...
/**
 * @brief Build alt_chat error response
 */
