...
static const char* get_cache_base_dir(void) {
    ...
    return env_dir ? env_dir : CHAT_LRU_CACHE_DIR_NAME;
}
...
static size_t hash_string(const char* str) {
    ...
    return hash % CHAT_LRU_CACHE_HASH_TABLE_SIZE;
}
...
static char* get_segment_path(const char* cache_dir, const char* hash) {
    if (!cache_dir || !hash || strlen(hash) < CHAT_LRU_CACHE_PREFIX_LEN) {
        return NULL;
    }

    /* Use first 2 chars as subdirectory for distribution */
    char prefix[CHAT_LRU_CACHE_PREFIX_LEN + 1];
    memcpy(prefix, hash, CHAT_LRU_CACHE_PREFIX_LEN);
    prefix[CHAT_LRU_CACHE_PREFIX_LEN] = '\0';
...
static char* get_metadata_path(const char* cache_dir) {
    ...
    size_t path_len = strlen(cache_dir) + strlen("/") + strlen(CHAT_LRU_CACHE_METADATA_FILE) + 2;
    char* path = calloc(path_len, 1);
    if (!path) {
        return NULL;
    }

    snprintf(path, path_len, "%s/%s", cache_dir, CHAT_LRU_CACHE_METADATA_FILE);
    return path;
}
...
static void* sync_thread_func(void* arg) {
    ...
    while (cache->sync_running) {
        ...
        ts.tv_sec += CHAT_LRU_CACHE_SYNC_INTERVAL_SEC;
        ...
        if (cache->sync_running) {
            ...
            wake_time.tv_sec += CHAT_LRU_CACHE_SYNC_INTERVAL_SEC;

            /* pthread_cond_timedwait would be better but keeping it simple */
            pthread_mutex_unlock(&cache->cache_mutex);

            /* Sleep in small intervals to allow quick shutdown */
            for (int i = 0; i < CHAT_LRU_CACHE_SYNC_INTERVAL_SEC && cache->sync_running; i++) {
                sleep(1);
            }
        } else {
            pthread_mutex_unlock(&cache->cache_mutex);
...
ChatLRUCache* chat_lru_cache_init(const char* database, size_t max_size_bytes) {
    ...
    cache->max_size_bytes = max_size_bytes > 0 ? max_size_bytes : CHAT_LRU_CACHE_MAX_SIZE_BYTES;
    cache->current_size_bytes = 0;

    /* Allocate hash table */
    cache->hash_table = calloc(CHAT_LRU_CACHE_HASH_TABLE_SIZE, sizeof(ChatLRUCacheEntry*));
    if (!cache->hash_table) {
        free(cache->database);
        free(cache);
        return NULL;
...
bool chat_lru_cache_put(ChatLRUCache* cache, const char* segment_hash,
    ...
    strncpy(entry->hash, segment_hash, CHAT_LRU_CACHE_MAX_HASH_LEN - 1);
    entry->content = strndup(content, content_size);
    entry->content_size = content_size;
    entry->last_accessed = time(NULL);
...
bool chat_lru_cache_clear(ChatLRUCache* cache) {
    ...
    /* Reset hash table */
    memset(cache->hash_table, 0, CHAT_LRU_CACHE_HASH_TABLE_SIZE * sizeof(ChatLRUCacheEntry*));
