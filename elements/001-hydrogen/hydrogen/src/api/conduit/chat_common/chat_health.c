...
static bool chat_health_check_openai(const ChatEngineConfig* engine) {
    ...
    config.request_timeout_seconds = CHAT_HEALTH_TIMEOUT_SECONDS;
...
const char* chat_health_status_to_string(ChatHealthStatus status) {
    switch (status) {
        case CHAT_HEALTH_HEALTHY:     return "healthy";
        case CHAT_HEALTH_DEGRADED:    return "degraded";
        case CHAT_HEALTH_UNAVAILABLE: return "unavailable";
        case CHAT_HEALTH_UNKNOWN:
...
ChatHealthStatus chat_health_get_engine_status(ChatEngineConfig* engine) {
    if (!engine) return CHAT_HEALTH_UNKNOWN;
    ...
    if (!is_healthy) {
        return CHAT_HEALTH_UNAVAILABLE;
    }
    if (failures > 0) {
        return CHAT_HEALTH_DEGRADED;
    }
    return CHAT_HEALTH_HEALTHY;
}
...
void chat_health_update_stats(ChatEngineConfig* engine, bool success, double response_time_ms, int tokens_used) {
    ...
    if (success) {
        ...
        if (engine->consecutive_failures >= CHAT_HEALTH_MAX_FAILURES) {
