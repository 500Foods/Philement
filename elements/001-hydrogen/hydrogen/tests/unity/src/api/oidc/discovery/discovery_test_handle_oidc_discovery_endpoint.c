/*
 * Unity Test File: handle_oidc_discovery_endpoint
 */

#include <src/hydrogen.h>
#include <unity.h>

#include <src/api/oidc/discovery/discovery.h>
...
void test_discovery_handler_without_service(void);
void test_discovery_handler_ok(void);

static struct MHD_Connection *const FAKE = (struct MHD_Connection *)0xD15C;
...
void test_discovery_handler_without_service(void) {
    /* oidc_generate_discovery_document fails → 500 path */
    TEST_ASSERT_EQUAL_INT(MHD_YES, handle_oidc_discovery_endpoint(FAKE));
}

void test_discovery_handler_ok(void) {
    char tmpl[] = "/tmp/oidc_disc_h_XXXXXX";
...
int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_discovery_handler_without_service);
    RUN_TEST(test_discovery_handler_ok);
    return UNITY_END();
}
