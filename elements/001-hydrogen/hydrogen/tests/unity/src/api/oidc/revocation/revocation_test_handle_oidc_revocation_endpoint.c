/*
 * Unity Test File: handle_oidc_revocation_endpoint
 */
...
void test_revoke_method_not_allowed(void);
void test_revoke_missing_token(void);
void test_revoke_no_client_credentials(void);
void test_revoke_failure_no_oidc(void);
...
static enum MHD_Result drive_post(const char *body) {
    ...
    ret = handle_oidc_revocation_endpoint(FAKE, "POST", NULL, &zero, &con_cls);
    if (ret != MHD_YES || !con_cls) {
        return ret;
    }
    if (len > 0) {
        ret = handle_oidc_revocation_endpoint(FAKE, "POST", body, &len, &con_cls);
        if (ret != MHD_YES) {
            return ret;
        }
    }
    zero = 0;
    return handle_oidc_revocation_endpoint(FAKE, "POST", NULL, &zero, &con_cls);
}

static bool init_oidc_with_client(void) {
    char tmpl[] = "/tmp/oidc_rev_ep_XXXXXX";
...
void test_revoke_method_not_allowed(void) {
    enum MHD_Result ret = handle_oidc_revocation_endpoint(FAKE, "GET", NULL, NULL, NULL);
...
int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_revoke_method_not_allowed);
    RUN_TEST(test_revoke_missing_token);
    RUN_TEST(test_revoke_no_client_credentials);
    RUN_TEST(test_revoke_failure_no_oidc);
