...
static void assert_jwt_shape(const char *jwt) {
    ...
    TEST_ASSERT_NULL(strchr(dot2 + 1, '.'));
    TEST_ASSERT_TRUE(strlen(jwt) > 20);
}
...
// Test with empty secret (HMAC still produces a token)
void test_api_create_jwt_empty_secret(void) {
    json_t *claims = json_object();
    json_object_set_new(claims, "sub", json_string("test_user"));
...
// Different claims/secrets produce different tokens
void test_api_create_jwt_consistency(void) {
    ...
    char *result1b = api_create_jwt(claims1, "secret1");

    assert_jwt_shape(result1);
    assert_jwt_shape(result2);
    TEST_ASSERT_EQUAL_STRING(result1, result1b);
