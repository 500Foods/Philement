/*
 * Unity Test File: OIDC RP account linker — Phase 19 (match_email_only).
 *
 * Maps to `src/api/auth/oidc_rp/oidc_rp_link_email.c`. Covers the
 * `match_email_only` strategy via `oidc_rp_link_resolve` with
 * `Strategy = "match_email_only"`. The Phase 18 (match_sub_only),
 ...
 *
 * Coverage — `match_email_only` (oidc_rp_link_email.c):
 *   Happy paths:
