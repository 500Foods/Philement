/*
 * Unity Test File: MCP Status API Endpoint
 *
 * Tests GET /api/mcp/status handler covering method validation, JWT
...
#include <src/api/mcp/status/status.h>
...
void test_status_wrong_method(void) {
    ...
    enum MHD_Result result = handle_mcp_status_request(conn, "/api/mcp/status",
                                                       "POST", NULL, NULL, NULL);
