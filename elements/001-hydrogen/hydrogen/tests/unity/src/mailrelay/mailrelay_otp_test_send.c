/*
 * Unity Test File: mailrelay_otp_test_send.c
 *
...
void test_otp_send_null_args(void);
void test_otp_send_req_overrides_config(void);
void test_otp_send_uses_defaults(void);
void test_otp_send_invalid_digits(void);
void test_otp_send_generation_failure(void);
void test_otp_send_insert_returns_false(void);
void test_otp_send_template_failure_empty_err(void);
void test_otp_send_response_helpers_null(void);
...
static bool g_insert_return_false = false;
static bool g_send_fail_empty_err = false;
static bool g_random_should_fail = false;
static unsigned char g_fake_fill = 7;

static bool fake_random_fill(unsigned char* buffer, size_t length) {
    if (g_random_should_fail) {
        return false;
    }
...
static bool mock_executor(int query_ref,
    ...
    if (query_ref == MAILRELAY_QREF_OTP_INSERT) {
        ...
        if (g_insert_return_false) {
            /* Simulate the executor failing before invoking the callback so the
             * caller's insert context is never populated (empty-error branch). */
            return false;
        }
...
static MailRelayStatus mock_send_template(const MailRelaySendTemplateRequest* req,
    ...
    if (g_send_fail_empty_err) {
        /* Fail without populating err so the caller supplies its own message. */
        return MAILRELAY_INVALID_ARGS;
    }
...
void setUp(void) {
    ...
    g_insert_return_false = false;
    g_send_fail_empty_err = false;
    g_random_should_fail = false;
    g_fake_fill = 7;
...
int main(void) {
    ...
    RUN_TEST(test_otp_send_null_args);
    RUN_TEST(test_otp_send_req_overrides_config);
    RUN_TEST(test_otp_send_uses_defaults);
    RUN_TEST(test_otp_send_invalid_digits);
    RUN_TEST(test_otp_send_generation_failure);
    RUN_TEST(test_otp_send_insert_returns_false);
    RUN_TEST(test_otp_send_template_failure_empty_err);
    RUN_TEST(test_otp_send_response_helpers_null);
    return UNITY_END();
}
...
void test_otp_send_null_args(void) {
    ...
    /* NULL request */
    MailRelayStatus st = mailrelay_otp_generate_and_send(NULL, &resp, err, sizeof(err));
    TEST_ASSERT_EQUAL(MAILRELAY_INVALID_ARGS, st);
    TEST_ASSERT_TRUE(strstr(err, "MAIL_PARAM_MISSING") != NULL);
    ...
    req.email = "user@example.com";
    req.purpose_a66 = MAIL_OTP_PURPOSE_LOGIN_MFA;
    st = mailrelay_otp_generate_and_send(&req, NULL, err, sizeof(err));
    TEST_ASSERT_EQUAL(MAILRELAY_INVALID_ARGS, st);
    TEST_ASSERT_EQUAL(0, g_insert_calls);
    TEST_ASSERT_EQUAL(0, g_send_calls);
}
...
void test_otp_send_req_overrides_config(void) {
    /* Request-level digits/expiry/max_attempts take precedence over config. */
    g_fake_fill = 3;
    ...
    MailRelayStatus st = mailrelay_otp_generate_and_send(&req, &resp, err, sizeof(err));
    TEST_ASSERT_EQUAL(MAILRELAY_OK, st);
    /* 8-digit code confirms req->digits was used. */
    TEST_ASSERT_EQUAL(8, (int)strlen(g_last_otp_seen_in_send));
    TEST_ASSERT_EQUAL_STRING("33333333", g_last_otp_seen_in_send);

    mailrelay_otp_send_response_free(&resp);
}

void test_otp_send_uses_defaults(void) {
    /* When neither the request nor config specify OTP settings, defaults apply. */
    g_test_config.mail_relay.Otp.Digits = 0;
    g_test_config.mail_relay.Otp.ExpirySeconds = 0;
    g_test_config.mail_relay.Otp.MaxAttempts = 0;
    ...
    MailRelayStatus st = mailrelay_otp_generate_and_send(&req, &resp, err, sizeof(err));
    TEST_ASSERT_EQUAL(MAILRELAY_OK, st);
    TEST_ASSERT_EQUAL(MAIL_OTP_DEFAULT_DIGITS, (int)strlen(g_last_otp_seen_in_send));

    mailrelay_otp_send_response_free(&resp);
}

void test_otp_send_invalid_digits(void) {
    /* A request digit count below MAIL_OTP_MIN_DIGITS is rejected before insert. */
    MailRelayOtpSendRequest req;
    memset(&req, 0, sizeof(req));
    req.email = "user@example.com";
    req.purpose_a66 = MAIL_OTP_PURPOSE_LOGIN_MFA;
    ...
    TEST_ASSERT_EQUAL(MAILRELAY_INVALID_ARGS, st);
    TEST_ASSERT_EQUAL(0, g_insert_calls);
    TEST_ASSERT_TRUE(strstr(err, "invalid OTP digits") != NULL);
}

void test_otp_send_generation_failure(void) {
    /* Random source failure aborts before any persistence or send. */
    g_random_should_fail = true;
    ...
    TEST_ASSERT_EQUAL(0, g_insert_calls);
    TEST_ASSERT_EQUAL(0, g_send_calls);
    TEST_ASSERT_TRUE(strstr(err, "MAIL_INTERNAL: OTP generation failed") != NULL);
}

void test_otp_send_insert_returns_false(void) {
    /* Executor returns false without invoking the callback -> generic error text. */
    g_insert_return_false = true;
    ...
    TEST_ASSERT_EQUAL(1, g_insert_calls);
    TEST_ASSERT_EQUAL(0, g_send_calls);
    TEST_ASSERT_TRUE(strstr(err, "OTP insert failed") != NULL);
    TEST_ASSERT_TRUE(strstr(err, "777777") == NULL);
}

void test_otp_send_template_failure_empty_err(void) {
    /* Send fails but leaves err empty -> caller supplies fallback message. */
    g_send_fail_empty_err = true;
    ...
    TEST_ASSERT_EQUAL(1, g_insert_calls);
    TEST_ASSERT_EQUAL(1, g_send_calls);
    TEST_ASSERT_TRUE(strstr(err, "MAIL_INTERNAL: OTP mail enqueue failed") != NULL);
}

void test_otp_send_response_helpers_null(void) {
    /* NULL-safe response init/free helpers. */
    mailrelay_otp_send_response_init(NULL);
    mailrelay_otp_send_response_free(NULL);
