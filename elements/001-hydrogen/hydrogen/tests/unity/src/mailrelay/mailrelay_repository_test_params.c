/*
 * Mail Relay Repository unit tests — parameter construction surface.
 *
 * Tests that each QueryRef helper invokes the execution seam with the correct
 * query_ref and builds the expected STRING/INTEGER BOOLEAN parameter JSON. Also
 * covers NULL-argument rejection for the helper wrappers.
 *
 * The execution-seam infrastructure itself (executor get/set, default executor,
 * database resolution, low-level repo_add_* primitives, and the ISO 8601
 * translator) lives in mailrelay_repository_test_seam.c.
 */
...
// Template helper parameter JSON tests
...
// Null argument tests for the helper wrappers
...
// Cleanup helper parameter JSON tests
