 ...
 *
 *   H.set_result("json", "inline:report:<job_id>")
 *
 *   - Reads the per-state job context to find the running job's
 *     job_id and scoreboard.
 *   - Copies the Lua strings out of Lua memory (UAF discipline from
 *     Phase 1).
 *   - Calls scoreboard_update_result, which strdup's its own
 *     copies and frees any prior values.
 *
 * Validates:
 *   - H.set_result is installed as a top-level function on H
 ...
 *     H.set_result is reflected in the scoreboard entry.
 ...
 *   - Non-string arguments: logs an error, does not raise.
 *   - Missing arguments: logs an error, does not raise.
 *   - The function returns no values.
 *   - Multiple updates overwrite the prior values.
 *   - Empty strings clear the fields.
 *   - Strings survive Lua GC.
 */
 ...
 // Mock logging introspection. H.set_result failure paths log
 ...
 void test_set_result_is_a_function_on_H(void);
 void test_set_result_direct_call_updates_scoreboard(void);
 void test_set_result_no_context_is_noop(void);
 void test_set_result_non_string_arg_logs_and_does_not_raise(void);
 void test_set_result_missing_arg_logs_and_does_not_raise(void);
 void test_set_result_returns_no_values(void);
 void test_set_result_multiple_calls_overwrite(void);
 ...
 // H.set_result is a function, not a table. Phase 26 replaces
 ...
 void test_set_result_is_a_function_on_H(void) {
     ...
     TEST_ASSERT_TRUE_MESSAGE(lua_isfunction(L, -1),
         "H.set_result must be a function");
     lua_pop(L, 2);
 ...
 // chunk that calls H.set_result, find the entry, assert the
 // fields are set.
 void test_set_result_direct_call_updates_scoreboard(void) {
     ...
     int rc = H_lua_run_string(L,
         "H.set_result('json', 'inline:report:abc12')",
         "[phase26:direct]");
     TEST_ASSERT_EQUAL_INT_MESSAGE(LUA_OK, rc,
         "H.set_result from a chunk should succeed");
     ...
     TEST_ASSERT_EQUAL_STRING("inline:report:abc12", e->result_location);
     scoreboard_entry_free(e);
     free(id);
 ...
 // context. H.set_result must be a clean no-op: no crash, no
 ...
 void test_set_result_no_context_is_noop(void) {
     ...
     int rc = H_lua_run_string(L,
         "H.set_result('json', 'inline:report:xyz')",
         "[phase26:noctx]");
     TEST_ASSERT_EQUAL_INT_MESSAGE(LUA_OK, rc,
 ...
 // Non-string arguments must not raise a Lua error. The host
 ...
 void test_set_result_non_string_arg_logs_and_does_not_raise(void) {
     ...
     TEST_ASSERT_NOT_NULL(strstr(mock_logging_get_last_message(), "must be strings"));
 ...
 void test_set_result_missing_arg_logs_and_does_not_raise(void) {
     ...
     TEST_ASSERT_EQUAL_INT_MESSAGE(LUA_OK, rc,
         "missing args must not raise a Lua error");

     TEST_ASSERT_NOT_NULL(strstr(mock_logging_get_last_message(), "missing string arguments"));

     H_lua_destroy_context(L);
 }

 // The function returns no values. A script that calls it without
 // assignment must not leave anything on the stack.
 void test_set_result_returns_no_values(void) {
     lua_State* L = H_lua_create_context();
     TEST_ASSERT_NOT_NULL(L);
     ...
     int rc = H_lua_run_string(L,
         "local before = select('#', ...)\n"
         "H.set_result('json', 'inline:test')\n"
         "local after = select('#', ...)\n"
         "return before, after\n",
         "[phase26:return-count]");
 ...
 // Multiple calls in the same job overwrite the prior values (the
 // scoreboard update frees the old strings before installing the new).
 void test_set_result_multiple_calls_overwrite(void) {
     Scoreboard* sb = scoreboard_create();
     ...
     int rc = H_lua_run_string(L,
         "H.set_result('json', 'inline:report:1')\n"
         "H.set_result('pdf', 'inline:report:2')\n"
         "H.set_result('csv', 'inline:report:3')\n",
         "[phase26:multi]");
     ...
     TEST_ASSERT_NOT_NULL(e);
     TEST_ASSERT_NOT_NULL(e->result_type);
     TEST_ASSERT_EQUAL_STRING("csv", e->result_type);
     TEST_ASSERT_NOT_NULL(e->result_location);
     TEST_ASSERT_EQUAL_STRING("inline:report:3", e->result_location);
     scoreboard_entry_free(e);
     free(id);
     scoreboard_destroy(sb);
 }

 // Empty strings are treated as "clear the fields" (the C side
 // converts them to NULL inside scoreboard_update_result).
 void test_set_result_empty_string_clears_fields(void) {
     Scoreboard* sb = scoreboard_create();
     char* id = scoreboard_submit(sb, "empty", NULL);
     scoreboard_update_result(sb, id, "initial_type", "initial_location");
     ...
     int rc = H_lua_run_string(L, "H.set_result('', '')\n",
                               "[phase26:empty]");
 ...
 // End-to-end via the worker pool: a submitted job that calls
 // H.set_result leaves non-NULL result_type and result_location on its
 // scoreboard entry when the job completes.
 void test_set_result_end_to_end_through_worker_pool(void) {
     TEST_ASSERT_TRUE(scripting_workers_init(2));
     char* id = scripting_submit_job_with_source("phase26_worker",
         "H.set_result('json', 'inline:report:done')\n"
         "return 0\n",
         NULL);
     TEST_ASSERT_NOT_NULL(id);
     ...
     TEST_ASSERT_EQUAL_STRING("json", e->result_type);
     TEST_ASSERT_NOT_NULL(e->result_location);
     TEST_ASSERT_EQUAL_STRING("inline:report:done", e->result_location);
     scoreboard_entry_free(e);
     free(id);
 }

 // The strings are copied to C-owned memory on the C side (UAF
 // discipline from Phase 1). A script that builds strings, reports
 // them, and then immediately drops the only Lua references must still
 // see the same values in the scoreboard.
 void test_set_result_strings_survive_lua_gc(void) {
     Scoreboard* sb = scoreboard_create();
     ...
     int rc = H_lua_run_string(L,
         // Build strings in a tight scope so they go out of scope
 ...
 int main(void) {
     ...
     RUN_TEST(test_set_result_is_a_function_on_H);
     RUN_TEST(test_set_result_direct_call_updates_scoreboard);
     RUN_TEST(test_set_result_no_context_is_noop);
     RUN_TEST(test_set_result_non_string_arg_logs_and_does_not_raise);
     RUN_TEST(test_set_result_missing_arg_logs_and_does_not_raise);
     RUN_TEST(test_set_result_returns_no_values);
