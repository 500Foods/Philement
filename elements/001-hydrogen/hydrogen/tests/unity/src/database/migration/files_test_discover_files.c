...
void test_sort_migration_files_single_element(void) {
    ...
    TEST_ASSERT_NOT_NULL(files); // Add null check for malloc failure

    files[0] = strdup("test_001.lua");
    TEST_ASSERT_NOT_NULL(files[0]); // Add null check for strdup failure
...
void test_sort_migration_files_already_sorted(void) {
    ...
    TEST_ASSERT_NOT_NULL(files[0]); // Add null checks for strdup failures
