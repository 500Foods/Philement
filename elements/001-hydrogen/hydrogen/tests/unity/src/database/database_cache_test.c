...
void setUp(void) {
    test_cache = query_cache_create();
    TEST_ASSERT_NOT_NULL(test_cache);
}

void tearDown(void) {
    if (test_cache) {
        query_cache_destroy(test_cache);
        test_cache = NULL;
    }
}
...
void test_query_cache_create_destroy(void) {
    QueryTableCache* cache = query_cache_create();
...
void test_query_cache_add_entry(void) {
    ...
    QueryCacheEntry* found = query_cache_lookup(test_cache, 789);
...
void test_query_cache_lookup_not_found(void) {
    QueryCacheEntry* found = query_cache_lookup(test_cache, 999);
...
void test_query_cache_usage_update(void) {
    ...
    QueryCacheEntry* found = query_cache_lookup(test_cache, 111);
    TEST_ASSERT_NOT_NULL(found);
    TEST_ASSERT_EQUAL(1, found->usage_count); // Incremented by lookup

    // Update usage
    query_cache_update_usage(test_cache, 111);

    // Lookup again to check increment
    found = query_cache_lookup(test_cache, 111);
    TEST_ASSERT_NOT_NULL(found);
...
void test_query_cache_stats(void) {
    ...
    query_cache_lookup(test_cache, 1);
    query_cache_lookup(test_cache, 1);
    query_cache_lookup(test_cache, 2);
...
void test_query_cache_resize(void) {
    ...
    for (int i = 0; i < 100; i++) {
        QueryCacheEntry* found = query_cache_lookup(test_cache, i);
        TEST_ASSERT_NOT_NULL(found);
