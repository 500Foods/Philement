/*
 * Unity Test File: MariaDB Query Coverage Enhancement Tests
...
void test_mariadb_execute_query_invalid_parameters(void);
void test_mariadb_execute_query_invalid_connection_handle(void);
void test_mariadb_execute_query_affected_rows_fallback(void);
void test_mariadb_execute_query_memory_allocation_failure(void);
void test_mariadb_execute_query_memory_allocation_failure_paths(void);
void test_mariadb_execute_query_non_select_query(void);
void test_mariadb_execute_prepared_invalid_parameters(void);
void test_mariadb_execute_prepared_invalid_connection_handle(void);
void test_mariadb_execute_prepared_no_executable_sql(void);
void test_mariadb_execute_prepared_execution_failure_paths(void);
void test_mariadb_execute_prepared_affected_rows_fallback(void);
void test_mariadb_execute_prepared_basic_execution(void);
void test_mariadb_execute_prepared_stmt_execute_unavailable(void);
void test_mariadb_execute_prepared_memory_allocation_failure(void);
...
// ============================================================================
// mariadb_execute_query additional coverage tests
// ============================================================================
...
void test_mariadb_execute_query_invalid_parameters(void) {
    ...
    success = mariadb_execute_query(connection, NULL, &result);
    TEST_ASSERT_FALSE(success);
    TEST_ASSERT_NULL(result);
    ...
    request.sql_template = strdup("SELECT 1");
    TEST_ASSERT_NOT_NULL(request.sql_template);
    success = mariadb_execute_query(connection, &request, NULL);
    TEST_ASSERT_FALSE(success);
    free(request.sql_template);
    free(connection);
    ...
    request.sql_template = strdup("SELECT 1");
    TEST_ASSERT_NOT_NULL(request.sql_template);
    success = mariadb_execute_query(connection, &request, &result);
    TEST_ASSERT_FALSE(success);
    TEST_ASSERT_NULL(result);
    free(request.sql_template);
    free(connection);
...
void test_mariadb_execute_query_invalid_connection_handle(void) {
    ...
    mariadb_conn->connection = NULL; // NULL MySQL connection
...
void test_mariadb_execute_query_affected_rows_fallback(void) {
    ...
    mock_libmariadb_set_mysql_affected_rows_result(0); // Mock function exists but returns 0
...
void test_mariadb_execute_query_non_select_query(void) {
    ...
    mock_libmariadb_set_mysql_query_result(0); // Success
    mock_libmariadb_set_mysql_store_result_result(NULL); // No result set
    mock_libmariadb_set_mysql_affected_rows_result(1); // 1 row affected
...
// ============================================================================
// mariadb_execute_prepared additional coverage tests
// ============================================================================
...
void test_mariadb_execute_prepared_invalid_parameters(void) {
    ...
    request.sql_template = strdup("SELECT 1");
    TEST_ASSERT_NOT_NULL(request.sql_template);
    bool success = mariadb_execute_prepared(NULL, &stmt, &request, &result);
    TEST_ASSERT_FALSE(success);
    TEST_ASSERT_NULL(result);
    free(stmt.name);
    free(request.sql_template);
    ...
    TEST_ASSERT_NOT_NULL(connection);
    connection->engine_type = DB_ENGINE_MARIADB;
    success = mariadb_execute_prepared(connection, NULL, &request, &result);
    TEST_ASSERT_FALSE(success);
    TEST_ASSERT_NULL(result);
    free(connection);
    ...
    stmt.name = strdup("test_stmt");
    TEST_ASSERT_NOT_NULL(stmt.name);
    success = mariadb_execute_prepared(connection, &stmt, NULL, &result);
    TEST_ASSERT_FALSE(success);
    TEST_ASSERT_NULL(result);
    free(stmt.name);
    free(connection);
    ...
    request.sql_template = strdup("SELECT 1");
    TEST_ASSERT_NOT_NULL(request.sql_template);
    success = mariadb_execute_prepared(connection, &stmt, &request, NULL);
    TEST_ASSERT_FALSE(success);
    free(stmt.name);
    free(request.sql_template);
    free(connection);
    ...
    request.sql_template = strdup("SELECT 1");
    TEST_ASSERT_NOT_NULL(request.sql_template);
    success = mariadb_execute_prepared(connection, &stmt, &request, &result);
    TEST_ASSERT_FALSE(success);
    TEST_ASSERT_NULL(result);
    free(stmt.name);
    free(request.sql_template);
...
void test_mariadb_execute_prepared_invalid_connection_handle(void) {
    ...
    // Test NULL connection handle
    connection->connection_handle = NULL;
    bool success = mariadb_execute_prepared(connection, &stmt, &request, &result);
    TEST_ASSERT_FALSE(success);
    TEST_ASSERT_NULL(result);
...
void test_mariadb_execute_prepared_stmt_execute_unavailable(void) {
    // Test mariadb_stmt_execute function unavailable (lines 266-268)
    // Note: Current mock infrastructure doesn't support making mariadb_stmt_execute unavailable
    // This test is skipped as the mock function is hardcoded to return 0 (success)
    TEST_IGNORE_MESSAGE("Mock infrastructure doesn't support mariadb_stmt_execute unavailability simulation");
}
...
void test_mariadb_execute_prepared_execution_failure_paths(void) {
    ...
    // These lines are ##### in BOTH blackbox and unity coverage files
    TEST_IGNORE_MESSAGE("Mock infrastructure doesn't support mariadb_stmt_execute failure simulation - hardcoded to return 0");
}
...
void test_mariadb_execute_prepared_affected_rows_fallback(void) {
    ...
    mock_libmariadb_set_mysql_store_result_result(NULL); // No result set for INSERT
...
int main(void) {
    ...
    // mariadb_execute_query additional coverage tests
    RUN_TEST(test_mariadb_execute_query_invalid_parameters);
    RUN_TEST(test_mariadb_execute_query_invalid_connection_handle);
    RUN_TEST(test_mariadb_execute_query_affected_rows_fallback); // Targets line 214
    RUN_TEST(test_mariadb_execute_query_memory_allocation_failure); // Skipped - requires mock_system
    RUN_TEST(test_mariadb_execute_query_non_select_query); // Targets lines 201-214

    // mariadb_execute_prepared additional coverage tests
    RUN_TEST(test_mariadb_execute_prepared_invalid_parameters); // Targets lines 225-228
    RUN_TEST(test_mariadb_execute_prepared_invalid_connection_handle);
    RUN_TEST(test_mariadb_execute_prepared_no_executable_sql); // Targets lines 244-263
    RUN_TEST(test_mariadb_execute_prepared_affected_rows_fallback); // Targets line 520
    RUN_TEST(test_mariadb_execute_prepared_basic_execution); // Targets lines 224-527 (basic execution)
    RUN_TEST(test_mariadb_execute_prepared_memory_allocation_failure); // Skipped - requires mock_system
