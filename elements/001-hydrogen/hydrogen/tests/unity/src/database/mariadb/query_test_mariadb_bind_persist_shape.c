/*
 * Unity Test File: MariaDB Parameter Binding - Persist Shape (PERSIST_PLAN)
 *
 ...
 *
 *   6. mariadb_cleanup_bound_values() releases every malloc'd length pointer
 *      - no leaks for the second-half length-pointer table
 *
 * This test uses the libmariadb mock (USE_MOCK_LIBMARIADB) but
 ...
 * bind array fields directly; it does NOT call any mariadb_stmt_* function
...
void test_persist_shape_json_null_uses_mariadb_type_null(void);
...
void test_persist_shape_length_non_null_for_all_types(void) {
    ...
    for (unsigned int i = 0; i < N; ++i) {
        TEST_ASSERT_TRUE(mariadb_bind_single_parameter(bind, i, params[i],
                                                      bound_values, N, "test"));
    }
    ...
    mariadb_cleanup_bound_values(bound_values, N);
    for (unsigned int i = 0; i < N; ++i) {
...
void test_persist_shape_boolean_length_allocated(void) {
    ...
    TEST_ASSERT_TRUE(mariadb_bind_single_parameter(bind, 0, p, bound_values, 1, "test"));
    ...
    mariadb_cleanup_bound_values(bound_values, 1);
...
void test_persist_shape_memory_failure_rollback_safe(void) {
    ...
    // mariadb_bind_single_parameter must return false and not leak.
    mock_system_set_malloc_failure(2);
    TEST_ASSERT_FALSE(mariadb_bind_single_parameter(bind, 0, p, bound_values, 1, "test"));
    mock_system_reset_all();
...
// ============================================================================
// mariadb_bind_attach_indicators: NULL bind is a no-op (defensive).
// ============================================================================

void test_persist_shape_attach_indicators_null_bind_safe(void) {
    mariadb_bind_attach_indicators(NULL, 0, 1);
    // If we reach here without crashing, the guard works.
    TEST_ASSERT_TRUE(true);
}

// ============================================================================
// mariadb_bind_single_parameter: NULL bind_ptr/param/bound_values must fail.
// ============================================================================
...
void test_persist_shape_bind_single_parameter_null_inputs_safe(void) {
    ...
    TEST_ASSERT_FALSE(mariadb_bind_single_parameter(NULL, 0, p, bound_values, 1, "test"));
    TEST_ASSERT_FALSE(mariadb_bind_single_parameter(bind, 0, NULL, bound_values, 1, "test"));
    TEST_ASSERT_FALSE(mariadb_bind_single_parameter(bind, 0, p, NULL, 1, "test"));
...
int main(void) {
    ...
    RUN_TEST(test_persist_shape_json_null_uses_mariadb_type_null);
