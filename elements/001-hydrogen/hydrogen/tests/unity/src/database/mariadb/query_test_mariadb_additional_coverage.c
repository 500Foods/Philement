/*
 * Unity Test File: MariaDB Query Additional Coverage Tests
...
bool mariadb_build_json_from_result(void* mariadb_result, size_t row_count, size_t column_count, char** column_names, char** json_buffer);
size_t mariadb_calculate_json_buffer_size(size_t row_count, size_t column_count);

// mariadb_cleanup_column_names is now in query.h

// Test function prototypes
void test_mariadb_helper_validate_query_parameters(void);
void test_mariadb_helper_execute_query_statement(void);
void test_mariadb_helper_execute_query_statement_empty_error(void);
void test_mariadb_helper_store_query_result(void);
void test_mariadb_helper_process_query_result_null(void);
void test_mariadb_helper_extract_column_names(void);
void test_mariadb_helper_build_json_from_result(void);
void test_mariadb_helper_calculate_json_buffer_size(void);
void test_mariadb_helper_cleanup_column_names(void);
...
void test_mariadb_helper_validate_query_parameters(void) {
    ...
    TEST_ASSERT_FALSE(mariadb_validate_query_parameters(NULL, &request, &result));
    
    // Test NULL request
    TEST_ASSERT_FALSE(mariadb_validate_query_parameters(&connection, NULL, &result));
    
    // Test NULL result pointer
    TEST_ASSERT_FALSE(mariadb_validate_query_parameters(&connection, &request, NULL));
    
    // Test wrong engine type
    connection.engine_type = DB_ENGINE_POSTGRESQL;
    TEST_ASSERT_FALSE(mariadb_validate_query_parameters(&connection, &request, &result));
    
    // Test valid parameters
    connection.engine_type = DB_ENGINE_MARIADB;
    TEST_ASSERT_TRUE(mariadb_validate_query_parameters(&connection, &request, &result));
}
...
void test_mariadb_helper_execute_query_statement(void) {
    ...
    TEST_ASSERT_TRUE(mariadb_execute_query_statement(connection, sql, designator));
    
    // Test failed query with error message
    mock_libmariadb_set_mysql_query_result(1);
    mock_libmariadb_set_mysql_error_result("Test error");
    TEST_ASSERT_FALSE(mariadb_execute_query_statement(connection, sql, designator));
}
...
void test_mariadb_helper_store_query_result(void) {
    ...
    mock_libmariadb_set_mysql_store_result_result(mock_result);
    void* result = mariadb_store_query_result(connection, designator);
    TEST_ASSERT_EQUAL_PTR(mock_result, result);
    
    // Test with no result
    mock_libmariadb_set_mysql_store_result_result(NULL);
    result = mariadb_store_query_result(connection, designator);
...
void test_mariadb_helper_process_query_result_null(void) {
    ...
    bool success = mariadb_process_query_result(NULL, &db_result, designator);
...
void test_mariadb_helper_extract_column_names(void) {
    ...
    // Test NULL result
    char** names = mariadb_extract_column_names(NULL, 2);
    TEST_ASSERT_NULL(names);
    
    // Test zero column count
    names = mariadb_extract_column_names(mock_result, 0);
    TEST_ASSERT_NULL(names);
    
    // Test valid extraction with setup
    const char* col_names[] = {"id", "name"};
    mock_libmariadb_setup_fields(2, col_names);
    
    names = mariadb_extract_column_names(mock_result, 2);
    TEST_ASSERT_NOT_NULL(names);
    TEST_ASSERT_EQUAL_STRING("id", names[0]);
    TEST_ASSERT_EQUAL_STRING("name", names[1]);
    
    mariadb_cleanup_column_names(names, 2);
}
...
void test_mariadb_helper_build_json_from_result(void) {
    ...
    // Test NULL result
    bool success = mariadb_build_json_from_result(NULL, 1, 2, column_names, &json_buffer);
    TEST_ASSERT_TRUE(success);
    ...
    // Test zero rows
    success = mariadb_build_json_from_result(mock_result, 0, 2, column_names, &json_buffer);
    TEST_ASSERT_TRUE(success);
    TEST_ASSERT_NOT_NULL(json_buffer);
    TEST_ASSERT_EQUAL_STRING("[]", json_buffer);
    free(json_buffer);
    ...
    // Test zero columns
    success = mariadb_build_json_from_result(mock_result, 1, 0, column_names, &json_buffer);
    TEST_ASSERT_TRUE(success);
    TEST_ASSERT_NOT_NULL(json_buffer);
    TEST_ASSERT_EQUAL_STRING("[]", json_buffer);
    free(json_buffer);
    ...
    success = mariadb_build_json_from_result(mock_result, 1, 2, column_names, NULL);
...
void test_mariadb_helper_calculate_json_buffer_size(void) {
    size_t size = mariadb_calculate_json_buffer_size(1, 5);
    TEST_ASSERT_EQUAL(1024, size);
    
    size = mariadb_calculate_json_buffer_size(10, 5);
    TEST_ASSERT_EQUAL(10240, size);
    
    size = mariadb_calculate_json_buffer_size(0, 5);
...
void test_mariadb_helper_cleanup_column_names(void) {
    ...
    // Should not crash
    mariadb_cleanup_column_names(names, 3);
...
int main(void) {
    ...
    RUN_TEST(test_mariadb_helper_validate_query_parameters);
    RUN_TEST(test_mariadb_helper_execute_query_statement);
    RUN_TEST(test_mariadb_helper_execute_query_statement_empty_error);
    RUN_TEST(test_mariadb_helper_store_query_result);
    RUN_TEST(test_mariadb_helper_process_query_result_null);
    RUN_TEST(test_mariadb_helper_extract_column_names);
    RUN_TEST(test_mariadb_helper_build_json_from_result);
    RUN_TEST(test_mariadb_helper_calculate_json_buffer_size);
    RUN_TEST(test_mariadb_helper_cleanup_column_names);
