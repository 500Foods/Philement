/*
 * Unity Test File: DB2 Interface Functions
 * This file contains unit tests for DB2 interface functions
 */

#include "../../../../../src/hydrogen.h"
#include "unity.h"

// Include necessary headers for the module being tested
#include "../../../../../src/database/database.h"
#include "../../../../../src/database/db2/interface.h"

// Forward declarations for functions being tested
DatabaseEngineInterface* db2_get_interface(void);

// Function prototypes for test functions
void test_db2_get_interface_not_null(void);
void test_db2_get_interface_valid_structure(void);
void test_db2_get_interface_function_pointers(void);

void setUp(void) {
    // Set up test fixtures, if any
}

void tearDown(void) {
    // Clean up test fixtures, if any
}

// Test db2_get_interface
void test_db2_get_interface_not_null(void) {
    DatabaseEngineInterface* interface = db2_get_interface();
    TEST_ASSERT_NOT_NULL(interface);
}

void test_db2_get_interface_valid_structure(void) {
    DatabaseEngineInterface* interface = db2_get_interface();
    TEST_ASSERT_NOT_NULL(interface);
    TEST_ASSERT_EQUAL(DB_ENGINE_DB2, interface->engine_type);
    TEST_ASSERT_NOT_NULL(interface->name);
    TEST_ASSERT_EQUAL_STRING("db2", interface->name);
}

void test_db2_get_interface_function_pointers(void) {
    DatabaseEngineInterface* interface = db2_get_interface();
    TEST_ASSERT_NOT_NULL(interface);

    // Test that essential function pointers are not null
    TEST_ASSERT_NOT_NULL(interface->connect);
    TEST_ASSERT_NOT_NULL(interface->disconnect);
    TEST_ASSERT_NOT_NULL(interface->execute_query);
    TEST_ASSERT_NOT_NULL(interface->begin_transaction);
    TEST_ASSERT_NOT_NULL(interface->commit_transaction);
    TEST_ASSERT_NOT_NULL(interface->rollback_transaction);
    TEST_ASSERT_NOT_NULL(interface->get_connection_string);
    TEST_ASSERT_NOT_NULL(interface->validate_connection_string);
}

int main(void) {
    UNITY_BEGIN();

    // Test db2_get_interface
    RUN_TEST(test_db2_get_interface_not_null);
    RUN_TEST(test_db2_get_interface_valid_structure);
    RUN_TEST(test_db2_get_interface_function_pointers);

    return UNITY_END();
}