...
# Ensure sufficient file descriptors and memory
...
# Verify test_00_all.sh exists and is executable
if [ ! -x "./test_00_all.sh" ]; then
    echo "Error: test_00_all.sh not found or not executable" | tee -a "$ERROR_LOG" "$SUMMARY_OUT"
    exit 1
fi

# Run strace with simplified options and verbose error logging
...
# Generate summary
{
    ...
    echo "-----------------------------------"
    echo "Insights:"
    echo "- High bash/sh counts (>100): Empty ~/.bashrc or use --norc to save ~0.1-0.5s."
    echo "- bash/sh >200: Switch to dash (~0.002-0.005s vs. 0.01-0.02s per spawn) for ~0.5-1s."
    echo "- High xargs counts (>10): Use -n 2 or -n 5 to reduce spawns (~0.5-1s)."
    echo "- High grep counts (>20): Use ripgrep (rg) for 2-10x speedup (~0.5-1s)."
    echo "- High shellcheck counts (~52): Cache results or use -n 5 (~0.5-0.8s)."
    echo "- High make counts (>1): Use ccache and tmpfs for test_01_compilation.sh (~2-3s)."
    echo "- Check $ERROR_LOG for strace errors or early termination."
} > "$SUMMARY_OUT"
