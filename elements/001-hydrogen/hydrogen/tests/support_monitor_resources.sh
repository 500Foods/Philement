...
#
NAME_SCRIPT="Hydrogen Resource Monitor"
VERS_SCRIPT="2.0.0"
...
while [ "$CURRENT_TIME" -lt "$END_TIME" ] && ps -p "$PID" >/dev/null; do
    ...
    # FD count
    FD_COUNT=$(find /proc/"$PID"/fd/ -type l 2>/dev/null | wc -l || echo "0")
    ...
    if [ $((ELAPSED % 10)) -eq 0 ]; then
        ...
        # File descriptors
        find /proc/"$PID"/fd/ -type l -exec ls -l {} \; > "$SNAPSHOT_DIR/open_files.txt" 2>/dev/null
...
if ps -p "$PID" >/dev/null; then
    ...
    find /proc/"$PID"/fd/ -type l -exec ls -l {} \; > "$FINAL_DIR/open_files.txt" 2>/dev/null
else
    FINAL_TIME=$(date +%s)
