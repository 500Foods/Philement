...
if check_unity_tests_available; then
    ...
    if run_unity_tests; then
        ...
        if [[ -n "${TOTAL_UNITY_TESTS}" ]] && [[ -n "${TOTAL_UNITY_PASSED}" ]]; then
            TEST_NAME="Unity  {BLUE}$(env LC_ALL=en_US.UTF_8 "${PRINTF}" "%'d" "${TOTAL_UNITY_PASSED}" || true) / $(env LC_ALL=en_US.UTF_8 "${PRINTF}" "%'d" "${TOTAL_UNITY_TESTS}" || true) unit tests passed{RESET}"
        fi
    else
        EXIT_CODE=1
    fi
