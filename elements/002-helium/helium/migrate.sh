...
# Usage: ./migrate.sh <command> [database] [schema] [migration]
#
# Commands:
#   generate <db> <schema> <migration>  - Generate SQL for specific migration
#   validate <db> <schema> <migration>  - Validate SQL syntax
#   status <db> <schema>                 - Show migration status
#   up <db> <schema> [target]            - Apply pending migrations
#   down <db> <schema> [steps]           - Rollback migrations
#   create <schema> <migration> <desc>   - Create new migration file
...
SCHEMA_NAME=${3:-helium}
MIGRATION_NUM=${4:-0000}

case "$1" in
    "generate")
        echo "Generating SQL for $DB_ENGINE ($SCHEMA_NAME schema, migration $MIGRATION_NUM)..."
        lua "$SCRIPT_DIR/test.lua" "$DB_ENGINE" "$SCHEMA_NAME" "$MIGRATION_NUM"
        ;;
