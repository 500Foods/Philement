...
# Usage: ./validate_sql.sh <design> <schema>
# Example: ./validate_sql.sh helium

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DESIGN_NAME=${1:-acuranzo}
SCHEMA_NAME=${1:-app}
...
for migration_file in $MIGRATION_FILES; do
    ...
    for engine in "${ENGINES[@]}"; do
        ...
        SQL_OUTPUT=$(lua "${SCRIPT_DIR}/get_migration.lua" "$engine" "$DESIGN_NAME" "$SCHEMA_NAME}" "$MIGRATION_NUM" 2>/dev/null | sed '/^Generated SQL/d; /^(/d; /^):/d')
