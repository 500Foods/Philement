-- Migration: acuranzo_1347.lua
-- PRIORITIZE 2.35: QueryRefs 147/148 SELECT retired
--
-- Do not restamp 1338 (already APPLIED). Adds c.retired.
-- #147 still returns retired published rows (unlike unpublish).
--
-- luacheck: no max line length
-- luacheck: no unused args
--
-- CHANGELOG
-- 1.0.0 - 2026-08-21 - Add retired to catalog QueryRefs 147/148
        ...
        [=[
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Add retired to QueryRefs 147/148'                                     AS name,
        [=[
            # Forward Migration ${MIGRATION}: Catalog retired flag

            PRIORITIZE 2.35. Replaces the SELECT body of public
            QueryRefs **147** (list) and **148** (by slug) so they
            return `retired`. Do **not** filter `retired=0` — retired
            published rows stay searchable. #147 still `published=1`
            only. Do not restamp 1338.
                ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
...
-- ----------------------------------------------------------------------------
-- Reverse: restore 1338 SELECT lists (tags, no retired)
-- ----------------------------------------------------------------------------
table.insert(queries,{sql=[====[
        ...
        [=[
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_APPLIED_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Restore QueryRefs 147/148 without retired'                            AS name,
        [=[
            # Reverse Migration ${MIGRATION}: Restore 1338 catalog SELECTs
                ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
