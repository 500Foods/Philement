-- Migration: acuranzo_1354.lua
-- PRIORITIZE 2.1 leftover: EnsureCanvasUser projects entitled Canvas seats
--
-- Do not restamp 1342.
--
...
--
-- CHANGELOG
-- 1.0.0 - 2026-08-21 - Project Helium seats onto Canvas after JIT link
...
table.insert(queries,{sql=[====[
    ...
    SELECT
        ...
        [=[
            ...
            SET code = [==[
-- Provision.EnsureCanvasUser (Phase 30/31; 1354 project entitled seats)
...
-- After link (and on a second poll of linked accounts whose
-- user_enrollments.canvas_enrollment_id is still NULL), project
-- Helium entitlement onto Canvas. PaidCourse 1353 may complete
-- the order before JIT exists. Archived rows are skipped.
...
local function stamp_canvas_enrollment(enrollment_id, canvas_enrollment_id)
    ...
    if serr then
        H.log.warn("EnsureCanvasUser: stamp canvas_enrollment_id err enrollment_id=%s: %s",
...
local function enroll_student(canvas_user_id, course_id)
    ...
    if not res then
        return nil, "canvas_enroll_failed:" .. tostring(err)
    end
    if res.status >= 200 and res.status < 300 then
        return object_id(res.body or ""), nil
    end
    local snippet = string.lower(string.sub(tostring(res.body or ""), 1, 400))
    if string.find(snippet, "already", 1, true) then
        return already_enrolled(canvas_user_id, course_id), nil
    end
    return nil, "canvas_enroll_http_" .. tostring(res.status)
end

local function project_entitled_seats(account_id, canvas_user_id)
    if not account_id or not canvas_user_id then
        return 0
    end
    local _er, eerr = H.query_sync([[
        SELECT enrollment_id, course_id, canvas_course_id
        FROM ${SCHEMA}user_enrollments
        WHERE account_id = :ACCOUNTID
    ...
    if eerr then
        H.log.warn("EnsureCanvasUser: pending seat query err account_id=%s: %s",
            tostring(account_id), tostring(eerr))
        return 0
    end
    local rows = qrows(_er)
    ...
    for _, row in ipairs(rows) do
        ...
        local cid = tonumber(pick(row, "canvas_course_id", "CANVAS_COURSE_ID"))
        if eid and cid and cid > 0 then
            local existing = already_enrolled(canvas_user_id, cid)
            if existing then
                stamp_canvas_enrollment(eid, existing)
                n = n + 1
            else
                local ceid, herr = enroll_student(canvas_user_id, cid)
                if ceid then
                    stamp_canvas_enrollment(eid, ceid)
                    n = n + 1
                else
                    H.log.warn("EnsureCanvasUser: project deferred enrollment_id=%s err=%s",
                        tostring(eid), tostring(herr))
                end
            end
        end
    end
...
local function ensure_one(account_id, email, display_name, sis_subject, aged)
    ...
    if canvas_user_id then
        ...
        project_entitled_seats(account_id, canvas_user_id)
    end
    return true
end
...
local _pr, perr = H.query_sync([[
    ...
    JOIN ${SCHEMA}accounts a ON a.account_id = ue.account_id
    JOIN ${SCHEMA}account_canvas_links acl ON acl.account_id = a.account_id
    WHERE ue.archived_at IS NULL
      AND ue.canvas_enrollment_id IS NULL
      AND ue.canvas_course_id IS NOT NULL
      AND ue.status IN ('active', 'completed')
      AND (ue.expires_at IS NULL OR ue.expires_at > NOW())
    ORDER BY a.account_id
    LIMIT 50
]])
if perr then
    H.log.warn("EnsureCanvasUser: pending projection query err: %s", tostring(perr))
else
    local prows = qrows(_pr)
    local pn = 0
    if prows then
        for _, row in ipairs(prows) do
            local account_id = row.account_id or row.ACCOUNT_ID
            local canvas_user_id = row.canvas_user_id or row.CANVAS_USER_ID
            pn = pn + project_entitled_seats(account_id, canvas_user_id)
        end
    end
    H.log.info("EnsureCanvasUser: projected %d seat(s)", pn)
end
...
H.set_current_state("done")
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'EnsureCanvasUser project entitled seats'                                                 AS name,
        [=[
            # Forward Migration ${MIGRATION}: EnsureCanvasUser project entitled seats

            PRIORITIZE 2.1 leftover. Do not restamp 1342. After JIT link, enroll Helium-entitled unarchived rows whose canvas_enrollment_id is still NULL.
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
