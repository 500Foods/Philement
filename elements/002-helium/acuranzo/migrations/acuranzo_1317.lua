-- Migration: acuranzo_1317.lua
-- STRIPE_PLAN Phase 5 follow-up: Stripe.Checkout nil-bind INSERT fix
--
-- Hydrogen drops nil named params. 1315 INSERT used :INTENT = nil and
-- failed. Create PaymentIntent first, then INSERT with pi_ id.

-- luacheck: no max line length
-- luacheck: no unused args

-- CHANGELOG
-- 1.0.0 - 2026-08-13 - Replace Stripe.Checkout body (nil INTENT bind)
...
-- ----------------------------------------------------------------------------
-- Forward: replace Stripe.Checkout body
-- ----------------------------------------------------------------------------
...
return 0
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Fix Stripe.Checkout nil INTENT bind' AS name,
        [=[
            # Forward Migration ${MIGRATION}: Fix Stripe.Checkout INSERT

            Hydrogen named binds omit nil values. 1315 inserted the
            order with `:INTENT = nil` before creating the PI, so
            DQM left a literal `:INTENT` and Yugabyte syntax-errored.

            Create the PaymentIntent first, then INSERT with the
            `pi_` string. Do not restamp 1315. No diagram.
...
table.insert(queries,{sql=[====[
    ...
    SELECT
        ...
        [=[
            UPDATE ${SCHEMA}scripts
            SET code = [==[
-- Stripe.Checkout (STRIPE_PLAN Phase 5)
-- JWT POST /api/conduit/script. Identity from params._hydrogen.
-- Input: cart.checkoutPayload().params { currency, items[] }.
-- Creates a PaymentIntent; returns client_secret. Fulfill on webhook.
-- Secrets: os.getenv("STRIPE_SECRET_KEY"). Never log sk_ / client_secret.
...
if ierr then
    ...
    local again = find_order(idem)
    if again then
        local pi = pick(again, "stripe_intent_id", "STRIPE_INTENT_ID")
        local oid = tonumber(pick(again, "order_id", "ORDER_ID"))
        if pi and pi ~= "" then
            local secret = retrieve_intent_secret(pi)
            if secret then
                H.set_result_json({
                    ok = true,
                    client_secret = secret,
...
local uerr = set_intent(order_id, pi)
if uerr then
    H.log.warn("Stripe.Checkout: store intent err: %s", tostring(uerr))
end
...
return 0
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_APPLIED_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Restore Stripe.Checkout 1315 body' AS name,
        [=[
            # Reverse Migration ${MIGRATION}: Restore Stripe.Checkout 1315 body
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
