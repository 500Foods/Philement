-- Migration: acuranzo_1336.lua
-- PRIORITIZE 2.29: seed Catalog.LogEvent (module, not invokable)
--
-- require("Catalog.LogEvent").record(opts)
-- INSERT catalog_events then best-effort Canvas wiki PUT.
...
--
-- CHANGELOG
-- 1.0.0 - 2026-08-20 - Seed Catalog.LogEvent module (PRIORITIZE 2.29)
...
-- ----------------------------------------------------------------------------
-- Forward: seed Catalog.LogEvent (not invokable)
-- ----------------------------------------------------------------------------
    ...
    SELECT
        ...
        [=[
            ...
            VALUES (
                ...
                1,
                [==[
-- Catalog.LogEvent (PRIORITIZE 2.29)
-- Module. require("Catalog.LogEvent").record(opts)
-- Not JWT-invokable. SyncFromCanvas calls it after a real field change.
-- INSERT is required; Canvas wiki PUT is best-effort.

local M = {}

local MODULE_NAME = "Course Information"
local PAGE_TITLE = "Management History"
local PAGE_URL = "management-history"
local QUERY_REF = 151
...
local function history_pre(course_id)
    ...
    if sql then
        ...
        if lerr then
            H.log.warn("Catalog.LogEvent: history query err: %s", tostring(lerr))
        else
        ...
        H.log.warn("Catalog.LogEvent: QueryRef %s load err: %s", tostring(QUERY_REF), tostring(serr))
    end
    if #lines == 0 then
        lines[1] = "(no management events)"
    end
    return "<pre>\n" .. table.concat(lines, "\n") .. "\n</pre>"
end
...
function M.record(opts)
    ...
    if course_id == 0 or event_type == "" then
        return { ok = false, code = "validation", message = "course_id, event_type required" }
    end
    local actor = str_or_empty(opts.actor)
    if actor == "" then actor = "system" end
    local canvas_course_id = nzi(opts.canvas_course_id)
    local detail = str_or_empty(opts.detail)
    local eid = next_event_id()
    local _, ierr = H.query_sync([[
        INSERT INTO ${SCHEMA}catalog_events (
            event_id, course_id, canvas_course_id,
            event_type, actor, detail,
            valid_after, valid_until, created_id, created_at, updated_id, updated_at
        ) VALUES (
            :EID, :COURSEID, NULLIF(:CANVASCOURSE, 0),
            :EVENTTYPE, :ACTOR, NULLIF(:DETAIL, ''),
    ...
    if ierr then
        H.log.warn("Catalog.LogEvent: insert err: %s", tostring(ierr))
        return { ok = false, code = "write_error", message = "Could not write catalog event" }
    end
    local pok, perr = project_canvas(course_id, canvas_course_id)
    if not pok then
        H.log.warn("Catalog.LogEvent: canvas project course_id=%s: %s",
...
return M
                ...
                'PRIORITIZE 2.29: catalog_events writer + Canvas Management History PUT',
                0,
                ${COMMON_VALUES}
            );
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Seed Catalog.LogEvent module script'                               AS name,
        [=[
            # Forward Migration ${MIGRATION}: Seed Catalog.LogEvent

            Inserts `Catalog.LogEvent` with `invokable = 0`.
            Writers `require("Catalog.LogEvent")` and call `record`.

            1. INSERT `catalog_events` (never a missing named bind;
            ...
            2. Best-effort Canvas projection: unpublished module
               ...
               "Management History" (`management-history`), body =
               `<pre>` + QueryRef #151 `line` rows. Page update uses
               POST + `X-HTTP-Method-Override: PUT`.
            3. Canvas HTTP failure is logged; `record` still returns
               `ok=true` with `canvas_ok=false`. Sync must not fail
               closed on the wiki PUT.
        ...
        [=[
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_APPLIED_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Remove Catalog.LogEvent script'                                    AS name,
        [=[
            # Reverse Migration ${MIGRATION}: Remove Catalog.LogEvent
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
