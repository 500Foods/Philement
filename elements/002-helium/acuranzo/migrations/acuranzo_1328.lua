-- Migration: acuranzo_1328.lua
-- PRIORITIZE 2.9: Enroll.FreeCourse appends enrollment_events
--
-- Do not restamp 1321. After a new Helium free grant, require
...
--
-- CHANGELOG
-- 1.0.0 - 2026-08-20 - FreeCourse logs enrolment events (PRIORITIZE 2.9)
...
table.insert(queries,{sql=[====[
    ...
    SELECT
        ...
        [=[
            ...
            SET code = [==[
-- Enroll.FreeCourse (Phase 49; 1321 user_enrollments; 1328 LogEvent)
...
return succeed(eerr == "already", canvas_user_id, canvas_course_id)
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'FreeCourse logs enrollment_events'                                                         AS name,
        [=[
            # Forward Migration ${MIGRATION}: Enroll.FreeCourse LogEvent

            After a **new** Helium free grant, `require("Enroll.LogEvent")`
            and `record` (`event_type=enrolled`, `actor=self`). Existing
            rows stay silent (`exists`). Canvas wiki PUT is best-effort
            inside LogEvent. No diagram (script restamp).
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
