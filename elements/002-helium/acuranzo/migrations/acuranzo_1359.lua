-- Migration: acuranzo_1359.lua
-- PRIORITIZE 2.18: Enroll.SyncEnrollments Canvas heal
--
-- Do not restamp 1332. Unenroll Canvas when Helium is archived /
-- revoked / expired. Do not auto-enroll Canvas for active Helium
-- rows with no seat (Unarchive / grant paths only). Insert Helium
-- only when no row exists (archive sticky).
--
-- luacheck: no max line length
-- luacheck: no unused args
--
-- CHANGELOG
-- 1.0.0 - 2026-08-21 - SyncEnrollments Canvas heal (PRIORITIZE 2.18)
...
table.insert(queries,{sql=[====[
    ...
    SELECT
        ...
        [=[
            ...
            SET code = [==[
-- Enroll.SyncEnrollments (PRIORITIZE 2.8 / 2.18)
-- JWT POST /api/conduit/script. Identity from params._hydrogen.
-- Backfill user_enrollments from Canvas seats. Archive is sticky.
-- 2.18: unenroll Canvas when Helium is archived / revoked / expired.
-- Do not mint a Canvas seat just because Helium is active.
-- Secrets: CANVAS_*.
...
if herr then
    H.log.warn("SyncEnrollments: helium list err: %s", tostring(herr))
else
    local hrows = qrows(_hr) or {}
    for _, hrow in ipairs(hrows) do
        local archived = present(pick(hrow, "archived_at", "ARCHIVED_AT"))
        local ended = nzi(pick(hrow, "ended", "ENDED")) == 1
        if (archived or ended) and type(seat) == "table" and type(seat.unenroll) == "function" then
            local cid = nzi(pick(hrow, "canvas_course_id", "CANVAS_COURSE_ID"))
            local ceid = nzi(pick(hrow, "canvas_enrollment_id", "CANVAS_ENROLLMENT_ID"))
            local eid = nzi(pick(hrow, "enrollment_id", "ENROLLMENT_ID"))
            local course_id = nzi(pick(hrow, "course_id", "COURSE_ID"))
            local u = seat.unenroll({
                canvas_user_id = canvas_user_id,
                canvas_course_id = cid,
                canvas_enrollment_id = ceid,
            })
            if type(u) == "table" and u.ok == true then
                if ceid > 0 then
                    H.query_sync([[
                        UPDATE ${SCHEMA}user_enrollments
                           SET canvas_enrollment_id = NULL,
                               updated_at = NOW()
                         WHERE enrollment_id = :EID
                           AND account_id = :ACCOUNTID
                    ]], { EID = eid, ACCOUNTID = account_id })
                end
                if not u.already then
                    healed = healed + 1
                    try_log_event({
                ...
                H.log.warn("SyncEnrollments: heal unenroll failed enrollment_id=%s code=%s",
                    tostring(eid), tostring(u and u.code))
            end
        end
    end
end
...
return 0
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_FORWARD_MIGRATION};
                                                                            ...
                                                                            AS code,
        'SyncEnrollments Canvas heal'                                       AS name,
        [=[
            # Forward Migration ${MIGRATION}: SyncEnrollments 2.18

            Heal: unenroll Canvas when Helium is archived / revoked /
            expired. Never auto-enroll Canvas for an active Helium row.
            Insert Helium only when no row exists.
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
...
-- ----------------------------------------------------------------------------
-- Reverse: restore 1332 body (see acuranzo_1332.lua forward)
-- ----------------------------------------------------------------------------
table.insert(queries,{sql=[====[
...
return 0
            ...
            WHERE query_ref = ${MIGRATION}
              and query_type_a28 = ${TYPE_APPLIED_MIGRATION};
                                                                            ...
                                                                            AS code,
        'Restore Enroll.SyncEnrollments 1332 body'            AS name,
        [=[
# Reverse Migration ${MIGRATION}: restore 1332 SyncEnrollments
        ]=]
                                                                            AS summary,
        '{}'                                                                AS collection,
        ${COMMON_INSERT}
