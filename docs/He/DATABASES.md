...
- **Status**: Fifth Hydrogen SQL engine (`libfbclient`). Lookup 030 key 6. Test 37 is the full Acuranzo migration run.
- **Features**: Native Base64 and SHA-256, `RETURNING`, `TIMESTAMP WITH TIME ZONE`. JSON is `BLOB SUB_TYPE TEXT` until Firebird 6.
- **Use Cases**: A local SQL engine with a database file as the isolation boundary. `${SCHEMA}` is empty.
- **Setup**: Fedora package Firebird 4.0 SuperServer on port 3050. Brotli and `JSON_VALUE` are UDRs, not in-process code. See [database_firebird.md](/docs/He/DATABASES/database_firebird.md) and [extras/firebird/README.md](/elements/001-hydrogen/hydrogen/extras/firebird/README.md).
- **Not in this list**: CockroachDB was a PostgreSQL alias and has been retired. Firestore was never a SQL engine.
...
| Concept | PostgreSQL | MySQL | SQLite | DB2 | Firebird |
|---------|------------|-------|--------|-----|----------|
| Auto-increment | `SERIAL` | `AUTO_INCREMENT` | `AUTOINCREMENT` | `GENERATED ALWAYS AS IDENTITY` | `GENERATED BY DEFAULT AS IDENTITY` |
| Large text | `TEXT` | `TEXT` | `TEXT` | `CLOB(1M)` | `BLOB SUB_TYPE TEXT` |
| JSON | `JSONB` | `LONGTEXT` | `TEXT` | `CLOB(1M)` | `BLOB SUB_TYPE TEXT` |
| Timestamps | `TIMESTAMPTZ` | `TIMESTAMP` | `TEXT` | `TIMESTAMP` | `TIMESTAMP WITH TIME ZONE` |

### Encoding and Compression

- **Base64 Decoding**: All databases support, but with different function names
- **Brotli Compression**: Supported via UDFs on PostgreSQL, MySQL, DB2, and Firebird; SQLite uses an extension
...
- **Firebird**: File-backed SQL engine. Native Base64 and SHA-256. Brotli and JSON extract are UDRs.
...
- **Firebird 4**: Local SQL engine when the deployment wants a server and a `.fdb` file rather than SQLite or PostgreSQL.
