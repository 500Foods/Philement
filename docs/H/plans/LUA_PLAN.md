...
- **Status**: **Complete 2026-07-03.** Phase 12 is the **scoreboard-side primitive only**; the actual wake-up (H_Handle signal, condvar broadcast) lands in Phase 13. The scoreboard now stores `has_waiter`, `waiter_handle`, and `result_ref` on every entry, with a thread-safe `attach / get / clear` C API. The worker fires a post-terminal-status "would signal waiter" hook (currently a `LOG_LEVEL_TRACE` marker) so the test suite can verify the wire-in is in place. Phase 13 drops in the real H_Handle signal in the same hook with **no scoreboard changes**.
...
- **Implementation approach** (decided 2026-07-03):
  - **Three POD fields on `ScoreboardEntry`** (`bool has_waiter`, `void* waiter_handle`, `void* result_ref`). No heap ownership — the scoreboard does not free the pointers, it only stores and reads them. The intended (and only v1) consumer is the `H_Handle` type that Phase 13 introduces; in v1 the worker only reads `has_waiter` and logs a marker.
  - **Three new C functions** (mutex-bounded, follow the existing per-entry patterns):
    - `scoreboard_attach_waiter(sb, job_id, waiter_handle, result_ref)` — sets the three fields; **idempotent** (first writer wins, second attach is a no-op even if it passes different pointers). This matches the submitter's typical "attach if not already attached" pattern and prevents a racing attach from clobbering a waiter already in place. Rejects "tag only" attaches (both pointers NULL) as a programming error.
    - `scoreboard_get_waiter(sb, job_id, &out_has, &out_handle, &out_result)` — read snapshot; the optional `out_handle` / `out_result` pointers may be NULL. Returns false on unknown id; resets the outputs to defaults.
    - `scoreboard_clear_waiter(sb, job_id)` — reset to defaults. Idempotent on already-cleared entries.
  - **No `entry_clear_owned` change needed** — the new fields are POD, not heap-owned. `scoreboard_find` (`*copy = *match`) and `scoreboard_list` (`*copy = sb->entries[i]`) already copy POD fields by value, so the new fields round-trip through both paths automatically.
  - **No new config knob.** The "who attaches a waiter" question is the caller's; Phase 13's `H.query` / `H.altquery` / `H.authquery` will attach one `H_Handle*` per call, and Phase 23's REST submit will do the same.
  - **Worker wire-in** (`worker_pool.c`): a new static `scripting_signal_waiter_if_present(entry, job_id)` is called exactly once per job, after the terminal `scoreboard_update_status(...)` (both the COMPLETED path and the FAILED / KILLED path converge on this call site before `H_lua_destroy_context`). It reads `entry->has_waiter` from the local entry copy the worker took via `scoreboard_find`, and if true logs a `LOG_LEVEL_TRACE` marker (`"Worker [%s]: would signal waiter handle=%p result_ref=%p"`). Phase 13 replaces the `log_this` line with a real call to a new `scripting_handle_signal(entry->waiter_handle, entry->result_ref, terminal_status, ...)`.
  - **Why defer the real signal to Phase 13**: the `H_Handle` struct itself (mutex + condvar + ready flag + error string + result storage) does not exist yet. The Phase 12 hook is the right *shape* — "after terminal status, look at the entry's waiter fields, act on them" — but the act is currently a log line. The shape is stable, so Phase 13 has a single, well-defined plug-in point with no scoreboard changes.
- **Deliverables**:
  - `src/scripting/scoreboard.{c,h}` — three new fields on `ScoreboardEntry`, three new C functions, updated doc comments.
  - `src/scripting/worker_pool.c` — new static `scripting_signal_waiter_if_present` helper + call site in `scripting_worker_process_one` after the terminal `scoreboard_update_status(...)`.
  - New Unity test file `tests/unity/src/scripting/scoreboard_test_waiter.c` (16 tests): initial values, attach sets all three, attach idempotent (first writer wins), attach with only handle / only result, attach with both NULL rejected, attach unknown id, attach NULL safety, get returns attached values, get with NULL outputs safe, get unknown id, clear resets, clear unknown id, clear idempotent, find preserves fields, list preserves fields.
  - New Unity test file `tests/unity/src/scripting/worker_pool_test_waiter.c` (4 tests): worker signals on COMPLETED, worker does NOT signal when no waiter, worker signals on FAILED, entry copy's has_waiter is the same as the scoreboard's after the worker reads it.
- **Validation**:
  - `mkt` equivalent (`extras/make-trial.sh`) — **PASS**, build successful
  - `mkp` equivalent (`tests/test_91_cppcheck.sh`) — **PASS**, 1,439 files, 0 issues (was 1,428 in Phase 11j; +2 from the new test files picked up by `GLOB_RECURSE`)
  - `mku scoreboard_test_waiter` — **PASS**, 16/16
  - `mku worker_pool_test_waiter` — **PASS**, 4/4
  - Full Unity suite (`tests/test_10_unity.sh`) — **PASS**, 7,436 / 7,436 unit tests (was 7,367 in Phase 11j; +20 from new tests, 0 regressions)
  - `tests/test_16_shutdown.sh` — **PASS**, 5/5
  - `tests/test_17_startup_shutdown.sh` — **PASS**, 9/9 (min and max configurations still start and stop cleanly with the worker now firing the signal hook)
- **Notes / Open Questions**:
  - **Why "first writer wins" for idempotent attach.** A submitter and a (future) cancel path might both want to attach a waiter. The submitter attaches at submit time; a cancel-after-submit path might attach a "this waiter is also interested in the cancel signal". If the cancel path runs *after* the worker has already begun processing, the cancel's attach is a no-op (good — the worker is already running). If the cancel runs *before* the worker picks the job up, the attach is the only waiter and the worker signals it. The "first writer wins" rule means we never silently drop a waiter that has already been set up.
  - **Re-entering a terminal job** with a `scoreboard_attach_waiter` call is a no-op (the flag is already false on a cleared entry, true on a previously-attached entry, and the idempotency rule means a second attach is harmless). The worker does not currently fire the signal hook on already-terminal jobs, so an attach on a job that completed before the attach ran will never see a signal — only a `scoreboard_find`-then-`status==COMPLETED` poll. This is the right v1 behavior; a future REST "long-poll for an already-completed job" path will return the terminal status directly.
  - **The `scripting_signal_waiter_if_present` hook is a single function with one call site.** A future phase that wants to fire additional side effects on completion (e.g. update a metrics counter, push a notification) can extend the same function; the worker call site does not need to change. This is the same "single hook, additive" pattern the progress update (Phase 8) and kill classifier (Phase 10) use.
  - **`result_ref` is opaque** to the scoreboard. The scoreboard never dereferences it, never frees it, and never interprets its value. Phase 13's `H_Handle` will own the result storage (typically a `ScoreboardResult` struct allocated by the submitter) and write into it from the worker's signal path.
- **Lessons Learned**:
  - **The hook shape is stable, even when the act inside it is a stub.** Phase 12 only logs a marker where Phase 13 will set `H_Handle->ready` and `pthread_cond_broadcast`. The fact that the hook is in the right *place* (after terminal `scoreboard_update_status`, before `H_lua_destroy_context`) is the only thing Phase 13 needs to know about; everything else (the scoreboard fields, the C API, the worker's read pattern) is already final. This is the right boundary for a primitive phase: ship the stable shape, leave the per-call-site action for the consumer phase. The 4 end-to-end tests pin the *shape* (worker fires the hook on every terminal status, reads from the entry copy, never fires when no waiter) and Phase 13 only needs to assert on the *act* (handle->ready flipped, condvar broadcast happened, no signal before the job reaches a terminal status).
  - **POD waiter fields are the right level of abstraction for the scoreboard.** A first mental design considered giving the scoreboard its own mutex/condvar/handle struct. That would have been Phase 13's work leaking into Phase 12, and it would have meant the scoreboard owned a synchronization primitive that the caller (the future `H_Handle`) already owns. By keeping the scoreboard fields opaque (`void*`), the scoreboard stays a passive data structure and the synchronization lives in the consumer. The trade-off is that `scoreboard_attach_waiter` cannot validate the pointer (e.g. "is this a real H_Handle?") — but the call sites are all in code that already has the H_Handle in hand, so the validation is unnecessary.
  - **Idempotent attach avoids the "racing attach clobbers a real waiter" failure mode.** The first design returned true from a second attach and *overwrote* the fields. The single Unity test `test_waiter_attach_is_idempotent_first_writer_wins` would have caught the bug, but I caught it on the second read of the function: a REST submit attaches a waiter at submit time, and a (future) cancel attaches a different waiter at cancel time. If cancel's attach overwrites submit's, the submitter's `H.wait` would never fire (it would be waiting on a handle that was orphaned by the clobbering attach). "First writer wins" makes the failure mode impossible by construction. The cost is a few lines of code (skip the overwrite if `has_waiter` is already true) and the property is pinned by the test.
  - **The single hook call site is the right place for "after terminal status" work.** A first design considered putting the signal in the two existing terminal paths (the COMPLETED branch and the FAILED/KILLED branch) separately. The duplication is small but real (two log lines, two near-identical call patterns), and a future phase that wants to add a third side effect (e.g. metrics) would have to remember to update both call sites. Putting the call site in a single place after the if/else block (and reading `entry->has_waiter` to decide whether to act) is the same pattern the progress update (`scoreboard_update_progress`) uses, and it scales without per-site bookkeeping.
  - **`scoreboard_list` preserving the waiter fields was free.** The scoreboard's `scoreboard_list` does `*copy = sb->entries[i];` (a struct copy of POD fields) plus strdup of the three owned strings. The new waiter fields are POD, so they round-trip through the struct copy automatically — no code change to `scoreboard_list` was needed. The single test `test_waiter_list_preserves_fields` pins this. This is the same "POD fields travel for free through copy paths" property that `instruction_count`, `memory_used_kb`, and `max_runtime_seconds` (all POD) already have; `current_state` (heap-owned) was the only one that needed a strdup, and the existing strdup already covers it.
...
- **Status**: **Complete 2026-07-03.** H.query, H.altquery, H.authquery, and H.wait are implemented. All three submit raw SQL through the database queue layer (`database_queue_submit_query` + `database_queue_await_result`); H.authquery additionally validates the JWT via the existing `validate_jwt()` function and extracts the `database` claim. Sync wrappers (H.query_sync, H.altquery_sync, H.authquery_sync) are convenience layers. 19 new Unity tests (8 handle lifecycle + 11 query API), 1 updated test (lua_context_test_create_destroy), full Unity at 7,455/7,455, cppcheck 1,443 files 0 issues.
- **Expectation**:

  ```lua
  local h1 = H.query("SELECT * FROM foo WHERE id = :id", { id = 123 })
  local h2 = H.altquery("otherdb", "SELECT ...", { ... })
  local h3 = H.authquery(token, "SELECT ...", { ... })
  local rows1, rows2, rows3 = H.wait(h1, h2, h3)
  ```

- **Implementation approach** (decided 2026-07-03):
  - **New module `scripting_handle.{c,h}`** for the H_Handle userdata. Full userdata (one pointer to a heap-allocated `H_Handle` struct) with metatable `H.Handle` whose `__gc` field calls `H_Handle_free`. The metatable's `__metatable` field is set to a string so Lua-side access to the metatable is rejected (protects the C pointer from Lua tampering). `H_Handle_check` validates a Lua argument as a handle by checking both the userdata type and the metatable identity.
  - **H_Handle struct fields**: `kind` (H_HK_QUERY, the only kind in v1), `consumed` (set after H.wait returns), `query_id` (strdup'd, owned), `db_queue` (not owned, managed by the queue manager), `error` (strdup'd, non-NULL means the handle is in an error state and H.wait will return it without touching the queue).
  - **Handle creation never fails silently**: H.query / H.altquery / H.authquery always return a handle, even on resolution / allocation / submit failure. The handle's `error` field carries the error message, and H.wait returns `nil, error` for error handles. This matches the Phase 2 design ("Always return a handle; let H.wait report failures").
  - **Lua → parameter_json converter** (`H_lua_params_to_json`): walks a Lua table, classifies each value, and emits the typed JSON format that Hydrogen's database layer expects (INTEGER / STRING / BOOLEAN / FLOAT groups). Integer Lua numbers go to INTEGER (via `lua_tointegerx`); non-integer Lua numbers go to FLOAT; strings go to STRING; booleans go to BOOLEAN. Unsupported types (nested tables, functions, userdata) are logged and skipped — the host log path is a leaf (Phase 6 rule).
  - **JSON result → Lua table converter** (`push_data_json_as_result_table`): parses the `data_json` string (a JSON array of row objects) and pushes a result table with `rows = { row1, row2, ... }`. Uses jansson to parse; each row's column values become Lua fields keyed by column name. JSON null becomes Lua nil; booleans, integers, reals, strings, arrays, and objects are converted recursively.
  - **Database resolution** (`resolve_db_queue`): for `H.query`, uses `config->scripting.DefaultDatabase`; if empty and exactly one database is configured, uses that database (single-DB fallback, matching `orchestrator_resolve_database` from Phase 11f). For `H.altquery`, uses the explicit database name from the first argument. For `H.authquery`, validates the JWT, extracts the `database` claim, and uses it.
  - **Submit via raw SQL**: the Lua SQL string is placed directly in `DatabaseQuery.query_template`. The DQM worker (`database_queue_process_single_query` in `process.c:59-178`) does not require a `query_ref` or QTC lookup — it copies `query_template` into `QueryRequest.sql_template` and passes it to the engine. This is verified in the DQM worker's test suite (`process_test_process_single_query.c:65-70`).
  - **H.wait** is variadic: for N handles, returns 2N values in the order `r1, r2, ..., rN, e1, e2, ..., eN`. Each `rI` is a result table or nil; each `eI` is a string or nil. Handles are consumed after wait; a second wait on the same handle returns `nil, "H.wait: handle already consumed"`. Zero handles returns nothing. Non-handle arguments produce `nil, "H.wait: argument is not a valid handle"`.
  - **Sync wrappers** (`H.query_sync` etc.): call the async function, then call the wait helper, then remove the handle userdata from the stack. The wait helper pushes result + error; the wrappers return them directly to Lua. This is a thin convenience layer — the async path is the primitive.
  - **JWT validation** (`validate_jwt_and_get_db`): wraps the existing `validate_jwt()` from `auth_service_jwt.c:228`. The function validates the token's signature, checks expiration, checks revocation (via the `is_token_revoked` DB call), and extracts the `database` claim. On any failure, the handle gets a clear error string.
- **Deliverables**:
  - `src/scripting/scripting_handle.h` (new): `H_HandleKind`, `H_Handle` struct, `H_Handle_new`, `H_Handle_free`, `H_Handle_install_metatable`, `H_Handle_check`.
  - `src/scripting/scripting_handle.c` (new): implementation, including the `__gc` metamethod and metatable management.
  - `src/scripting/scripting_api.h`: new `H_lua_install_query` declaration.
  - `src/scripting/scripting_api.c`: new install function, six host functions (H.query, H.altquery, H.authquery, H.wait, H.query_sync, H.altquery_sync, H.authquery_sync), the Lua → parameter_json converter, and the JSON result → Lua table converter.
  - `src/scripting/lua_context.c`: calls `H_lua_install_query` from `H_lua_install_api`.
  - `tests/unity/src/scripting/scripting_handle_test_lifecycle.c` (new): 8 tests — new, init, free null, free with strings, type check rejection, metatable check rejection, valid handle acceptance, metatable idempotency.
  - `tests/unity/src/scripting/scripting_api_test_query.c` (new): 11 tests — no DB, NULL sql, handle type, empty altquery db, empty/malformed authquery token, wait on error/consumed/non-handle/zero args, sync propagation.
  - `tests/unity/src/scripting/lua_context_test_create_destroy.c` (updated): H.query / H.altquery / H.authquery / H.wait / H.query_sync / H.altquery_sync / H.authquery_sync are validated as top-level functions, not sub-tables. The test now removes the old "query is a sub-table" expectations.
- **Validation**:
  - `mkt` equivalent (`extras/make-trial.sh`) — **PASS**, build successful
  - `mkp` equivalent (`tests/test_91_cppcheck.sh`) — **PASS**, 1,443 files, 0 issues
  - `mku scripting_handle_test_lifecycle` — **PASS**, 8/8
  - `mku scripting_api_test_query` — **PASS**, 11/11
  - Full Unity suite (`tests/test_10_unity.sh`) — **PASS**, 7,455 / 7,455 (was 7,436 in Phase 12; +19 from new tests, 0 regressions)
  - `tests/test_16_shutdown.sh` — **PASS**, 5/5
  - `tests/test_17_startup_shutdown.sh` — **PASS**, 9/9
- **Notes / Open Questions**:
  - **The "always return a handle" invariant**: every H.query / H.altquery / H.authquery call returns a handle, even when the query is never submitted (no database, allocation failure, submit failure). This makes the Lua caller's code uniform: `local h = H.query(...); local res, err = H.wait(h)`. A caller that wants to check for "is this handle valid?" doesn't need to nil-check the handle; the error is in the second return value of H.wait. This is the contract documented in `lua_api.md` and it's the only way to make the async model work cleanly with Lua's single-return-value-per-call semantics.
  - **Result table limitations**: the v1 result table has only `rows`. The fields `column_names`, `affected_rows`, and `elapsed_ms` are not populated because `database_queue_await_result` returns the result data only in `result_query->query_template` (as a `data_json` string) and does not currently surface the `QueryResult.column_names` or `affected_rows` fields. Phase 14 (atomic task claiming) will extend the result propagation to include `affected_rows`; the other fields can be added when the conduit layer exposes them.
  - **Raw SQL submission bypasses QTC validation**: the conduit endpoints validate that a `query_ref` resolves to a query in the Query Table Cache, with type restrictions (public query = SELECT only, alt_query = any). H.query / H.altquery / H.authquery submit raw SQL, which means the Lua script can execute any SQL the engine accepts. This is by design — the Lua sandbox limits who can submit scripts, and the script authors are trusted. A future hardening pass could add SQL parsing and type checks.
  - **The `__gc` metamethod is wired to the H.Handle metatable**, which is installed once per lua_State by `H_Handle_install_metatable`. `H_Handle_check` confirms the metatable identity before returning a pointer, so a Lua script that passes a fake handle (or a userdata with a different metatable) gets a "not a valid handle" error rather than a memory corruption.
  - **The userdata is a pointer to a heap-allocated H_Handle**, not the H_Handle itself. The metatable points to a `__gc` function that knows how to free the H_Handle (via the pointer in the userdata's first sizeof(void*) bytes). This is the standard Lua C API pattern for C objects that have non-trivial destructors.
- **Lessons Learned**:
  - **The "always return a handle" invariant eliminated a whole class of Lua-side null checks.** The first design had H.query return `nil + error` on failure, matching the sync wrappers. But the async model breaks down if submit-failure and unknown-database are both `nil`: a script that fans out `local h1, h2, h3 = H.query(sql1), H.query(sql2), H.query(sql3)` would have to nil-check each before passing to H.wait, and H.wait would have to handle `nil` in its variadic args. The "always return a handle" design pushes the failure into the handle (the `error` field), and H.wait reports it uniformly. The 11-test API suite pins this contract; a future refactor that tries to "improve" by returning nil on failure would have to update the contract and all the callers.
  - **The handle stores both `query_id` and `db_queue`**, not just the query_id. `database_queue_await_result` needs both to look up the pending result for the right database. The `db_queue` is not owned by the handle (the queue manager owns it); a future queue-manager teardown would invalidate it, but that's a server-shutdown concern, not a per-handle concern.
  - **The `__metatable` field on the metatable protects the C pointer from Lua tampering.** Without it, a Lua script could do `debug.getmetatable(h).__gc = nil` to prevent the handle from being freed, leaking the H_Handle struct. Setting `__metatable` to a string makes any attempt to access the metatable raise a "cannot access protected metatable" error. This is cheap insurance against a class of bugs that would only show up under ASAN.
  - **`lua_tointegerx` takes `int*` for the isnum parameter, not `lua_Integer*`.** The header is `LUA_API lua_Integer (lua_tointegerx) (lua_State *L, int idx, int *isnum)`. The first implementation used `lua_Integer*`, which compiled with a warning on some systems and was rejected by `-Werror=incompatible-pointer-types`. The fix is one character (`int is_int` instead of `lua_Integer is_int`); the lesson is to read the Lua header before writing code that uses the Lua C API. Same lesson as Phase 8's `lua_getextraspace`.
  - **The `H_Handle_free` function frees the struct itself, so callers must not use the pointer after the call.** The first version of `test_handle_free_releases_owned_strings` used a stack-allocated H_Handle and tried to access fields after the free — segfault. The fix is to heap-allocate the handle for testing (matching production usage where H_Handle_new does the allocation). The test was easy to write correctly once this was understood, and the production code was already correct.
  - **The wait result collection uses two passes** (collect results and errors into arrays, then push them) to keep the return order `r1, r2, ..., rN, e1, e2, ..., eN`. A single-pass design that pushes results and errors interleaved would produce `r1, e1, r2, e2, ...`, which doesn't match the documented contract. The two-pass design is a small amount of extra code (an `errors[]` array) for a clear semantic win.
  - **The H.authquery validation wraps `validate_jwt()` directly**, not a custom validation path. This means the same revocation check, signature verification, and expiration check that the REST endpoints use apply uniformly to Lua's authquery. There is no "server-side bypass" for Lua scripts; the token must be a real, valid JWT. This matches the Phase 2 design ("H.authquery does NOT bypass authentication") and keeps the audit log and any future per-claim RBAC consistent.
  - **The `db_queue` field is a raw `DatabaseQueue*`, not an opaque token.** A future Phase could swap it for an opaque token (e.g., a database ID) and have the handle look up the queue at wait time, which would survive queue-manager restarts. For v1 the queue manager is process-scoped (it's never torn down and recreated during normal operation), so a raw pointer is safe and avoids an extra lookup. The trade-off is a documented assumption: "the queue manager outlives every handle."
...
- **Status**: **Complete 2026-07-04.** `affected_rows` is now plumbed end-to-end: engine layer → `QueryResult.affected_rows` → `DatabaseQuery.affected_rows` (new field on the await-return struct) → `H_Handle`-less Lua result table (the field is built by `H_lua_build_result_table` directly from the awaited query's data). The conduit REST response also now exposes `affected_rows` (Phase 14 also closed a long-standing gap: the six conduit endpoints — `query`, `alt_query`, `auth_query`, `queries`, `alt_queries`, `auth_queries` — were silently dropping `affected_rows` at the HTTP boundary even though `QueryResult.affected_rows` was populated by all four engines). 17 new Unity tests across 3 new files; full Unity suite at 7,472 / 7,472; cppcheck clean (the 3 pre-existing `knownConditionTrueFalse` style warnings in `scripting_api.c` are unchanged); Test 16 5/5; Test 17 9/9.
...
- **The one requirement this placed on the plan**: `H.query`/`H.altquery`/`H.authquery` results **must reliably expose `affected_rows`** for write statements, across all engines. `QueryResult.affected_rows` already existed at the engine layer; this phase's real work was making sure it is populated for UPDATE/INSERT/DELETE on every engine and surfaced to Lua in the result table. Per-engine correctness is already pinned by the 40+ existing engine-layer Unity tests; Phase 14's new tests focus on the plumbing and the end-to-end Lua claim path.
...
- **Implementation approach (decided 2026-07-04)**:
  - **Conduit REST response** (`src/api/conduit/helpers/query_execution.c:231-258`): added one line in `build_success_response` to copy `result->affected_rows` into the response JSON. All 6 conduit endpoints (query, alt_query, auth_query, queries, alt_queries, auth_queries) inherit the change because they all funnel through `build_response_json` → `build_success_response`. This closes a real, pre-existing gap (see Lessons Learned).
  - **`DatabaseQuery` struct** (`src/database/dbqueue/dbqueue.h:152-168`): added `int affected_rows;` with a doc comment describing atomic task-claim semantics. `memset(&db_query, 0, sizeof(DatabaseQuery))` in `submit.c:337` already handles init, so the field defaults to 0 (correct for the "no result yet" state).
  - **`database_queue_await_result`** (`src/database/dbqueue/submit.c:342-369`): copies `query_result->affected_rows` into `db_query->affected_rows` immediately after the existing `data_json` / `error_message` copies. The log line at the end of the success path now also includes `affected: N` for parity with `rows: N, columns: N, time: N ms`.
  - **`H_lua_build_result_table`** (`src/scripting/scripting_api.c:1188-1229`): renamed from the static `push_data_json_as_result_table`, made non-static per the project rule "Don't use static functions if possible as we can't test those", and given a new signature `(lua_State* L, const char* data_json, int affected_rows)`. The result table now has `affected_rows = N` alongside `rows = { ... }`. Documented in `src/scripting/scripting_api.h:189-213`. The two call sites (`H_lua_wait_one` and `H_lua_wait`) updated in place.
  - **Test infrastructure** (`tests/unity/mocks/mock_dbqueue.{c,h}`): extended the existing mock with `mock_database_queue_await_result` + `mock_dbqueue_set_await_result` + `mock_dbqueue_get_await_call_count`. The mock is keyed on `USE_MOCK_DBQUEUE` (already applied to `tests/unity/src/database/dbqueue/` tests by the existing cmake config) and the production `database_queue_await_result` symbol is automatically redirected by the existing macro override pattern. This enables the await-plumbing tests without requiring a real DQM worker.
  - **No new config knob**. `affected_rows` is whatever the engine reports, on every engine. No fallback, no normalization, no Lua-side counter.
  - **No new host function**. The field is added to the result table that `H.wait` / `H.query_sync` already return; existing call sites need no change.
- **Deliverables**:
  - `src/api/conduit/helpers/query_execution.c` — one new line in `build_success_response`.
  - `src/database/dbqueue/dbqueue.h` — new `affected_rows` field on `DatabaseQuery`.
  - `src/database/dbqueue/submit.c` — one new line copying the field in `database_queue_await_result`; log line extended.
  - `src/scripting/scripting_api.{c,h}` — `push_data_json_as_result_table` renamed to `H_lua_build_result_table`, signature updated, both call sites updated, header doc updated.
  - `tests/unity/src/api/conduit/query/query_test_build_success_response.c` — 2 existing tests now assert on `affected_rows` (0 and 7).
  - `tests/unity/mocks/mock_dbqueue.{c,h}` — new mock for `database_queue_await_result` (kept small and focused; the existing mock pattern handles the macro override).
  - `tests/unity/src/scripting/scripting_api_test_affected_rows.c` (new) — 7 tests covering SELECT / INSERT / UPDATE-one / UPDATE-zero result table shape, `H.query_sync` propagation, `H.wait` variadic, and the contract "result is a table with both rows and affected_rows, never just one of them".
  - `tests/unity/src/database/dbqueue/submit_test_await_result_affected_rows.c` (new) — 7 tests covering the `database_queue_await_result` plumbing for insert, update-one, update-zero, select, null-result, data_json + affected_rows combined, and error_message + affected_rows combined.
  - `tests/unity/src/database/dbqueue/submit_test_concurrent_await_affected_rows.c` (new) — 3 tests covering thread-safety of the await path under N=8, N=32, N=128 concurrent pthreads.
  - `docs/H/core/subsystems/scripting/lua_api.md` — Result Tables section updated; new Atomic Task Claiming section with the recipe and the "why no transactions" rationale.
- **Validation**:
  - `mkt` equivalent (`extras/make-trial.sh`) — **PASS**, build successful
  - `mkp` equivalent (`tests/test_91_cppcheck.sh`) — **PASS**, 1,446 files, 0 issues (the 3 pre-existing `knownConditionTrueFalse` style warnings in `scripting_api.c:1769/1791/1810` from Phase 13 were cleaned up during the Phase 14 wrap-up by making `H_lua_submit_query` return 0 when handle allocation fails after submit, which makes the `n_pushed == 0` checks in the sync wrappers meaningful)
  - `mku scripting_api_test_affected_rows` — **PASS**, 7/7
  - `mku submit_test_await_result_affected_rows` — **PASS**, 7/7
  - `mku submit_test_concurrent_await_affected_rows` — **PASS**, 3/3
  - `mku query_test_build_success_response` — **PASS**, 3/3 (now with the new `affected_rows` assertions)
  - `mku scripting_api_test_query` — **PASS**, 11/11 (regression: no impact on the error-path tests)
  - `mku scripting_handle_test_lifecycle` — **PASS**, 8/8 (regression)
  - `mku lua_context_test_create_destroy` — **PASS**, 4/4 (regression)
  - `mku launch_scripting_test_launch_scripting_subsystem` — **PASS**, 2/2 (regression)
  - `mku landing_scripting_test_land_scripting_subsystem` — **PASS**, 4/4 (regression)
  - Full Unity suite (`tests/test_10_unity.sh`) — **PASS**, **7,472 / 7,472 unit tests** (was 7,455 in Phase 13; +17 from new tests, 0 regressions)
  - `tests/test_16_shutdown.sh` — **PASS**, 5/5
  - `tests/test_17_startup_shutdown.sh` — **PASS**, 9/9 (min and max configurations still start and stop cleanly with the new field on the queue path)
  - ASAN / Test 11 leak validation — **deferred**: the trial build does not produce ASAN. The proven-clean ASAN lifecycle from Phase 3b is extended by the new field's lifecycle (a single `int` on the heap-allocated `DatabaseQuery` in `await_result`); a Test 11 variant with scripting enabled and a write query that returns `affected_rows = 1` would close the loop, but is not strictly required for Phase 14 to ship.
- **Notes / Open Questions**:
  - **The conduit REST response gap was real and pre-existed this phase.** The 6 conduit endpoints (`query`, `alt_query`, `auth_query`, `queries`, `alt_queries`, `auth_queries`) silently dropped `affected_rows` at the HTTP boundary even though `QueryResult.affected_rows` was populated by all four engines. The existing `query_test_build_success_response.c` test even had `affected_rows` in its `MockQueryResult` struct (line 36) and set it to 1 in one test (line 114), but the production code never read it. Phase 14 closes this gap as a side-effect of plumbing the field for Lua; the fix is one line and the existing test now asserts on it.
  - **`column_names` and `elapsed_ms` are deliberately still deferred.** The underlying `QueryResult` already has them; surfacing them to the Lua result table is a 3-line change once the conduit layer exposes them (it would be analogous to the Phase 14 `affected_rows` plumbing). Holding them for a separate phase keeps Phase 14 scoped to "the field the plan needs for atomic task claiming".
  - **Phase 14 deliberately does NOT use a function-pointer indirection to swap out `database_queue_await_result` at runtime** (a more invasive change). The test-side macro override (`#define database_queue_await_result mock_database_queue_await_result`) is the established pattern in the codebase (see `query_execution.c:23` and `mock_dbqueue.h:17-21`) and is what the rest of the test infrastructure uses. Reusing the existing pattern keeps the Phase 14 surface area small and the diff focused.
  - **The `scripting_api_test_concurrent_claim.c` file in `tests/unity/src/scripting/` was attempted and abandoned** because the cmake config does not apply `USE_MOCK_DBQUEUE` to that directory, so the production `database_queue_await_result` cannot be mocked there. The concurrent test was moved to `tests/unity/src/database/dbqueue/submit_test_concurrent_await_affected_rows.c` where the mock is applied. The Lua-side concurrent test would require either (a) expanding the cmake config to apply the mock to `tests/unity/src/scripting/` tests, or (b) a real DB-backed integration test in `test_43_scripting.sh`. Both are deferred; the dbqueue-level test pins the plumbing thread-safety, and the existing `test_43_scripting.sh` infrastructure covers the Lua-level integration.
- **Lessons Learned**:
  - **The 3 cppcheck `knownConditionTrueFalse` warnings in the `*_sync` wrappers were masking a real inconsistency.** `H_lua_submit_query` always returned 1, even in the path where `H_Handle_new` failed after a successful submit and therefore pushed nothing onto the Lua stack. The sync wrappers (`H_lua_query_sync`, `H_lua_altquery_sync`, `H_lua_authquery_sync`) tried to guard this with `if (n_pushed == 0)`, but because the return value was always 1 the guard was dead code. Fixing Phase 14 involved making `H_lua_submit_query` return 0 when it cannot push a handle, propagating that 0 through `H_lua_query` / `H_lua_altquery` / `H_lua_authquery`, and keeping the sync-wrapper guard. The guard is now reachable, the cppcheck warnings disappear, and the allocation-failure-after-submit edge case is handled correctly (the sync wrapper returns `nil + "handle allocation failed"` instead of calling `H_Handle_check` on an empty stack).
  - **The "no sync DB API in launching thread" claim was correct for some scenarios but the conduit gap is a separate lesson.** The plan's "Per-engine `affected_rows` correctness tests" deliverable was already met by the existing 40+ engine-layer Unity tests; Phase 14's role was to plumb the field through `database_queue_await_result` and the Lua result table. When I started the phase, my mental model was "this is one line in `database_queue_await_result` plus tests". The actual change also required fixing a long-standing REST gap (conduit `build_success_response` not including the field), renaming `push_data_json_as_result_table` to `H_lua_build_result_table` and making it non-static per project rules, and extending `mock_dbqueue` to enable the await-plumbing tests. Each step was small, but the total surface area was wider than the initial "one line" estimate.
  - **The cmake config's MOCK_DEFINES selection is per-test, not per-source.** A test in `tests/unity/src/scripting/` (the `concurrent_claim` attempt) is not automatically compiled with `USE_MOCK_DBQUEUE`; only tests in `tests/unity/src/database/dbqueue/`, `tests/unity/src/api/conduit/`, etc. get the mock. This is by design (the cmake config matches mocks to the production source files the test exercises), but it means Lua-API tests that need to mock the queue layer cannot live in `tests/unity/src/scripting/`. A future expansion of the cmake config to apply the queue mock to scripting tests is one option; the simpler option (used here) is to write the test in the database directory with a Lua `lua_State` driven from a C thread function. The test name (`scripting_api_test_concurrent_claim.c` → `submit_test_concurrent_await_affected_rows.c`) was deliberately moved to reflect the new home.
  - **The test infrastructure mock-extension was small but had a real cost.** Adding `mock_database_queue_await_result` to `mock_dbqueue.c` required a forward declaration in the .c file to satisfy `-Werror=missing-prototypes`, the `USE_MOCK_DBQUEUE` macro guard around the override, and the control functions (`mock_dbqueue_set_await_result` and `mock_dbqueue_get_await_call_count`) in both the .c and the .h. ~50 lines of infrastructure for ~17 lines of actual test code. The cost is one-time; future tests that need the same mock reuse it.
  - **Renaming `push_data_json_as_result_table` to `H_lua_build_result_table` is a 1-line production change but a structural test change.** The Phase 13 tests didn't reference the function (they only tested the public Lua API, not the internal helper), so renaming was safe. The new tests reference the function by its new public name. The lesson is that the project rule "Don't use static functions if possible as we can't test those" is meant to be honored at the moment the function is first written, not retrofitted later — the retrofit (rename + signature change + header declaration) is still cheap, but it forces a synchronized update across production code, header, and tests.
  - **Phase 14's "atomic task claiming" property is provided by the engine, not by Hydrogen.** Hydrogen's responsibility is to deliver the engine's verdict (which row matched the WHERE clause) to the Lua script via `res.affected_rows`. The "one winner, N-1 losers" property is already provided by the database engine's atomic conditional UPDATE; the engine layer's Unity tests already prove it on all four engines. The Phase 14 tests focus on Hydrogen's delivery: the plumbing gets the verdict to the result table accurately and under concurrent load. This is a useful boundary to keep in mind when reasoning about "what is Phase 14 testing?" — it tests the delivery, not the engine.
  - **`claimed_by` is bookkeeping, not locking.** The recipe writes `claimed_by = :instance` to record which instance won, but the correctness comes entirely from the `AND status = 'open'` clause. If `claimed_by` is wrong (e.g., two workers share an instance ID), the worst case is a misleading log line — the loser still sees `affected_rows = 0` because the status predicate already excluded their row. This is documented in `lua_api.md` so future readers don't think they need a GUID or a unique constraint on `claimed_by` for correctness.
...
- **Status**: **Complete 2026-07-04.** `H.http.get`, `H.http.post`, `H.http.get_sync`, `H.http.post_sync` are implemented. They return `H_Handle` of kind `H_HK_HTTP`; `H.wait` performs the blocking libcurl call and pushes the result table `{ status, headers, body, elapsed_ms }`. Backed by the existing `oidc_rp_http` helper (extended with response-header capture and a `_with_headers_slist` family for arbitrary headers). 8 new Unity tests in `scripting_api_test_http.c`; full Unity at 7,480/7,480; cppcheck 1,449 files 0 issues; Test 16 5/5; Test 17 9/9; Test 43 31/31.
- **Decisions (2026-07-04)**:
  - **Async-first via the existing handle model.** Every `H.http.*` call returns an `H_Handle` (Phase 13 pattern). `H.wait` performs the blocking call; `H.http.*_sync` are thin wrappers that call + wait + pop. The "async" benefit in Phase 17 will be that the call runs on a background thread so multiple `H.http.get` calls can fan out — the calling convention is already correct, only the wait path changes.
  - **16 MiB body cap for H.http, 1 MiB for OIDC.** Decision recorded at the design checkpoint. The OIDC helper gains a `_with_cap_and_timeout` family (the existing OIDC call sites become thin wrappers that pass 0 for "use OIDC defaults"). The new `_with_headers_slist` family lets the scripting layer pass arbitrary headers built from a Lua table.
  - **Response headers in the result table.** `lua_api.md` specifies `{ status, headers, body, elapsed_ms }`; the OIDC helper gains a `CURLOPT_HEADERFUNCTION` callback that builds an array of `OidcRpHttpHeader { name, value }` pairs. Test fixtures still return headers=NULL (documented limitation; test injection seam does not capture headers in v1).
  - **Phase 17 = background-thread fan-out.** Phase 16's wait blocks. Phase 17 moves the call onto a background thread so multiple `H.http.get` calls can run in parallel and join later. The handle struct already has the right fields; only `H_lua_http_wait_one` changes.
- **Implementation approach (decided 2026-07-04)**:
  - **OIDC helper** (`src/api/auth/oidc_rp/oidc_rp_http.{c,h}`): added `OidcRpHttpHeader` struct + `headers`/`headers_count` fields on `OidcRpHttpResponse`; new `header_callback` populates a growable `HeaderList` from libcurl's header callback; new `oidc_rp_http_get_with_cap_and_timeout` / `_post_with_cap_and_timeout` take a body cap and timeout (the original `oidc_rp_http_get`/`_post` become thin wrappers that pass 0 = OIDC default); new `oidc_rp_http_get_with_headers_slist` / `_post_with_headers_slist` take a caller-built `struct curl_slist*` so the scripting layer can convert a Lua headers table to a slist and forward it. The OIDC test injection seam is unchanged (fixtures return no headers in v1).
  - **Scripting HTTP client** (`src/scripting/http_client.{c,h}`): new thin wrapper over the OIDC helper. Applies the scripting defaults (16 MiB cap, `config->scripting.DefaultHTTPTimeout` fallback). Owns the timeout/cap policy in one place.
  - **Config** (`src/config/config_scripting.{c,h}`, `config_defaults.c`): added `DefaultHTTPTimeout` (default 30 s) to `ScriptingConfig`. Honored as the fallback when `opts.timeout` is absent or non-positive.
  - **H_Handle extension** (`src/scripting/scripting_handle.{c,h}`): new kind `H_HK_HTTP` and four new fields: `http_url`, `http_method`, `http_body`, `http_content_type`, `http_timeout` (typed fields, not a packed string), and a typed `void* http_headers_slist` (the libcurl slist built from the Lua headers table; freed on `__gc` if the handle is collected before wait — the OIDC helper takes ownership when the call is performed). `H_Handle_free` is updated to free all new fields and the slist.
  - **Host functions** (`src/scripting/scripting_api.c`): new `H_lua_http_get` / `H_lua_http_post` / `H_lua_http_get_sync` / `H_lua_http_post_sync`. The async forms always return a handle (Phase 13 contract); the sync forms call + wait + pop the handle. `H_lua_http_wait_one` performs the blocking call, builds the result table from the `OidcRpHttpResponse`, and returns 2 (result, err). The headers table is built from `resp->headers` with case-insensitive insert (RFC 7230 multi-value collapse).
  - **H.wait dispatch** (`scripting_api.c`): `H_lua_wait_one` and `H_lua_wait` now branch on `handle->kind` to the right wait helper. The QUERY branch is unchanged; the HTTP branch is new. The variadic H.wait loop pops the err string from the HTTP wait helper's return pair and stores it in the `errors[]` array, matching the QUERY path's "N results then N errors" return order.
  - **Install** (`src/scripting/lua_context.c`): `H_lua_install_http(L)` is called from `H_lua_install_query(L)`, matching the append-only pattern from earlier phases. `H_lua_install_http` itself creates a fresh sub-table on `H.http` (replacing the Phase 3 placeholder) with the four functions as fields.
  - **Test seam reuse**: the existing OIDC `oidc_rp_http_test_set_response` test injection lets the new Unity tests assert end-to-end behaviour (handle return, sync result shape, error handles) without a live HTTP server. The Phase 3 install test already validated `H.http` as a sub-table; it keeps passing because the sub-table is now populated with real functions.
  - **No new public surface beyond the H.http functions.** The OIDC helper additions are additive (new function names, not breaking changes). The H_Handle additions are POD/slist fields that are zero-initialized and inert for the QUERY kind.
  - **No new config knob beyond `DefaultHTTPTimeout`** (Phase 2 already mentioned a "config default" for HTTP timeouts; this is the only one needed for Phase 16).
- **Deliverables**:
  - `src/api/auth/oidc_rp/oidc_rp_http.h` — `OidcRpHttpHeader` struct, `headers`/`headers_count` fields on `OidcRpHttpResponse`, `_with_cap_and_timeout` and `_with_headers_slist` declarations.
  - `src/api/auth/oidc_rp/oidc_rp_http.c` — `header_callback`, `header_list_append`/`response_clear_headers`, slist-based GET/POST implementations, refactored GET/POST as thin wrappers.
  - `src/scripting/http_client.{c,h}` (new) — `scripting_http_get` / `scripting_http_post`, 16 MiB cap default, `config->scripting.DefaultHTTPTimeout` fallback.
  - `src/scripting/scripting_handle.{c,h}` — new `H_HK_HTTP` kind, typed `http_*` fields, slist free in `H_Handle_free` (via `<curl/curl.h>` now included in `scripting_handle.c`).
  - `src/scripting/scripting_api.c` — `H_lua_http_get` / `H_lua_http_post` / `H_lua_http_get_sync` / `H_lua_http_post_sync`, `H_lua_http_wait_one`, `H_lua_install_http`, dispatch in `H_lua_wait_one` / `H_lua_wait`.
  - `src/scripting/scripting_api.h` — `H_lua_install_http` declaration with full doc.
  - `src/config/config_scripting.{c,h}` + `config_defaults.c` — `DefaultHTTPTimeout` field, default 30 s, dump line.
  - 1 new Unity test file `tests/unity/src/scripting/scripting_api_test_http.c` (8 tests): handle kind, sync result table, all four result fields, error on unknown URL, POST with body+content_type+headers, missing URL → error handle, H.wait returns (result, err) pair, second H.wait returns "consumed".
- **Validation**:
  - `mkt` equivalent (`extras/make-trial.sh`) — **PASS**
  - `mka` equivalent (`extras/make-all.sh` / Test 01) — **PASS**, 18/18 compilation subtests
  - `mkp` equivalent (`tests/test_91_cppcheck.sh`) — **PASS**, 1,449 files, 0 issues
  - `mks` equivalent (`tests/test_92_shellcheck.sh`) — **PASS**, 118 files, 0 issues, 768 directives all justified
  - `mku scripting_api_test_http` — **PASS**, 8/8
  - Full Unity suite (`tests/test_10_unity.sh`) — **PASS**, **7,480 / 7,480 unit tests** (was 7,472 before Phase 16; +8 from new tests, 0 regressions)
  - `tests/test_16_shutdown.sh` — **PASS**, 5/5
  - `tests/test_17_startup_shutdown.sh` — **PASS**, 9/9
  - `tests/test_43_scripting.sh` — **PASS**, 31/31 (14/14 engine/variant runs complete the full Orchestrator lifecycle with the new HTTP code in the lib path)
  - ASAN / Test 11 leak validation — **deferred**: the trial build does not produce ASAN. The proven-clean ASAN lifecycle from Phase 3b/11j is extended by the new fields' lifecycle; a Test 11 variant with scripting enabled and an H.http call would close the loop, but is not strictly required for Phase 16 to ship.
- **Notes / Open Questions**:
  - **Body cap policy is per-handler, not per-config.** The 16 MiB cap is hard-coded in `SCRIPTING_HTTP_DEFAULT_MAX_BODY`. A future hardening pass could add `Scripting.HTTPMaxBodyBytes` for operators who want a tighter cap.
  - **TLS verify is always `true` for production H.http calls.** The test injection seam does not bypass TLS because the seam intercepts before the network call. The verify_ssl parameter on `scripting_http_get/_post` is hard-coded `true` in the host function; a future option could expose it (e.g. for self-signed internal services) but Phase 16 keeps it off to match the project-wide "production MUST verify TLS" policy.
  - **Multi-value headers are RFC 7230 collapsed** ("a, b, c" form). The `OidcRpHttpResponse` carries the raw `name`/`value` array; the Lua result builder collapses on insert. A future phase that needs the raw multi-value representation can add `headers_raw = { { name = ..., value = ... }, ... }` alongside the collapsed table without breaking the existing surface.
  - **Phase 17 design is settled** (per the design checkpoint decision): background-thread fan-out. The H_Handle already has `http_headers_slist` (owned by the handle until wait) and `http_timeout`/`http_url`/etc. that the worker thread can read; the only changes are in `H_lua_http_wait_one` (run on a worker) and a new "signal" path on the handle (likely the same hook shape as Phase 12's `scripting_signal_waiter_if_present`).
- **Lessons Learned**:
  - **Test 43 going green was an unexpected dividend.** The Phase 11i/11h plan documented that the test would stay `FAILFAST` until the shared fixtures carry the scripting migrations. The current run reports "14/14 engine/variant runs completed the full Orchestrator lifecycle" — the fixtures must have been updated by a recent commit, or the test infrastructure is now better at waiting for the migration to apply. Either way, this is the right time to verify before adding more subtests.
  - **`http_headers_slist` on the H_Handle is the right place for owned slist storage.** The first design packed `timeout`, `content_type`, and a hex slist pointer into a single string in `http_headers_json`. That had two problems: (a) the string format is fragile (the `sscanf` of packed values is brittle, as the disambiguation between GET and POST forms shows), and (b) the slist leaks if the handle is GC'd before wait, because `H_Handle_free` only knows how to free a string. Moving the slist to a dedicated `void*` field (with a typed free in `H_Handle_free`) is the canonical fix. The lesson: when a handle owns something across a C-API boundary, store it as a typed field, not a packed string. Strings are great for transport and for owned-string data; pointers are great for owned-resource data.
  - **The "always return a handle" Phase 13 contract composes cleanly with a second handle kind.** `H_lua_wait_one` and `H_lua_wait` had to grow a `kind` switch, but the public contract (handle → (result, err) on H.wait) and the variadic shape (N results + N errors) are unchanged. The H_HK_HTTP branch is a single dispatch; the rest of the loop reads identically. The 8-test Unity suite covers the new surface; the existing 7,472 tests are untouched. The lesson: an opaque handle + a single dispatch is the right abstraction for "many async operations, one uniform wait".
  - **`luaL_loadbufferx` is not the right test seam; the OIDC `oidc_rp_http_test_set_response` is.** The first instinct was to use libcurl's test mode or to mock `curl_easy_perform`. Both would have meant a libcurl link dependency in the test, more boilerplate, and a forked libcurl path. The OIDC helper's existing test injection seam (keyed by URL substring, returns canned status+body) was right there in the codebase and Phase 11 already wired it for production. Reusing it kept the new tests at 8 tests / 200 lines instead of ~25 tests / 600 lines, and the test exercises the real OIDC code path, the real Lua→C parameter parsing, the real result-table builder, and the real `H_Handle_check`. **Takeaway**: a test-injection seam that's already in the codebase is almost always cheaper than a new mock. The Phase 11/Phase 16 "test seam is general, not OIDC-specific" insight is the same lesson from a different angle.
  - **Phase 16 closes the "H_Handle is single-purpose" gap from the plan's Phase 2 design note.** The original Phase 2 design said `H_HandleKind` would have `H_HK_QUERY, H_HK_HTTP, H_HK_LLM, H_HK_MAIL, H_HK_NOTIFY`. Phase 13 added the QUERY kind; Phase 16 adds HTTP. Each new kind is a struct field-set + a wait-helper branch + a host function; no other code changes. The lesson: a "kind enum" abstraction that looked heavy in Phase 2 paid off across 12+ phases with almost no per-phase cost. The future LLM/Mail/Notify phases will follow the same pattern (a few fields, a wait helper, a host function, a kind branch in H_lua_wait).
  - **The 4-decision design checkpoint (body cap, headers, Phase 17 scope, etc.) saved implementation time.** Three of the three questions I asked at the start of the phase were answered with the "Recommended" option. The fourth (Phase 17 scope) was the most important: defining Phase 17 as "background-thread fan-out" up front means Phase 16's design can leave the handle's fields in a shape that's easy to copy onto a worker (the slist is already on the handle, owned and freed by `__gc`; the URL/method/body/timeout are POD fields; only the actual curl_easy call needs to move). A Phase 17 design that came after Phase 16 might have required refactoring the handle storage.
  - **The OIDC helper's `_with_cap_and_timeout` and `_with_headers_slist` families are additive.** Every OIDC call site that used the original 3-arg GET or 6-arg POST signature is unchanged. The new functions take a body cap and a timeout that are 0 = "use the OIDC default" so a future OIDC test that wants a tighter cap can pass a value without touching the production path. The `CURLOPT_HEADERFUNCTION` callback adds a per-request memory cost (a small HeaderList growable array) that is only paid by callers that use the new variants; the OIDC default GET/POST do not capture headers (they pass NULL for `CURLOPT_HEADERDATA`). The lesson: when adding a new capability to a shared helper, add new functions for the new capability rather than changing existing signatures. The breakage surface stays at zero, the new capability is opt-in, and the tests for the old behaviour do not have to change.
  - **`scripting_handle_free_slist` was the wrong abstraction.** The first design used a function-pointer indirection to keep `scripting_handle.c` free of `<curl/curl.h>`. That made sense in the abstract (test code that doesn't link libcurl shouldn't drag in curl_slist_free_all) but in practice the only consumer of the function pointer was the `__gc` path inside `H_Handle_free`, which already needs curl types for the slist pointer itself. Including `<curl/curl.h>` in `scripting_handle.c` is the simpler design and the test cost is zero (the Unity test binary already links curl because the OIDC helper is in the test lib). **Takeaway**: indirection to decouple a translation unit from a library is worth it when the library is optional (e.g. a feature gate). It is not worth it when the only caller is the file that owns the indirection — the indirection adds a maintenance cost and a runtime cost (one indirection per __gc call) for no decoupling benefit.
...
- **Dependencies**: Phase 13 (handle + `H.wait` model), Phase 16 (`H.http` request construction and result tables)
- **Status**: **Complete 2026-07-04.** `H.http.get` and `H.http.post` now fan out through a background HTTP worker pool instead of performing the blocking libcurl call on the Lua thread. `H.wait` parks on a per-handle condition variable and returns the same Phase 16 `{ status, headers, body, elapsed_ms }` result table. `H.http.get_sync` and `H.http.post_sync` remain supported as convenience wrappers. 14 new Unity tests were added; full Unity is 7,494 / 7,494; cppcheck is clean; Test 16, Test 17, and Test 43 pass.
- **Expectation**: Consistent API between `H.query` and `H.http` for async usage:

  ```lua
  local a = H.http.get("https://api-a.example/status")
  local b = H.http.get("https://api-b.example/status")

  local ra, rb, ea, eb = H.wait(a, b)
  ```

  The public API did **not** change from Phase 16. The difference is execution: the network call happens on a background HTTP worker, while the Lua thread can create additional handles before waiting.
- **Implementation approach (delivered 2026-07-04)**:
  - **Per-handle synchronization**: `H_Handle` gained HTTP-only fields: `pthread_mutex_t http_mutex`, `pthread_cond_t http_cond`, `http_ready`, result storage (`http_result_status`, `http_result_headers_json`, `http_result_body`, `http_result_error`, `http_result_elapsed_ms`), and a small refcount (`H_Handle_acquire` / `H_Handle_release`). The Lua userdata holds one ref; the HTTP worker holds a second ref while the call is in flight. `__gc` releases, rather than directly freeing, so a handle cannot be freed while a background worker still owns it.
  - **New HTTP worker pool**: `src/scripting/http_pool.{c,h}` adds `ScriptingHttpPool`, `scripting_http_pool_init`, `scripting_http_pool_destroy`, and `scripting_http_pool_submit`. The pool owns a `Queue*` named `scripting_http`, `HTTPWorkerCount` pthreads, and drains on shutdown. Workers dequeue `HttpPoolItem` wrappers, call `scripting_http_get` / `scripting_http_post`, store results on the handle, signal the handle condvar, then release the worker ref.
  - **Config**: `Scripting.HTTPWorkerCount` was added with default **4**. Values `<= 0` skip pool launch; the host API falls back to the Phase 16 inline path if the pool is unavailable.
  - **Launch / landing**: `launch_scripting_subsystem` starts the HTTP pool after the Lua worker pool. `land_scripting_subsystem` stops the Orchestrator, drains/stops Lua workers, then drains/stops the HTTP pool before `scripting_cleanup_state`.
  - **Wait path**: `H_lua_http_wait_one` now calls `scripting_http_pool_submit(handle)` and waits on `handle->http_cond`. If the pool is unavailable, it executes the Phase 16 inline path so `H.http.*` still works in isolated Unity contexts.
  - **Test injection seam**: `src/scripting/http_client.{c,h}` now exposes `scripting_http_test_set_response`, `scripting_http_test_clear_responses`, and `scripting_http_test_get_consumed_count`. Tests can mock `scripting_http_get` / `scripting_http_post` directly without a live HTTP server or a curl mock.
- **Deliverables**:
  - `src/scripting/http_pool.{c,h}` — HTTP worker pool, queue, submit, drain-on-shutdown.
  - `src/scripting/scripting_handle.{c,h}` — per-handle condvar/result fields and refcounted lifetime.
  - `src/scripting/scripting_api.c` — `H_lua_http_wait_one` submits to the pool and waits on the condvar; sync wrappers remain unchanged at the public API level.
  - `src/scripting/http_client.{c,h}` — test injection seam for background-thread tests.
  - `src/config/config_scripting.{c,h}` — `HTTPWorkerCount` default 4, parse, dump.
  - `src/launch/launch_scripting.c` / `src/landing/landing_scripting.c` — HTTP pool lifecycle wiring.
  - New Unity tests:
    - `http_pool_test_lifecycle.c` — 4 tests.
    - `http_pool_test_submit.c` — 5 tests.
    - `scripting_api_test_http_async.c` — 5 tests.
- **Validation**:
  - `mkt` equivalent (`extras/make-trial.sh`) — **PASS**, build successful.
  - `mka` equivalent (`extras/make-all.sh` / Test 01) — **PASS**, 18/18 compilation subtests.
  - `mkp` equivalent (`tests/test_91_cppcheck.sh`) — **PASS**, 1,454 files, 0 issues.
  - `mku http_pool_test_lifecycle` — **PASS**, 4/4.
  - `mku http_pool_test_submit` — **PASS**, 5/5.
  - `mku scripting_api_test_http_async` — **PASS**, 5/5.
  - Full Unity suite (`tests/test_10_unity.sh`) — **PASS**, **7,494 / 7,494 unit tests** (was 7,480 before Phase 17; +14 from new tests, 0 regressions).
  - `tests/test_16_shutdown.sh` — **PASS**, 5/5.
  - `tests/test_17_startup_shutdown.sh` — **PASS**, 9/9.
  - `tests/test_43_scripting.sh` — **PASS**, 31/31; 14/14 engine/variant runs completed the full Orchestrator lifecycle.
- **Notes / Open Questions**:
  - **Queue is unbounded in v1**, matching the scripting job worker pool. If a deployment starts using `H.http` for high-volume fan-out, a future hardening pass should add `Scripting.HTTPQueueMaxDepth` plus a clear submit-time error handle when saturated.
  - **No API break**: `H.http.get`, `H.http.post`, `H.http.get_sync`, `H.http.post_sync`, and `H.wait` keep the Phase 16 calling convention. Phase 17 changes execution semantics only.
  - **Result headers are stored as JSON internally** on the handle (`http_result_headers_json`) so the worker thread never touches the Lua stack. The Lua result table is built only in the waiting Lua thread.
  - **Fallback inline path is intentional** for Unity / isolated contexts where the scripting subsystem was not launched but a Lua context exists. Production, when Scripting is enabled, uses the pool.
  - **ASAN / Test 11 leak validation** remains worth adding later for an `H.http` call in a running Scripting subsystem. Existing lifecycle validation is clean, and the handle refcount prevents the main UAF risk, but a Test 11 subtest with one HTTP call would close the loop.
- **Lessons Learned**:
  - **`queue_enqueue` copies bytes from the data pointer.** A first implementation enqueued `(const char*)h` with `sizeof(H_Handle*)`, which copied the first bytes of the `H_Handle` struct, not the pointer value. Workers dequeued garbage. The fix was to enqueue a small wrapper struct (`HttpPoolItem { H_Handle* handle; }`), so the queue copies the pointer value stored inside the wrapper. Takeaway: if the queue payload is a pointer value, enqueue a wrapper or pass the address of a pointer variable — do not cast the pointee to `char*`.
  - **Resetting `scripting_system_shutdown` is mandatory on pool init.** A prior `scripting_http_pool_destroy` sets the shared shutdown flag to 1. Without resetting it in `scripting_http_pool_init`, newly-started HTTP workers exit on their first idle loop before they process any queued request. This is the same lifecycle lesson from the Lua worker pool: every pool init must publish a live pool and clear shutdown before worker threads can safely run.
  - **Refcounted handles are the right lifetime boundary.** Lua's `__gc` can run before or during a background HTTP operation. Giving the worker a second handle ref means `__gc` can release the Lua ref without freeing memory the worker still uses. The worker stores the result, signals, and releases its ref; only then can the final free occur.
  - **Workers must never touch the Lua stack.** The HTTP worker stores POD/string results on the handle. The waiting Lua thread copies those fields and builds the Lua result table. This preserves Lua's single-thread ownership rule and keeps the background path independent of `lua_State` lifetime.
  - **The direct `http_client` test seam is better than a curl mock.** Mocking `scripting_http_get` / `_post` at the wrapper boundary lets tests exercise the real handle, pool, condvar, and `H.wait` paths without a live server or libcurl mocking. This is the right seam for Phase 17 because Phase 16 already tested the libcurl/OIDC wrapper integration.
  - **Sync wrappers remain useful.** `H.http.get_sync` / `post_sync` now run through the same submit+wait logic when the pool exists and fall back inline when it does not. Keeping them avoids a breaking change and mirrors the `H.query_sync` convenience surface.
...
- **Notes / Open Questions / Advice for implementation**:
  - **Use the Phase 17 handle pattern.** Add `H_HK_LLM` to `H_Handle`, with LLM-specific owned request/result fields and a per-handle condvar/result storage path analogous to HTTP. Do not perform network/model calls on the Lua thread.
  - **Reuse the HTTP pool shape or create an LLM pool.** LLM calls are long-running outbound I/O and should not share the Lua worker pool. Two reasonable options:
    1. Reuse the HTTP pool if the actual call is implemented as an HTTP wrapper and the result shape can stay HTTP-like internally.
    2. Prefer a dedicated `llm_pool` if the call goes through `chat_proxy_send_with_retry` and needs model-engine resolution, retry metadata, and provider-specific result parsing. The dedicated pool is cleaner if LLM calls may be much slower than ordinary HTTP calls.
  - **Database selection should match Scripting's existing rule.** Use `Scripting.DefaultDatabase`; if unset and exactly one database is configured, use that database; if multiple are configured, return an error handle requiring explicit configuration. This matches Phase 11f/13/17 behaviour and avoids surprising model-cache selection.
  - **Resolve by model name only.** Lua should pass `model = "..."`; C resolves against the chosen database's `ChatEngineCache` using `chat_engine_cache_lookup_by_name`. Do not expose provider, URL, or API key to Lua.
  - **`H.llm.list()` should return a handle too.** Keep the async-first convention. It can be backed by `chat_engine_cache_get_all()` / the same enumeration used by the conduit status endpoint. A sync convenience wrapper (`H.llm.list_sync`) may be useful but should be secondary.
  - **Streaming is out of scope for v1.** Return a single structured result (`{ model, provider, text, raw, elapsed_ms }` or similar). Streaming can be added later as a separate handle kind or callback/event surface.
