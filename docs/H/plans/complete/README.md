# Completed Plans

Finished Hydrogen implementation plans. These are historical records — do not
re-open work here. Active backlog: [`/docs/H/TODO.md`](/docs/H/TODO.md).
Active plans folder: [`/docs/H/plans/`](/docs/H/plans/).

| File | Topic |
|------|--------|
| [AUTH_PLAN_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_COMPLETE.md) | Auth endpoints (TOC) |
| [AUTH_PLAN_ACURANZO_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_ACURANZO_COMPLETE.md) | Auth DB / Acuranzo |
| [AUTH_PLAN_API_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_API_COMPLETE.md) | Auth API specs |
| [AUTH_PLAN_DIAGRAMS_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_DIAGRAMS_COMPLETE.md) | Auth diagrams |
| [AUTH_PLAN_ERRORS_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_ERRORS_COMPLETE.md) | Auth errors |
| [AUTH_PLAN_IMPLEMENTATION_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_IMPLEMENTATION_COMPLETE.md) | Auth implementation |
| [AUTH_PLAN_INTEGRATION_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_INTEGRATION_COMPLETE.md) | Auth integration |
| [AUTH_PLAN_OPERATIONS_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_OPERATIONS_COMPLETE.md) | Auth operations |
| [AUTH_PLAN_PERFORMANCE_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_PERFORMANCE_COMPLETE.md) | Auth performance |
| [AUTH_PLAN_PROGRESS_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_PROGRESS_COMPLETE.md) | Auth progress log |
| [AUTH_PLAN_REVIEW_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_REVIEW_COMPLETE.md) | Auth plan review |
| [AUTH_PLAN_SECURITY_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_SECURITY_COMPLETE.md) | Auth security |
| [AUTH_PLAN_TESTS_COMPLETE.md](/docs/H/plans/complete/AUTH_PLAN_TESTS_COMPLETE.md) | Auth tests |
| [CAP_PLAN_QUERY-COMPLETE.md](/docs/H/plans/complete/CAP_PLAN_QUERY-COMPLETE.md) | Cap query endpoint |
| [CHAT_PLAN_ORIGINAL_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_ORIGINAL_COMPLETE.md) | Chat original draft |
| [CHAT_PLAN_SUMMARY_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_SUMMARY_COMPLETE.md) | Chat Phases 1–12 index (historical) |
| [CHAT_PLAN_PHASE_13_SUPERSEDED.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_13_SUPERSEDED.md) | Chat Phase 13 wishlist (superseded by CHAT_FINALE) |
| [CHAT_PLAN_PHASE_1_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_1_COMPLETE.md) | Chat Phase 1 |
| [CHAT_PLAN_PHASE_2_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_2_COMPLETE.md) | Chat Phase 2 |
| [CHAT_PLAN_PHASE_2_5_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_2_5_COMPLETE.md) | Chat Phase 2.5 |
| [CHAT_PLAN_PHASE_3_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_3_COMPLETE.md) | Chat Phase 3 |
| [CHAT_PLAN_PHASE_4_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_4_COMPLETE.md) | Chat Phase 4 |
| [CHAT_PLAN_PHASE_5_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_5_COMPLETE.md) | Chat Phase 5 |
| [CHAT_PLAN_PHASE_6_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_6_COMPLETE.md) | Chat Phase 6 |
| [CHAT_PLAN_PHASE_7_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_7_COMPLETE.md) | Chat Phase 7 |
| [CHAT_PLAN_PHASE_8_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_8_COMPLETE.md) | Chat Phase 8 |
| [CHAT_PLAN_PHASE_9_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_9_COMPLETE.md) | Chat Phase 9 |
| [CHAT_PLAN_PHASE_10_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_10_COMPLETE.md) | Chat Phase 10 |
| [CHAT_PLAN_PHASE_11_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_11_COMPLETE.md) | Chat Phase 11 |
| [CHAT_PLAN_PHASE_12_COMPLETE.md](/docs/H/plans/complete/CHAT_PLAN_PHASE_12_COMPLETE.md) | Chat Phase 12 |
| [CONDUIT_COMPLETE.md](/docs/H/plans/complete/CONDUIT_COMPLETE.md) | Conduit service |
| [CONDUIT_DIAGRAMS_COMPLETE.md](/docs/H/plans/complete/CONDUIT_DIAGRAMS_COMPLETE.md) | Conduit diagrams |
| [CONDUIT_FIX_COMPLETE.md](/docs/H/plans/complete/CONDUIT_FIX_COMPLETE.md) | Conduit auth_query fix |
| [DATABASE_PLAN_COMPLETE.md](/docs/H/plans/complete/DATABASE_PLAN_COMPLETE.md) | Database subsystem |
| [DATABASE_UPDATE_PLAN_COMPLETE.md](/docs/H/plans/complete/DATABASE_UPDATE_PLAN_COMPLETE.md) | Named/typed parameter binding (all engines) |
| [FORTIES_COMPLETE.md](/docs/H/plans/complete/FORTIES_COMPLETE.md) | Tests 40–47 parallel suite load |
| [IMAGE_PLAN_COMPLETE.md](/docs/H/plans/complete/IMAGE_PLAN_COMPLETE.md) | Reporting image_scale (MagickWand) |
| [LOG_FANOUT_PLAN_COMPLETE.md](/docs/H/plans/complete/LOG_FANOUT_PLAN_COMPLETE.md) | Log fanout consumer |
| [LUA_PLAN_COMPLETE.md](/docs/H/plans/complete/LUA_PLAN_COMPLETE.md) | Lua scripting |
| [LUA_CLIENT_COMPLETE.md](/docs/H/plans/complete/LUA_CLIENT_COMPLETE.md) | Client REST script invoke (conduit/script) |
| [MAILRELAY_BLACKBOX_PLAN_COMPLETE.md](/docs/H/plans/complete/MAILRELAY_BLACKBOX_PLAN_COMPLETE.md) | Mail Relay blackbox |
| [MDNS_UPGRADE_COMPLETE.md](/docs/H/plans/complete/MDNS_UPGRADE_COMPLETE.md) | mDNS RFC 6762/6763 server + real client (Phases 0–8) |
| [MCP_COMPLETE.md](/docs/H/plans/complete/MCP_COMPLETE.md) | MCP server (Streamable HTTP + Lua) |
| [MIGRATIONS_COMPLETE.md](/docs/H/plans/complete/MIGRATIONS_COMPLETE.md) | Migration performance |
| [SCHEMATOOL_PLAN_COMPLETE.md](/docs/H/plans/complete/SCHEMATOOL_PLAN_COMPLETE.md) | SchemaTool drift auditor (metadata + catalog, CLI 1.7.1) |
| [STATIC_COMPLETE.md](/docs/H/plans/complete/STATIC_COMPLETE.md) | Static-function purge |
| [TERMINAL_PLAN_COMPLETE.md](/docs/H/plans/complete/TERMINAL_PLAN_COMPLETE.md) | Terminal subsystem |
| [TEST_40_DEBUG_PLAN_COMPLETE.md](/docs/H/plans/complete/TEST_40_DEBUG_PLAN_COMPLETE.md) | Test 40 debug (historical) |
| [TODO_COMPLETE.md](/docs/H/plans/complete/TODO_COMPLETE.md) | Old code-level TODO list |
| [UNITY_CLEANUP_COMPLETE.md](/docs/H/plans/complete/UNITY_CLEANUP_COMPLETE.md) | Unity disabled-test cleanup (43 tests) |
| [VICTORIALOGGING_COMPLETE.md](/docs/H/plans/complete/VICTORIALOGGING_COMPLETE.md) | VictoriaLogs integration |
