# Lithium Documentation

This directory contains the complete documentation for the Lithium PWA.

**Project Home:** [elements/003-lithium/README.md](/elements/003-lithium/README.md)  
**Live Site:** https://lithium.philement.com  
**Coverage:** https://lithium.philement.com/coverage/

---

## Start Here

| Document | Purpose |
|----------|---------|
| **[LITHIUM-TOC.md](LITHIUM-TOC.md)** | **Table of contents** — Master index for all docs |
| **[LITHIUM-INS.md](LITHIUM-INS.md)** | **⚠️ Mandatory** — Coding standards for AI Models |
| **[LITHIUM-DEV.md](LITHIUM-DEV.md)** | Development environment and build tools |
| **[LITHIUM-TST.md](LITHIUM-TST.md)** | Testing framework and coverage |
| **[LITHIUM-FAQ.md](LITHIUM-FAQ.md)** | Common issues and troubleshooting |

---

## Key Documentation

| Document | Purpose |
|----------|---------|
| [LITHIUM-MGR.md](LITHIUM-MGR.md) | Manager system overview |
| [LITHIUM-MGR-NEW.md](LITHIUM-MGR-NEW.md) | New manager step-by-step guide |
| [LITHIUM-TAB.md](LITHIUM-TAB.md) | LithiumTable component |
| [LITHIUM-TAB-TYPES.md](LITHIUM-TAB-TYPES.md) | Column types reference |
| [LITHIUM-API.md](LITHIUM-API.md) | API endpoints |
| [LITHIUM-WEB.md](LITHIUM-WEB.md) | Deployment process |
| [LITHIUM-CHANGELOG.md](LITHIUM-CHANGELOG.md) | Migration notes |
| [AGENTS.md](/elements/003-lithium/AGENTS.md) | Agent / session map |
| [plans/LITHIUM_SPRINT.md](plans/LITHIUM_SPRINT.md) | Active sprint plan |
| [TODO.md](TODO.md) | Backlog pointer |

---

## Tech Stack

Vanilla JavaScript ES modules • Vite • Vitest • Tabulator • CodeMirror 6 • JWT Auth

---

**← Back to [Project README](/elements/003-lithium/README.md)**