...
### Current Runtime Implementation ✅ Complete (April 2026)

**✅ ALL PHASES IMPLEMENTED:** All runtime implementation phases are now complete.

| Component | Status | Notes |
|-----------|--------|-------|
| `loadColtypes()` | ✅ Works | Loads from Lookup 059 Key 0 correctly |
| `loadTableDef()` | ✅ Works | Uses correct key_idx mappings (Phase 0) |
| `resolveColumn()` | ✅ Works | Already uses correct merge order: default → type → colDef |
| `discoverColumns()` | ✅ Works | Enhanced with coltype detection (Phase 2) |
| `columnPri` ordering | ✅ Works | Implemented via `_sortColumnsByPriority()` (Phase 4) |
| Schema validation | ✅ Works | Implemented via `validateTableDef()` (Phase 3) |
| Data-first init | ✅ Works | Loads data before table creation (Phase 4) |
| Lookup 059 key mapping | ✅ Works | tableToKeyIdx uses CORRECT key_idx values per Migration Reference |
...
**✅ ALL PHASES COMPLETE (April 2026):** All 7 phases have been implemented and verified with tests.

---

### Phase 0: Lookup 059 Key Mapping Update ✅ COMPLETE
...
### Phase 1: Default Merge Engine (FOUNDATION) ✅ COMPLETE
...
### Phase 2: Auto-Discovery with Coltype Detection ✅ COMPLETE
...
### Phase 4: Data-First Initialization ✅ COMPLETE
...
### Phase 5: Lookup 059 Integration ✅ COMPLETE
...
### Phase 6: Column Manager (Stage 3) ✅ COMPLETE
...
### Phase 7: Migrate Internal Coltypes ✅ COMPLETE
...
- 597 tests passing
**Post-change (April 2026):** 599 tests passing

### Additional Test Suggestions

To verify each phase is working correctly, new tests should be added:

#### Phase 0 Tests (Lookup 059 Key Mapping)
- ✅ Existing tests verify key mappings work (updated from key_idx 1 to 5 for query-manager)
- Integration: Test loading tableDef for each mapped table path

#### Phase 1 Tests (Default Merge Engine)
- ✅ Already verified by existing tests:
  - "should merge from default stanza first (default → coltype → colDef)"
  - "should apply column overrides over coltype defaults"
- Add: Test unknown properties pass through unchanged

#### Phase 2 Tests (Auto-Discovery)
New tests needed:
```javascript
describe('discoverColumns', () => {
  it('should detect integer coltype from numeric data', () => {
    // Test _detectColtype returns 'integer' for numeric values
  });
  it('should detect decimal coltype from float values', () => {
    // Test _detectColtype returns 'decimal' for floats
  });
  it('should detect boolean coltype from boolean values', () => {
    // Test _detectColtype returns 'boolean' for booleans
  });
  it('should assign columnPri based on field order', () => {
    // Test first field gets columnPri=1, second gets 2, etc.
  });
  it('should derive title from field name', () => {
    // Test query_id -> "Query Id" transformation
  });
});
```
...
describe('validateTableDef', () => {
  ...
  it('should report invalid coltype as error', () => {
    // Test invalid coltype is caught
  });
  it('should report unknown table properties as warnings', () => {
    // Test unknown props don't fail (allow pass-through)
  });
  it('should validate column properties', () => {
    // Test column-level validation works
  });
});
```

#### Phase 4 Tests (Data-First Init)
- ✅ Integration tests confirm table renders with complete tableDef
- Add: Test `_sortColumnsByPriority()` sorts correctly

#### Phase 5-7 Tests
- ✅ Existing functionality covers these phases
- Manual: Verify existing tables work in UI
...
**Post-change (April 2026):** 616 tests passing (+19 new tests)
